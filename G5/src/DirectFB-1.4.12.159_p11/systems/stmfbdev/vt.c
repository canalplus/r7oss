...
/* Pipe file descriptor for terminating the vt_flush thread. */
static int               vt_flush_quitpipe[2];

static void
vt_start_flushing( void )
{
     int ret;
     ret = pipe( vt_flush_quitpipe );
     if (ret < 0) {
          D_PERROR( "DirectFB/linux_input: could not open quitpipe for vt_flush" );
	 }
     dfb_vt->flush = true;
...
{
     dfb_vt->flush = false;
     /* Write to the vt flush quit pipe to cause the thread to terminate */
     (void)write( vt_flush_quitpipe[1], " ", 1 );
/*     direct_thread_cancel( dfb_vt->flush_thread );*/
     direct_thread_join( dfb_vt->flush_thread );
     direct_thread_destroy( dfb_vt->flush_thread );
     close( vt_flush_quitpipe[0] );
     close( vt_flush_quitpipe[1] );
...
vt_flush_thread( DirectThread *thread, void *arg )
{
     D_DEBUG_AT( VT, "%s( %p, %p )\n", __FUNCTION__, thread, arg );
     int                fdmax;
     fdmax = MAX( dfb_vt->fd, vt_flush_quitpipe[0] );
     ...
     while (dfb_vt->flush) {
          ...
          FD_SET(vt_flush_quitpipe[0], &set);

          ret = select( fdmax+1, &set, NULL, NULL, NULL );

          if (FD_ISSET( vt_flush_quitpipe[0], &set ))
               break;
