                        ...
                        CoreFont *font, unsigned int layers, CardState *state )
{
     ...
     scaled_clip = state->clip;
     scaled_ascender = font->ascender;
     scaled_descender = font->descender;
     if (fixed_point) {
          dfb_region_upscale( &scaled_clip );
          scaled_ascender *= DFB_FIXED_POINT_ONE;
          scaled_descender *= DFB_FIXED_POINT_ONE;
     }

     /* simple prechecks */
     if (!(state->render_options & DSRO_MATRIX) &&
         (x > scaled_clip.x2 || y > scaled_clip.y2 + scaled_ascender ||
          y - scaled_descender <= scaled_clip.y1)) {
          return;
     }
