...
linux_input_EventThread( DirectThread *thread, void *driver_data )
{
     ...
     while (1) {
          ...
          if (devt.type != DIET_UNKNOWN) {
               flush_xy( data, false );
               /* let events other than repeat go through */
               if (!(devt.flags & DIEF_REPEAT) ||
                   /* for repeat events, 
                    * send them 3 times slowly than the driver
                    * since it is thrice faster than lirc
                    * and the customer wants the same behaviour as lirc.
                    * lirc skips first 3 repeats so do the same also (i.e. 3*3 => 9 skips)
                    */
                   ((repeat_counter++ > 9) && (repeat_counter % 3 == 0)))
               {
                   dfb_input_dispatch( data->device, &devt );
               }
               if (devt.type == DIET_KEYRELEASE)
               {
                   repeat_counter = 0;
               }
     ...
     D_DEBUG_AT( Debug_LinuxInput, "%s() %d\n", __FUNCTION__,__LINE__ );
     return NULL;
}
