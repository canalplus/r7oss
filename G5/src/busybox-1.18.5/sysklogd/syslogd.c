...
static int ipc_sem_down(void)/* const char *from */
{
	int ret = 0;
	int localerrno;
	...
	do
	{
		//printf("%s try down : %d, %d\n", from,  semctl(G.s_semid, 0, GETVAL, 0),  semctl(G.s_semid, 1, GETVAL, 0));
		ret = semop(G.s_semid, G.SMwdn, 3);
		localerrno = errno;
	} while ((localerrno == EINTR) && (!bb_got_signal) && (ret < 0));
	
	return ret;
} 
static int ipc_sem_up(void)/* const char *from */
{
	int ret = 0;
	int localerrno;
...
static void ipcsyslog_init(void)
{
	...
	G.shmid = shmget(KEY_ID, G.shm_size, IPC_CREAT | 0600);
	if (G.shmid == -1) {
	...
	G.s_semid = semget(KEY_ID, 2, IPC_CREAT | IPC_EXCL | 0700);
	if (G.s_semid == -1) {
		if (errno == EEXIST) {
...
static void log_to_shmem(const char *msg, int len)
{
	...
	if(ipc_sem_down(/*__FUNCTION__*/) == -1) {
...
static void log_to_file(time_t now, const char *msg)
{
...
static void ram_to_file(void)
{
	...
	shbuf_size = G.shbuf->size;
	shbuf_tail = G.shbuf->tail;
	shbuf_data = G.shbuf->data;
	...
	if (cur >= shbuf_size) /* last byte in buffer? */
		cur = 0;
	if (strlen(shbuf_data + cur) == 0) /*there is no message after tail (buffer not full)*/
		cur = 0;
	while (cur != shbuf_tail && cur != shbuf_tail) {
		log_to_file(0, shbuf_data + cur);
...
static void log_locally(time_t now, char *msg)
{
	...
	if(G.to_ram)
	{
		if (ENABLE_FEATURE_IPC_SYSLOG && (option_mask32 & OPT_circularlog) && (G.shbuf == 0)) {
			log_to_file(0, "Start ram log\n");
			ipcsyslog_init();
		}
 ...
 * permanently */
static NOINLINE int create_socket(const char *root)
{
	...
	snprintf(path, PATH_MAX, "%s%s/log", root, (root[strlen(root) - 1] != '/') ? "/" : "");
	...
	dev_log_name = xmalloc_follow_symlinks(path);
	if (dev_log_name) {
...
void sig_usr1(int i)
{
	...
	G.to_ram = 0;
	/*bb_got_signal = i;*/
	signal_SA_RESTART_empty_mask(SIGUSR1, sig_usr1);
}
void sig_usr2(int i)
{
	(void) i; //unused
	...
	G.to_ram = 1;
	/*bb_got_signal = i;*/
	signal_SA_RESTART_empty_mask(SIGUSR2, sig_usr2);
}
#endif
...
#define recvbuf (G.recvbuf)
	...
	roots = getenv("SYSLOGD_ROOTS");
	memset(fds, 0, sizeof(fds));

	fds[0].fd = create_socket("/dev");
	fds[0].events = POLLIN | POLLERR;

	for (s = roots, nfds = 1; s && *s && nfds < MAX_CHROOTS; s++) {
		char c, *b = s;
	...
	printf("syslogd started: BusyBox v" BB_VER "\n");
