...
/* We define initialization functions.  Those are called very early in
   _dl_sysdep_start.  */
...
#define DL_BASE_PLATFORM_INIT dl_base_platform_init ()

static inline void __attribute__ ((unused))
dl_base_platform_init (void)
{
  if (GLRO(dl_base_platform) != NULL && *GLRO(dl_base_platform) == '\0')
    /* Avoid an empty string which would disturb us.  */
