     ...
     const pthread_mutexattr_t *mutexattr;
{
  ...
  switch (__builtin_expect (imutexattr->mutexkind
	...
	{
#ifdef __ST200__
	  /* [SC] ST200 linux kernel is in an unexpected state: it supports
	     PI mutexes but does not support robust_list syscalls.  Glibc just
	     does not expect that: it assumes that if there is no
	     robust_list support, then there can be no PI mutexes.
	     So here I force disable support for PI mutexes.
	  */
	  tpi_supported = -1;
#else
	  int lock = 0;
