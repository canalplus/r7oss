...
/* Default to nothing known about the platform and base_platform. */
  GLRO(dl_platform) = NULL;
  GLRO(dl_base_platform) = NULL;
  ...
  for (av = _dl_auxv; av->a_type != AT_NULL; set_seen (av++))
	...
	GLRO(dl_base_platform) = (void *) av->a_un.a_val;
	break;
      case AT_HWCAP:
...
#ifdef DL_BASE_PLATFORM_INIT
  DL_BASE_PLATFORM_INIT;
#endif

  /* Determine the length of the base platform name.  */
  if (GLRO(dl_base_platform) != NULL)
    GLRO(dl_base_platformlen) = strlen (GLRO(dl_base_platform));
...
_dl_important_hwcaps (const char *platform, size_t platform_len,
		      const char *base_platform, size_t base_platform_len,
 ...
 */
  ...
  if (base_platform != NULL)
    {
      temp[m].str = base_platform;
      temp[m].len = base_platform_len;
      ++m;
    }
