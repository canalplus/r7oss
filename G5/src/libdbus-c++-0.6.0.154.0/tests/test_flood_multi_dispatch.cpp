...
class AnotherDispatcher
{
public:
    AnotherDispatcher(ThreadedDispatcher &disp) :
        m_disp(disp)
    {}

    void enter()
    {
        /* check that it's already running when we enter */
        CPPUNIT_ASSERT(m_disp.is_running());
        while (m_disp.is_running())
            m_disp.do_iteration();
    }
protected:
    ThreadedDispatcher &m_disp;
};

class testFloodMultiDispatch : public CppUnit::TestFixture
{

  CPPUNIT_TEST_SUITE( testFloodMultiDispatch );
...
public:
  ...
  /**
   * @idetails
   * two threads are dispatching the same connection.
   */
    virtual void setUp()
    {
        sem_t *sem;
...
CPPUNIT_TEST_SUITE_REGISTRATION(testFloodMultiDispatch);
