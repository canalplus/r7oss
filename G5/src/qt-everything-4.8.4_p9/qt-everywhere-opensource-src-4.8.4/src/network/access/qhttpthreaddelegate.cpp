...
static QByteArray makeCacheKey(QUrl &url, QNetworkProxy *proxy, const QByteArray &sslConfigHash)
{
    QByteArray result;
    QUrl copy = url;
    ...
    return "http-connection:" + result + sslConfigHash;
}
...
void QHttpThreadDelegate::startRequest()
{
    ...
    QByteArray sslConfigHash;

#ifndef QT_NO_OPENSSL
    if (ssl)
    {
        if (incomingSslConfiguration.localCertificate().isValid())
        {
            sslConfigHash += incomingSslConfiguration.localCertificate().digest(QCryptographicHash::Md5);
            if (!incomingSslConfiguration.privateKey().isNull())
            {
                sslConfigHash += QCryptographicHash::hash(incomingSslConfiguration.privateKey().toDer(), QCryptographicHash::Md5);
            }
        }
    }
#endif
...
#ifdef QHTTPTHREADDELEGATE_DEBUG
    qDebug() << "QHttpThreadDelegate::startRequest() : connection ssl hash = " << sslConfigHash;
#endif

#ifndef QT_NO_NETWORKPROXY
    if (transparentProxy.type() != QNetworkProxy::NoProxy)
        cacheKey = makeCacheKey(urlCopy, &transparentProxy, sslConfigHash);
    else if (cacheProxy.type() != QNetworkProxy::NoProxy)
        cacheKey = makeCacheKey(urlCopy, &cacheProxy, sslConfigHash);
    else
#endif
        cacheKey = makeCacheKey(urlCopy, 0, sslConfigHash);

    // the http object is actually a QHttpNetworkConnection
    httpConnection = static_cast<QNetworkAccessCachedHttpConnection *>(connections.localData()->requestEntryNow(cacheKey));
    if (httpConnection == 0) {
#ifdef QHTTPTHREADDELEGATE_DEBUG
        qDebug() << "QHttpThreadDelegate::startRequest() : New connection has to be made";
#endif
    ...
    {
        qDebug() << "QHttpThreadDelegate::startRequest() : connection already in cache";
    }
#endif
