...
bool QDirectFBPixmapData::fromDirectFBSurface(IDirectFBSurface* p_pDirectDBSurface)
{
    if (!p_pDirectDBSurface)
    {
        qWarning("QDirectFBPixmapData::fromDFBSurface : p_pDirectDBSurface == NULL !");
        invalidate();
        return false;
    }

    DFBResult   l_dfbResult = DFB_OK;
    int         l_nWidth = 0;
    int         l_nHeight = 0;
    l_dfbResult = p_pDirectDBSurface->GetSize(p_pDirectDBSurface, &l_nWidth, &l_nHeight);
    if (l_dfbResult != DFB_OK) {
        DirectFBError("QDirectFBPixmapData::fromDFBSurface() : p_pDirectDBSurface->GetSize() FAILED !", l_dfbResult);
        invalidate();
        return false;
    }

    //printf("%s:%s():%d : l_nWidth = %d, l_nHeight = %d\n", __FILE__, __FUNCTION__, __LINE__, l_nWidth, l_nHeight);
    if (l_nWidth <= 0 || l_nHeight <= 0) {
        qWarning("QDirectFBPixmapData::fromDFBSurface : (l_nWidth <= 0 || l_nHeight <= 0) !");
        invalidate();
        return false;
    }

    imageFormat = QDirectFBScreen::getImageFormat(p_pDirectDBSurface);

    dfbSurface = p_pDirectDBSurface;
    screen->trackDFBSurface(dfbSurface);   // Will keep a new reference on it !

    d = QDirectFBScreen::depth(imageFormat);
    alpha = false;   // Maby this can be calculated (by analysing surface
...
IDirectFBSurface* QPixmap::toDirectFBSurface() const
{
    if (isNull())
        return NULL;

    IDirectFBSurface*   l_pDirectFBSurface = NULL;
    l_pDirectFBSurface = qt_directfb_surface_for_pixmap(*this);

    if (l_pDirectFBSurface)
    {
        // Give one reference to the caller
        l_pDirectFBSurface->AddRef(l_pDirectFBSurface);
    }

    return l_pDirectFBSurface;
}

QPixmap QPixmap::fromDirectFBSurface(IDirectFBSurface* p_pDirectDBSurface)
{
    QDirectFBPixmapData *data = new QDirectFBPixmapData(QDirectFBScreen::instance(), QPixmapData::PixmapType);
    data->fromDirectFBSurface(p_pDirectDBSurface);
    return QPixmap(data);
}
