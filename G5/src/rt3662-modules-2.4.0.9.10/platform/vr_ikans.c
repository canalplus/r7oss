...
static PF_HARD_START_XMIT	rlk_inic_hard_start_xmit;
...
void VR_IKANOS_FP_Init(
	IN iNIC_PRIVATE	*pAd,
	IN PF_HARD_START_XMIT pfun)
{
	UINT32 i;

	/* add all MAC of multiple BSS */
	for(i=0; i<pAd->RaCfgObj.BssidNum; i++)
	{
		apMacAddrConfig(7, pAd->RaCfgObj.MBSSID[i].Bssid, 0xAD);
	} /* End of for */

	rlk_inic_hard_start_xmit = pfun;
...
{
	iNIC_PRIVATE *pAd = (iNIC_PRIVATE *)pNetDev->priv;
	IkanosWlanTxCbFuncP *fp = &IKANOS_WlanDataFramesTx;

	pSkb->apFlowData.txDev = pNetDev;
	pSkb->apFlowData.txApId = IKANOS_PERAP_ID;
	pAd->RaCfgObj.IkanosTxInfo.netdev = pNetDev;
	pAd->RaCfgObj.IkanosTxInfo.fp = fp;
	pSkb->apFlowData.txHandle = &(pAd->RaCfgObj.IkanosTxInfo);
	...
	return rlk_inic_hard_start_xmit(pSkb, pNetDev);
...
iNIC_PRIVATE	*pIkanosAd;

void IKANOS_DataFrameRx(
	IN iNIC_PRIVATE	*pAd,
	IN unsigned char dev_id, 
	IN void				*pRxParam,
	IN struct sk_buff	*pSkb,
	IN UINT32			Length)
{
    ...
    apBuf->specInfoElement = dev_id; //RTMP_GET_PACKET_NET_DEVICE_MBSSID(pSkb); // MBSS
	pIkanosAd = pAd;
	...
	IN struct net_device	*pNetDev)
{
	...
	return rlk_inic_hard_start_xmit(sk, pNetDev);
	...
	IN apPreHeader_t		*pFrame)
{
	...
	dev = pAd->RaCfgObj.MBSSID[index].MSSIDDev;
    if (dev == NULL)
    {
    ...
    pAd->RaCfgObj.IkanosRxInfo[index].netdev = dev;
    pAd->RaCfgObj.IkanosRxInfo[index].fp = &IKANOS_WlanDataFramesTx;

    skb->dev = dev;
    skb->apFlowData.rxApId = IKANOS_PERAP_ID;
    //skb->apFlowData.txHandle = &(txinforx[index]);
    skb->apFlowData.rxHandle = &(pAd->RaCfgObj.IkanosRxInfo[index]);
