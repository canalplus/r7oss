					...
					const uint8_t *buf, int len)
{
	...
	while (len > 0) {
		...
		dma_cache_wback_inv(data->io_data, CACHEDIO_BLOCK_SIZE);
				       ...
				       uint8_t *buf, int len)
{
	...
	while (len > 0) {
		local_irq_save(irq_flags);
		dma_cache_inv(data->io_data, CACHEDIO_BLOCK_SIZE);
...
static int nand_config_emi(int bank, struct nand_timing_data *td)
{
	...
	/*
	   don't compute new timings for EMI BANK,
	   they are applied by JTAG or bootloader
	*/
...
static int __init stm_nand_emi_probe(struct platform_device *pdev)
{
...
#endif
	/* Map cmd and addr addresses (emi_addr_17 and emi_addr_18) except on Pace boards*/
#if defined(CONFIG_SH_ST_CUSTOM002007) || \
	defined(CONFIG_SH_ST_CUSTOM002012) || \
	defined(CONFIG_SH_ST_C275) || \
	defined(CONFIG_SH_ST_CUSTOM003019)
	data->io_cmd = ioremap_nocache(data->emi_base | (1 << 12), 1);
#else
	...
	defined(CONFIG_SH_ST_C275) || \
	defined(CONFIG_SH_ST_CUSTOM003019)
	data->io_addr = ioremap_nocache(data->emi_base | (1 << 11), 1);
#else
