...
struct asc_port {
  ...
  unsigned int ri_irq;
  bool  aux_ctlpresent;
  unsigned int		iobasesc;			/* in/out */
  unsigned char	*ioscname;			/* in/out */
  unsigned int		iosize;			/* in/out */
  unsigned char __iomem	*membasesc;		/* read/write */
...
#define ASC_CTL_NACKDISABLE		0x2000
#define ASC_CTL_PARITYERRTRASH		0x4000
/* ASC_GUARDTIME */
 ...
 *
 *                   baudrate * 16 * 2^16               baudrate * 2^13
 * ASCBaudRate =   ------------------------ + 0.5 =  (--------------------- + 1) / 2
 *                          ICCLK                          ICCLK / 2^8
 */

#define BAUDRATE_VAL_M0(bps, clk)	((clk) / (16 * (bps)))
#define BAUDRATE_VAL_M1(bps, clk)	(((bps << 13) / (clk >> 8) + 1) / 2)
