...
static const struct stv090x_tab stv090x_iq_tab[] = {
	{ 125, 0x0a38 }, //-70
	{  97, 0x2261 }, //-80
	{  81, 0x4600 }, //-90
	{  22, 0x5500 }, //-100
...
int stv090x_set_gpio(struct stv090x_state *state, u8 gpio_number, u8 cfg_value)
{
	/* The cfg_value must not exceed 6 bits */
	if (gpio_number == 0 || gpio_number > 13 || cfg_value >0x3f)
		return 0;

	return stv090x_write_reg(state, STV090x_GPIOxCFG(gpio_number), cfg_value << 1);
}
...
static enum dvbfe_search stv090x_search(struct dvb_frontend *fe, struct dvb_frontend_parameters *p)
{
	...
	state->search_mode = STV090x_SEARCH_AUTO;
	if(p->u.qpsk.symbol_rate < 1000000)	{
		state->srate = 22000000;
...
static int stv090x_get_frontend(struct dvb_frontend* fe, struct dvb_frontend_parameters* params)
{
	struct stv090x_state *state = fe->demodulator_priv;
	s32 car_off;
	params->frequency = state->frequency;
	if (state->config->tuner_get_frequency) {
		if (stv090x_i2c_gate_ctrl(fe, 1) < 0)
			goto err_get;
		if (state->config->tuner_get_frequency(fe, &params->frequency) < 0)
			dprintk(FE_ERROR, 1, "error in retrieve frequency");
		if (stv090x_i2c_gate_ctrl(fe, 0) < 0)
			goto err_get;
	}
err_get:
	dprintk(FE_INFO, 1,"params->frequency = %d\n",params->frequency);
	car_off =  stv090x_get_car_freq(state, state->internal->mclk)/1000;
	dprintk(FE_INFO, 1,"carrier offset = %d\n",car_off);
	params->frequency += car_off;
	dprintk(FE_INFO, 1,"calculated freq = %d  (%d  or %d) \n",params->frequency, params->frequency+9750000,params->frequency+10600000 );
	
	params->u.qpsk.symbol_rate = stv090x_get_srate(state, state->internal->mclk);
	return 0;
}
...
static int stv090x_read_ber(struct dvb_frontend *fe, u32 *ber)
{
	...
	if (status & FE_HAS_LOCK) {
		...
		*ber = (((h << 16) | (m << 8) | l)) * 119; // 10^9/(8*2^20)

	}
	else
	{
		/* be not confused with signal without error and _read_ber when unlocked */
		*ber = (u32)-1;
	}

	if (STV090x_WRITE_DEMOD(state, ERRCTRL1, 0x25) < 0)
		goto err;
...
static int stv090x_read_signal_strength(struct dvb_frontend *fe, u16 *strength)
{
	...
	agc = MAKEWORD16(STV090x_READ_DEMOD(state, AGCIQIN1),
	      STV090x_READ_DEMOD(state, AGCIQIN0));
	dprintk(FE_DEBUG, 1,"stv090x_read_signal_strength: AGC = %x (%d)\n",agc,agc);
	
	if(agc == 0) {
		agc2 = MAKEWORD16(STV090x_READ_DEMOD(state, AGC2I1),
			  STV090x_READ_DEMOD(state, AGC2I0));
		dprintk(FE_DEBUG, 1,"stv090x_read_signal_strength: AGC2 = %x (%d)\n",agc2,agc2);
		str = stv090x_table_lookup(stv090x_iq_tab,
				ARRAY_SIZE(stv090x_iq_tab) - 1, agc2);
	}
	else
	{
...
static int stv090x_read_cnr(struct dvb_frontend *fe, u16 *cnr)
{
	...
	case STV090x_DVBS2:
		...
		if (lock_f) {
			...
			db = stv090x_table_lookup(stv090x_s2cn_tab,
	...
	case STV090x_DSS:
		...
		if (lock_f) {
			...
			val /= 16;
			db = stv090x_table_lookup(stv090x_s1cn_tab,
...
static int stv090x_set_tone(struct dvb_frontend *fe, fe_sec_tone_mode_t tone)
{
	struct stv090x_state *state = fe->demodulator_priv;
	u32 reg;
	unsigned int io_cfg = 0;

	if (state->config->diseqc_envelope_mode) {
		io_cfg = STV090x_DISEQCOxCFG(state->demod);
		reg = stv090x_read_reg(state, io_cfg);
	} else
		reg = STV090x_READ_DEMOD(state, DISTXCTL);
	switch (tone) {
	case SEC_TONE_ON:
		if (state->config->diseqc_envelope_mode) {
			reg &= ~(((1 << STV090x_WIDTH_DISEQCOx_CONFIG_FIELD) -
				1) << STV090x_OFFST_DISEQCOx_CONFIG_FIELD);
			reg |= STV090x_GPIO_CONFIG_LOGICAL_1 <<
				STV090x_OFFST_DISEQCOx_CONFIG_FIELD;
			if (stv090x_write_reg(state, io_cfg, reg) < 0)
				goto err;
		} else {
	...
	case SEC_TONE_OFF:
		if (state->config->diseqc_envelope_mode) {
			reg &= ~(((1 << STV090x_WIDTH_DISEQCOx_CONFIG_FIELD) -
				1) << STV090x_OFFST_DISEQCOx_CONFIG_FIELD);
			reg |= STV090x_GPIO_CONFIG_DISEQCOUTx(state->demod) <<
				STV090x_OFFST_DISEQCOx_CONFIG_FIELD;
			if (stv090x_write_reg(state, io_cfg, reg) < 0)
				goto err;
		} else {
...
{
#ifdef CONFIG_DVB_STV090x_SHARED_GPIO
	extern int (*stv090x_set_gpio_fct)(void*, u8, u8);
#endif
...
#ifdef CONFIG_DVB_STV090x_SHARED_GPIO
	stv090x_set_gpio_fct = stv090x_set_gpio;
#endif
