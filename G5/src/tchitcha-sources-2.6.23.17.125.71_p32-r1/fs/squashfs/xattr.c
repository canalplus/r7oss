...
ssize_t squashfs_listxattr(struct dentry *d, char *buffer, size_t buffer_size)
{
	...
	while (count--) {
		...
		if (handler)
			prefix_size = handler->list(inode, buffer, rest, NULL, name_size);
		if (prefix_size) {
			if (buffer) {
 ...
 */
static size_t squashfs_user_list(struct inode *i, char *list, size_t list_size,
...
static int squashfs_user_get(struct inode *i, const char *name, void *buffer, size_t size)
{
	if (name[0] == '\0')
		return  -EINVAL;

	return squashfs_xattr_get(i, SQUASHFS_XATTR_USER, name,
		buffer, size);
}
 ...
 */
static size_t squashfs_trusted_list(struct inode *i, char *list,
...
static int squashfs_trusted_get(struct inode *i, const char *name,
	void *buffer, size_t size)
{
	if (name[0] == '\0')
		return  -EINVAL;

	return squashfs_xattr_get(i, SQUASHFS_XATTR_TRUSTED, name,
		buffer, size);
}
 ...
 */
static size_t squashfs_security_list(struct inode *i, char *list,
	size_t list_size, const char *name, size_t name_len)
{
...
static int squashfs_security_get(struct inode *i, const char *name,
	void *buffer, size_t size)
{
	if (name[0] == '\0')
		return  -EINVAL;

	return squashfs_xattr_get(i, SQUASHFS_XATTR_SECURITY, name,
		buffer, size);
}
