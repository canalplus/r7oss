			 ...
			 const union ubifs_key *key, const void *buf, int len)
{
	...
	err = ubifs_crypt(c->mount_opts.encryption_mode, 1, &data->data, &out_len, c->uuid, data->key, &(data->padding[0]));
	if (err)
		goto out_free;
 ...
 */
static int recomp_data_node(struct ubifs_info *c, struct ubifs_data_node *dn, int *new_len)
{
	void *buf;
	...
	err = ubifs_crypt(c->mount_opts.encryption_mode,0, &dn->data, &len, c->uuid, dn->key, &(dn->padding[0]));
	if (err)
		goto out;
	...
	err = ubifs_crypt(c->mount_opts.encryption_mode, 1, &dn->data, &out_len, c->uuid, dn->key, &(dn->padding[0]));
	if (err)
		goto out;
		...
		if (err == -ENOENT)
			...
			if (le32_to_cpu(dn->size) <= dlen)
				...
				if (compr_type != UBIFS_COMPR_NONE) {
					...
					ubifs_crypt_truncate(c->mount_opts.encryption_mode, &dlen, &(dn->padding[0]));
