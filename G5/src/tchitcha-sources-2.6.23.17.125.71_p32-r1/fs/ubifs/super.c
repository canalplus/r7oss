...
static const char *ubifs_enc_mode(int mode)
{
	static const char *enc_modes[UBIFS_ENCRYPTION_CNT] = {"off", "on", "strict"};

	ubifs_assert(mode >= 0 && mode < UBIFS_ENCRYPTION_CNT);

	return enc_modes[mode];
}
...
static int ubifs_show_options(struct seq_file *s, struct vfsmount *mnt)
{
	...
	seq_printf(s, ",enc=");
	seq_printf(s, ubifs_enc_mode(c->mount_opts.encryption_mode));
	...
	while ((p = strsep(&options, ","))) {
		...
		case Opt_encryption:
		{
			...
			if (!name)
				return -ENOMEM;
			if (!strcmp(name, "on"))
				c->mount_opts.encryption_mode = UBIFS_ENCRYPTION_ON;
			else if (!strcmp(name, "off"))
				c->mount_opts.encryption_mode = UBIFS_ENCRYPTION_OFF;
			else if (!strcmp(name, "strict"))
				c->mount_opts.encryption_mode = UBIFS_ENCRYPTION_STRICT;
			else {
				ubifs_err("unknown encryption mode \"%s\"", name);
				kfree(name);
				return -EINVAL;
			}
			kfree(name);
...
static int mount_ubifs(struct ubifs_info *c)
{
	...
	c->sbuf = kmalloc(c->leb_size, GFP_DMA);
	if (!c->sbuf)
		goto out_free;

	if (!mounted_read_only) {
		c->ileb_buf = kmalloc(c->leb_size, GFP_DMA);
		if (!c->ileb_buf)
			goto out_free;
	}
...
static int ubifs_remount_rw(struct ubifs_info *c)
{
	...
	c->orph_buf = kmalloc(c->leb_size, GFP_DMA);
	if (!c->orph_buf) {
		err = -ENOMEM;
		goto out;
	}
