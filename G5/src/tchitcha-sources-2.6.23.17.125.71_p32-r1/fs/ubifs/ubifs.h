 ...
 * UBIFS encryption.
 *
 * UBIFS_ENCRYPTION_OFF: encryption disabled
 * UBIFS_ENCRYPTION_ON: encryption enabled (non-encrypted data is allowed, all written data is encrypted)
 * UBIFS_ENCRYPTION_STRICT: encryption strict (non-encrypted read fails)
 * UBIFS_ENCRYPTION_CNT: count of supported encryption modes
 */
enum {
	UBIFS_ENCRYPTION_OFF,
	UBIFS_ENCRYPTION_ON,
	UBIFS_ENCRYPTION_STRICT,
	UBIFS_ENCRYPTION_CNT,
};
 ...
 * @encryption_mode: enable/disable or use strict encryption mode
 */
struct ubifs_mount_opts {
	...
	unsigned int compr_type:2;
	unsigned int encryption_mode:2;
};
...
/* crypto.c */
#ifdef CONFIG_UBIFS_FS_CRYPT
int ubifs_crypt(int mode, int enc, void *buf, int *len, uint8_t *key, uint8_t *iv, uint8_t *pad);
void ubifs_crypt_truncate(int mode, int *len, uint8_t *pad);
#else
static inline int ubifs_crypt(int mode, int enc, void *buf, int *len, uint8_t *key, uint8_t *iv, uint8_t *pad) { return 0; }
static inline void ubifs_crypt_truncate(int mode, int *len, uint8_t *pad) {}
#endif
