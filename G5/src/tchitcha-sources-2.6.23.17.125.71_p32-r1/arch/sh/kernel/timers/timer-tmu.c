...
{
	u16 tcr_val = TMU_TCR_INIT;
	u8 divisor  = TMU_TCR_INIT & 0x7;
	int tmu_num = clk->name[3]-'0';

	if (tmu_num == TMU1)
		tcr_val &= ~(1 << 5);
	ctrl_outw(tcr_val, TMU0_TCR+(tmu_num*0xC));
...
unsigned long tmu1_read_raw(void)
{
	return _tmu_read(TMU1);
}

void tmu1_restart(unsigned long cycles)
{
	if (cycles == 0)
		tmu_timer_set_interval(TMU1, ~0, 1);
	else {
		_tmu_stop(TMU1);
		ctrl_outl(cycles, TMU1_TCNT);
		_tmu_start(TMU1);
	}
}
...
static int tmu2_set_rate(struct clk *clk, unsigned long freq)
{
