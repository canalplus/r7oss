...
extern void sh_reset_TRAP0 (void);
...
void machine_restart(char *__unused)
{
	...
	if (sh_mv.mv_reset) {
		...
		/* Restore the reset chain. Note that we cannot use the sysconfig API here, as it calls
		 * kmalloc(GFP_KERNEL) but we get here with interrupts disabled. */
		writel(DEFAULT_RESET_CHAIN, (void __iomem *)(SYS_CONFIG_BASE + SYS_CONFIG_9));
	}
				 ...
				 struct pt_regs __regs)
{
	...
	struct pt_regs *regs = RELOC_HIDE(&__regs, 0);
	printk(KERN_NOTICE "Hit hardware watchpoint at 0x%.8x\n", regs->pc);
	if (__current_hw_watch.handler)
		__current_hw_watch.handler(r4, r5, r6, r7, regs);
	else {
		show_regs(regs);
		dump_stack();
	}
	if (__current_hw_watch.oneshot == 1)
		disable_hw_watch();
#else
	/* Clear tracing.  */
#if defined(CONFIG_CPU_SH4A)
