...
#if defined(CONFIG_ST_BIOS)
void *bios_entry_address = NULL;
EXPORT_SYMBOL(bios_entry_address);

static inline void __init st_bios_vbr_init(void)
{
	register unsigned long vbr;

	/* Read the old value of VBR configured by the BIOS before
	 * the kernel started and get main handler function offset
	 * from *(vbr + 8 bytes)
	 */
	asm volatile("stc vbr, %0" : "=r" (vbr));

	if (vbr){
		bios_entry_address = (void *)(*((unsigned long *)(vbr + 8)));

		pr_info("BIOS entry address at 0x%08lx\n",
				(unsigned long)bios_entry_address);
	}
}
#endif /* CONFIG_ST_BIOS */
