/*
 * arch/sh/boards/st/custom002012/setup.c
 *
 ...
 *
 * STMicroelectronics custom002012-SDK support.
 */
...
static void __init custom002012_setup(char** cmdline_p)
{
#ifdef CONFIG_SH_G5_SPLASH
	int show_it = 0;
	...
	printk("STMicroelectronics custom002012-SDK board initialisation\n");
...
static int custom002012_phy_reset(void* bus)
{
...
static struct plat_stmmacphy_data phy_private_data = {
	...
	.phy_reset = &custom002012_phy_reset,
	.get_mac_addr = &get_plat_mac_addr,
};

static struct platform_device custom002012_phy_device = {
	.name		= "stmmacphy",
	.id		= 0,
	.num_resources	= 1,
	.resource	= (struct resource[]) {
...
static struct platform_device custom002012_leds_device = {
        .name = "leds-gpio",
        .id = 0,
...
static struct platform_device custom002012_nand_device = {
	.name		= "gen_nand",
	.id		= 0,
	.num_resources	= 1,
	.resource	= (struct resource[]) {
...
static struct plat_stmnand_data nand_config = {
	...
	.timing_data = &(struct nand_timing_data) {
		...
		.rd_on		= 10,
		.rd_off		= 40,
		.chip_delay	= 45,		/* PACE times: 25us (Hynix), 30us (Toshiba); Wyplay test: 40us (FDMA + printk disabled) */
	},
...
static struct gpio_keys_button custom002012_buttons[] = {
	{
		.code = KEY_Q,
...
static struct gpio_keys_platform_data custom002012_button_data = {
	.buttons = custom002012_buttons,
	.nbuttons = ARRAY_SIZE(custom002012_buttons),
	.rep = 1,
};


static struct platform_device custom002012_button_device = {
	.name = "gpio-keys",
	.id = -1,
	.num_resources = 0,
	.dev = {
		.platform_data = &custom002012_button_data,
	}
};
#endif


static struct platform_device *custom002012_devices[] __initdata = {
#ifdef CONFIG_LEDS_GPIO
        &custom002012_leds_device,
#endif
        &physmap_flash,
	&custom002012_phy_device,
#if defined(CONFIG_MTD_NAND_PLATFORM) && !defined(PACE_CYFRA_SUPPORT)
	&custom002012_nand_device,
#endif
#if defined(CONFIG_KEYBOARD_GPIO) || defined(CONFIG_KEYBOARD_GPIO_MODULE)
	&custom002012_button_device,
#endif
};
...
static int custom002012_usbhub_reset(void)
{
...
static void __init custom002012_usbdevices_reset(void)
{
...
static int custom002012_frontend_reset(void)
{
...
static int custom002012_wlan_reset(void)
{
...
static int custom002012_hdmi_enable(void)
{
...
static void custom002012_hardware_reboot_enable(void)
{
	stpio_request_set_pin(1, 2, "RESET_ENnot", STPIO_OUT, 0);
}

/* Ind 2/4 dedicated line, not-connected on Ind 3/5 */
static void custom002012_disable_st8_reset(void)
{
	struct stpio_pin *pin;
...
{
	custom002012_disable_st8_reset();

	custom002012_hardware_reboot_enable();

	stx7105_configure_ssc(&ssc_private_info);

	custom002012_usbhub_reset();
	stx7105_configure_usb(0, &usb_init[0]);
	stx7105_configure_usb(1, &usb_init[1]);
	custom002012_usbdevices_reset();

	custom002012_hdmi_enable();
	custom002012_frontend_reset();
	custom002012_wlan_reset();
	...
	return platform_add_devices(custom002012_devices, ARRAY_SIZE(custom002012_devices));
}
arch_initcall(device_init);

static void __iomem *custom002012_ioport_map(unsigned long port, unsigned int size)
{
...
static void __init custom002012_init_irq(void)
{
...
static void custom002012_reset(void)
{
...
struct sh_machine_vector mv_custom002012 __initmv = {
	.mv_name		= "custom002012",
	.mv_setup		= custom002012_setup,
	.mv_nr_irqs		= NR_IRQS,
	.mv_init_irq		= custom002012_init_irq,
	.mv_ioport_map		= custom002012_ioport_map,
	.mv_reset		= custom002012_reset,
};
