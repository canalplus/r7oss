...
static struct stv6120_config stv6120_config = {
	.refclk                 = 16000000,
};
...
static struct dvb_frontend* tuner_stv6120_attach(void *config, struct dvb_frontend *fe, struct i2c_adapter *i2c)
{
	const struct fe_config* const fe_cfg = (struct fe_config *)config;
	struct stv6120_devctl *fe2 = NULL;

	stv6120_config.addr = fe_cfg->tuner_address;
	if (fe_cfg->demod_section == 1) {
		stv6120_config.rfin = STV6120_RFB_IN;
		stv6120_config.path = STV6120_PATH_1;
	} else {
		stv6120_config.rfin = STV6120_RFC_IN;
		stv6120_config.path = STV6120_PATH_2;
	}
	fe2 = stv6120_attach(fe, &stv6120_config, i2c);
...
static struct dvb_frontend *lnbr_a8292_attach(void *config, struct dvb_frontend *fe, struct i2c_adapter *i2c)
{
	const struct fe_config* const fe_cfg = (struct fe_config *)config;
	const struct a8292_config lnbr_cfg = {
		.i2c_addr	= fe_cfg->lnb_address,
		.section	= fe_cfg->lnb_section,
	};

	return a8292_attach(fe, i2c, &lnbr_cfg);
}
		...
		.platform_data = (struct plat_frontend_config[]) {
			{
				...
				.channels = (struct plat_frontend_channel[]) {
					{
						/* demodulator 2 is wired to STi7105:TSin2 */
						.option = STM_TSM_CHANNEL_2 | STM_PACKET_CLOCK_VALID | STM_SERIAL_NOT_PARALLEL,
						.config = (struct fe_config[]) {
							{
								...
								.lnb_address = (0x16 >> 1),
								.lnb_section = 1, /* LNB2 = "ENTREE SAT" port */
							}
						},

						.demod_attach = demod_900_attach,
						.demod_i2c_bus = 0,	/* i2c adapter 0 */
						.tuner_attach = tuner_stv6120_attach,
						.tuner_i2c_bus = 0,	/* i2c adapter 0 */
						.lnb_attach = lnbr_a8292_attach,
		...
		.platform_data = (struct plat_frontend_config[]) {
			{
				...
				.channels = (struct plat_frontend_channel[]) {
					{
						/* demodulator 1 is wired to STi7105:TSin3 */
						.option = STM_TSM_CHANNEL_3 | STM_PACKET_CLOCK_VALID | STM_SERIAL_NOT_PARALLEL,
						.config = (struct fe_config[]) {
							{
								.demod_address = (0xd0 >> 1),
								...
								.tuner_id = DVB_PLL_THOMSON_DTT759X,
								.lnb_address = (0x16 >> 1),
								.lnb_section = 0, /* LNB1 = "SAT AUX" port */
							}
						},
...
static struct platform_device *board_panty[] __initdata = {
	&tsm_device,
	&pti1_device,
	&pti2_device,
};
...
{
	return platform_add_devices(board_panty, sizeof(board_panty) / sizeof(struct platform_device *));
}
