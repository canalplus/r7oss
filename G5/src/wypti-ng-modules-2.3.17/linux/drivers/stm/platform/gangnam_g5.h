/*
 * gangnam_g5 Platform registration
 *
...
#ifndef _GANGNAM_G5_H
#define _GANGNAM_G5_H
...
static struct dvb_frontend* demod_900_attach(void *config, struct i2c_adapter *i2c)
{
	...
	return stv090x_attach(&stv090x_config, i2c, section);
}
...
static struct dvb_frontend* tuner_stv6110_attach(void *config, struct dvb_frontend *fe, struct i2c_adapter *i2c)
{
	...
	struct stv6110x_config *stv6110x_config = kmalloc(sizeof(struct stv6110x_config), GFP_KERNEL);
...
{
	const struct fe_config* const fe_cfg = (struct fe_config *)config;
	u8 set_mask = LNBH24_TTX | LNBH24_EN;
	u8 clear_mask = LNBH24_TEN | LNBH24_LLC;

	return lnbh24_attach(fe, i2c, set_mask, clear_mask, fe_cfg->lnb_address);
}
		...
		.platform_data = (struct plat_frontend_config[]) {
			{
				...
				.channels = (struct plat_frontend_channel[]) {
					{
						/* demodulator 2 is wired to STi7105:TSin0 */
						...
						.config = (struct fe_config[]) {
							{
								...
								.lnb_address = (0x12 >> 1),	/* LNBR-A = "ENTREE SAT" port */
							}
						},
...
static struct platform_device *board_gangnam_g5[] __initdata = {
	&tsm_device,
	&pti1_device,
	&pti2_device,
};
...
{
	return platform_add_devices(board_gangnam_g5, sizeof(board_gangnam_g5) / sizeof(struct platform_device *));
}
