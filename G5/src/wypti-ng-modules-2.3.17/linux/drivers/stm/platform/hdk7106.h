 ...
 *
 * Nick generated this 7106 equivalent file from the 
 * 7105 file by doing a global replace of pdk7105 to hdk7106
 */
#ifndef _HDK7106_H_
#define _HDK7106_H_

#ifdef CONFIG_SH_ST_HDK7106 
...
#else
    .platform_data 	= (struct stm_pad_config[]) {
      {
        .gpios_num = 1,
        .gpios = (struct stm_pad_gpio[]) {
	    STM_PAD_PIO_OUT(5, 5, 0x01)		// NOTE the 01 is a deduced value, the 7105 datasheet seems to omit this pin
	}
      }
    }
#endif
...
static struct platform_device *board_HDK7106[] __initdata = {
	&tsm_device,
//	&diseqc_device,
        &pti1_device,
        &pti2_device,
...
{	
	if (adapter->nr == 2) {
		...
		int ret;
		struct i2c_msg msg = { .addr = 0x40, .flags = 0, .buf = (__u8 *)i2c_cmd, .len = 3};
...
#define STARWIN_RESET_PIN	stm_gpio(5, 7)
	...
	iounmap((void __iomem *)data);

    return platform_add_devices(board_HDK7106,sizeof(board_HDK7106)/sizeof(struct platform_device*));
}
