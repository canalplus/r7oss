...
offset_t url_fseek(ByteIOContext *s, offset_t offset, int whence)
{
    ...
    if (!s->must_flush &&
        ...
        s->buf_ptr = s->buffer;
        if (!s->seek || (offset = s->seek(s->opaque, offset, SEEK_SET)) < 0)
            return offset >= 0? AVERROR(EPIPE) : offset;
        s->pos = offset;
    }
    s->eof_reached = 0;
    s->error = 0;
...
int url_fopen2(ByteIOContext **s, const char *filename, int flags, int *interrupted)
{
    URLContext *h;
    int err;

    err = url_open2(&h, filename, flags, interrupted);
    if (err < 0)
        return err;
    err = url_fdopen(s, h);
    if (err < 0) {
...
{
    return url_fopen2(s, filename, flags, NULL);
}
