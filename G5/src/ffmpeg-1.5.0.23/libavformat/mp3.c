...
/* fixup ID3v1 genre tags masquerading as ID3v2.
 * see http://www.id3.org/d3v2.3.0 section 4.2.1 */
static void id3v2_fixup_v1_genre(char *genre, size_t len)
{
    char *v1_fixup = NULL;

    if (av_strstart(genre, "(", (const char **)&v1_fixup)) {
        unsigned long i;
        char *err = NULL;

        if (v1_fixup && (i = strcspn(v1_fixup, ")"))) {
            char bkp = v1_fixup[i];
            v1_fixup[i] = '\0';
            i = strtol(v1_fixup, &err, 10);

            if (err && *err == '\0' && i <= ID3v1_GENRE_MAX)
                av_strlcpy(genre, id3v1_genre_str[i], len);
            else
                v1_fixup[i] = bkp;
            }
        }
...
static void id3v2_parse(AVFormatContext *s, int len, uint8_t version, uint8_t flags)
{
    ...
    while(len >= taghdrlen) {
        ...
        case MKBETAG(0,   'T', 'C', 'O'):
            ...
            id3v2_fixup_v1_genre(s->genre, sizeof(s->genre));
            break;
                             ...
                             const uint8_t *buf, int buf_size)
{
    ...
    for(i = 0; i < buf_size; i++) {
        ...
        PUT_UTF8(buf[i], tmp, if (q - str < str_size - 1) *q++ = tmp;)
...
static int mp3_read_probe(AVProbeData *p)
{
    ...
    uint32_t header;
    uint8_t *real_start, *buf, *buf2, *end;
    AVCodecContext avctx;

    /* skip leading zeroes */
    real_start = p->buf;
    for (; real_start < p->buf + p->buf_size && *real_start == 0; real_start++);

    if(id3v2_match(real_start))
        return AVPROBE_SCORE_MAX/2+1; // this must be less than mpeg-ps because some retards put id3v2 tags before mpeg-ps files

    max_frames = 0;
    buf = real_start;
    end = FFMIN(buf + 4096, p->buf + p->buf_size - sizeof(uint32_t));
