        ...
        switch (id) {
            ...
            case MATROSKA_ID_TRACKCONTENTENCODINGS: {
                ...
                while (res == 0) {
                    ...
                    if (id != MATROSKA_ID_TRACKCONTENTENCODING) {
                        av_log(matroska->ctx, AV_LOG_INFO,
                                "Unknown encodings track header entry "
                                "0x%x - ignoring\n", id);
                        res = ebml_read_skip(matroska);
                        goto next;
                    }
                    ...
                    while (res2 == 0) {
                        ...
                        if (id != MATROSKA_ID_ENCODINGCOMPRESSION) {
                            ebml_read_skip(matroska);
                            goto next1;
                        }

                        if ((res2 = ebml_read_master(matroska, &id)) < 0)
                            break;
                        ...
                        while (res3 == 0) {
                            ...
                            switch (id) {
                                case MATROSKA_ID_ENCODINGCOMPALGO: {
                                    ...
                                    if ((res3 = ebml_read_uint(matroska, &id, &algo)) < 0)
                                        break;

                                    if (algo == MATROSKA_TRACK_ENCODING_COMP_HEADERSTRIP)
                                        track->hdr_strip = 1;
                                    break;
                                }
                                ...
                                case MATROSKA_ID_ENCODINGCOMPSETTINGS: {
                                    ...
                                    if ((res3 = ebml_read_binary(matroska, &id, &data, &size)) < 0)
                                        break;

                                    if (track->hdr_data)
                                        av_free(track->hdr_data);

                                    track->hdr_data = data;
                                    track->hdr_data_size = size;
                                    break;
                                }

                                default:
                                    av_log(matroska->ctx, AV_LOG_INFO, "unknown comp id: %x\n", id);
                                    ebml_read_skip(matroska);
                                    break;
                            }
                      ...
                      AVFormatParameters *ap)
{
    ...
    if (matroska->index_parsed) {
        ...
        for (i=0; i<matroska->num_indexes; i++) {
            ...
            if (stream >= 0 && matroska->ctx->streams[stream])
    ...
    if (res == 0) {
        ...
        for (n = 0; n < laces; n++) {
            if (st->codec->codec_id == CODEC_ID_RA_288 ||
                ...
                if (matroska->tracks[track]->hdr_strip && matroska->tracks[track]->hdr_data)
                    offset = matroska->tracks[track]->hdr_data_size;
                ...
                if (offset)
                        memcpy(pkt->data, matroska->tracks[track]->hdr_data, offset);
