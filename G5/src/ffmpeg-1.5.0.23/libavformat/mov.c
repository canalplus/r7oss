        ...
        if(st->codec->codec_type==CODEC_TYPE_VIDEO) {
            ...
            if ((color_depth == 2) || (color_depth == 4) ||
                ...
                if (color_greyscale) {
                    ...
                    if (color_start < 0 || color_end >= sizeof(c->palette_control.palette) / sizeof(c->palette_control.palette[0])) {
                        av_log(c->fc, AV_LOG_ERROR, "inconsistent color count\n");
                        return -1;
                    }
...
static int mov_read_ctts(MOVContext *c, ByteIOContext *pb, MOV_atom_t atom)
{
    ...
    for(i=0; i<entries; i++) {
        ...
        if (duration < 0) {
            ...
            c->fc->ctx_flags &= ~AVFMTCTX_RELIABLE_PTS;
            break;
        }
...
static int mov_read_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    for (i = 0; i < mov->total_streams; i++) {
        ...
        if (s->streams[i]->discard != AVDISCARD_ALL && msc->current_sample < msc->sample_count) {
            ...
            if ((s->pb->is_streamed && (!sample || current_sample->pos < sample->pos)) || /* on streamed sources, avoid at all cost seeking backward */
                (!s->pb->is_streamed && dts < best_dts)) {
