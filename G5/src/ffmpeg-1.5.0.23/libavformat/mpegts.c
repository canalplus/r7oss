...
typedef void PESCallback(MpegTSFilter *f, const uint8_t *buf, int len, int is_start, int cc_ok);
...
struct PESContext {
    ...
    float bitrate; /**< approximate bitrate used for seeking (actually byterate) */
    int64_t bitrate_calc_pts;
    offset_t bitrate_calc_pos;
};
...
static void pmt_cb(MpegTSFilter *filter, const uint8_t *section, int section_len)
{
    ...
    for(;;) {
        ...
        for(;;) {
            ...
            if (stream_type == STREAM_TYPE_PRIVATE_DATA) {
                if((desc_tag == 0x6A) || (desc_tag == 0x7A)) {
                    ...
                    /* teletext data */
                    stream_type = STREAM_TYPE_DATA_TELETEXT;
		}
            }
        ...
        case STREAM_TYPE_DATA_TELETEXT:
...
/* push up EIT sections in a data stream */
static void eit_cb(MpegTSFilter *filter, const uint8_t *section, int section_len)
{
    ...
    if (pkt && ts->eit_stream && av_new_packet(pkt, section_len) == 0) {
        memcpy(pkt->data, section, section_len);
        pkt->stream_index = ts->eit_stream->index;
...
static void mpegts_scan_eit(MpegTSContext *ts)
{
...
/* number of second over which we calculate the average bitrate */
#define BITRATE_AVERAGE_TIME 1
static void update_bitrate(PESContext *pes, int64_t pts)
{
    MpegTSContext *ts = pes->ts;
    AVFormatContext *st = ts->stream;
    offset_t pos;
    int64_t d;
    if (pts == AV_NOPTS_VALUE) return;

    d = pts - pes->bitrate_calc_pts;
    pos = url_ftell(st->pb);
    if (pes->bitrate_calc_pts != AV_NOPTS_VALUE &&
        d >= 0 && d < (BITRATE_AVERAGE_TIME + 10) * 90000) {
        if (d < BITRATE_AVERAGE_TIME * 90000)
            return;
        pes->bitrate = (pos - pes->bitrate_calc_pos) * 90000.0f / d;
    }
    
    pes->bitrate_calc_pts = pts;
    pes->bitrate_calc_pos = pos;
}

/* return non zero if a packet could be constructed */
static void mpegts_push_data(MpegTSFilter *filter,
                             const uint8_t *buf, int buf_size, int is_start, int cc_ok)
{
            ...
            if (len > 0) {
                ...
                if (pes->st && av_new_packet(pkt, len) == 0) {
                    ...
                    if (pes->discontinuity)
                        pkt->flags |= PKT_FLAG_TAINTED;

                    pes->discontinuity = 0;
                    ts->stop_parse = 1;

                    update_bitrate(pes, pkt->dts);
                    return;
                } else
     	            pes->discontinuity = 0;
            }
...
static PESContext *add_pes_stream(MpegTSContext *ts, int pid, int pcr_pid, int stream_type)
{
    ...
    pes->bitrate = 700.0f * 1024;
    pes->bitrate_calc_pts = AV_NOPTS_VALUE;
...
static void handle_packet(MpegTSContext *ts, const uint8_t *packet)
{
    ...
    if(pid && discard_pid(ts, pid))
        return;
    if (packet[1] & 0x80) {
        av_log(s, AV_LOG_ERROR, " transport_error_indicator=1 @ 0x%llx: pid=0x%x\n",
               url_ftell(s->pb) - TS_PACKET_SIZE, pid);
        return;
    }
    ...
    if (!cc_ok)
        av_log(s, AV_LOG_ERROR, " TS discontinuity @ 0x%llx: pid=%d, cc=%d, last_cc=%d\n",
               url_ftell(s->pb) - TS_PACKET_SIZE, pid, cc, tss->last_cc);
    tss->last_cc = cc;
                              ...
                              AVFormatParameters *ap)
{
    ...
    if (s->iformat == &mpegts_demuxer) {
        ...
        /* create a new stream for EIT sections.
         * maybe we should make this optional through AVFormatParameters. */
        mpegts_scan_eit(ts);
        ts->eit_stream = av_new_stream(s, EIT_PID);
        av_set_pts_info(ts->eit_stream, 33, 1, 90000);
        ts->eit_stream->codec->codec_type = CODEC_TYPE_DATA;
        ts->eit_stream->codec->codec_id = CODEC_ID_DVB_EIT;
                              ...
                              int64_t *ppos, int64_t pos_limit)
{
    ...
    if (find_next) {
        for(;;) {
            url_fseek(s->pb, pos, SEEK_SET);
            if (pos > pos_limit ||
		get_buffer(s->pb, buf, TS_PACKET_SIZE) != TS_PACKET_SIZE)
                return AV_NOPTS_VALUE;
            if (!(buf[1] & 0x80) && 
		(pcr_pid < 0 || (AV_RB16(buf + 1) & 0x1fff) == pcr_pid) &&
...
#define SEEK_BASED_ON_BITRATE 1
static int read_seek(AVFormatContext *s, int stream_index, int64_t target_ts, int flags){
    MpegTSContext *ts = s->priv_data;
    uint8_t buf[TS_PACKET_SIZE];
    int64_t pos;

#if SEEK_BASED_ON_BITRATE    
    PESContext *pes = s->streams[stream_index]->priv_data;
    int i;
    if (pes->bitrate_calc_pts == AV_NOPTS_VALUE) return -1;
    pos = pes->bitrate_calc_pos + pes->bitrate * (target_ts - pes->bitrate_calc_pts) / 90000.0f;
    pos -= (pos % ts->raw_packet_size);
    if (pos < 0) pos = 0;
    for (i = 0; i < s->nb_streams; i++)
        if (s->streams[i]->priv_data)
            ((PESContext*)s->streams[i]->priv_data)->bitrate_calc_pts = AV_NOPTS_VALUE;
#else
    ...
    for(;;) {
        pos = url_fseek(s->pb, pos, SEEK_SET);
        if (pos < 0) return -1;
        pos -= (pos % ts->raw_packet_size);
