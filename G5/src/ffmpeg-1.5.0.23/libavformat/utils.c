...
static int av_read_frame_internal(AVFormatContext *s, AVPacket *pkt)
{
    ...
    for(;;) {
	    ...
	    if (s->cur_pkt.flags & PKT_FLAG_TAINTED)
		    tainted = 1;
...
int av_seek_frame_binary2(AVFormatContext *s, int stream_index, int64_t target_ts, int flags, int (* cb)(void *), void * userdata){
    ...
    pos= av_gen_search2(s, stream_index, target_ts, pos_min, pos_max, pos_limit, ts_min, ts_max, flags, &ts, avif->read_timestamp, cb, userdata);
    if(pos<0)
        return -1;
...
int av_seek_frame_binary(AVFormatContext *s, int stream_index, int64_t target_ts, int flags){
    return av_seek_frame_binary2(s, stream_index, target_ts, flags, NULL, NULL);
}
...
int64_t av_gen_search2(AVFormatContext *s, int stream_index, int64_t target_ts, int64_t pos_min, int64_t pos_max, int64_t pos_limit, int64_t ts_min, int64_t ts_max, int flags, int64_t *ts_ret, int64_t (*read_timestamp)(struct AVFormatContext *, int , int64_t *, int64_t ), int (* cb)(void *), void * userdata){
    ...
    if(ts_max == AV_NOPTS_VALUE){
        ...
        do{
            ...
            if (cb && cb(userdata))
                return -1;
        }while(ts_max == AV_NOPTS_VALUE && pos_max >= step && step < 1024 * 1024);
...
int64_t av_gen_search(AVFormatContext *s, int stream_index, int64_t target_ts, int64_t pos_min, int64_t pos_max, int64_t pos_limit,
                       int64_t ts_min, int64_t ts_max, int flags, int64_t *ts_ret, int64_t (*read_timestamp)(struct AVFormatContext *, int , int64_t *, int64_t )){
    return  av_gen_search2(s, stream_index, target_ts, pos_min, pos_max, pos_limit, ts_min, ts_max, flags, ts_ret, read_timestamp, NULL, NULL);
}
...
static int av_seek_frame_generic(AVFormatContext *s,
                                 int stream_index, int64_t timestamp, int flags, int (* cb)(void *), void * userdata)
{
    int index;
    AVStream *st;
    AVIndexEntry *ie;
    ...
    if(index < 0 || index==st->nb_index_entries-1){
        ...
        if(st->index_entries && st->nb_index_entries){
            ...
            if (!s->iformat->read_seek ||
                s->iformat->read_seek(s, stream_index, ie->timestamp,
                                      flags | AVSEEK_FLAG_ANY) < 0){
...
int av_seek_frame2(AVFormatContext *s, int stream_index, int64_t timestamp, int flags,  int (* cb)(void *), void * userdata)
{
    ...
    if(s->iformat->read_timestamp)
        return av_seek_frame_binary2(s, stream_index, timestamp, flags, cb, userdata);
    else if(s->iformat->flags & AVFMT_GENERIC_INDEX)
        return av_seek_frame_generic(s, stream_index, timestamp, flags, cb, userdata);
    else
        return ret;
...
//                        av_log(NULL, AV_LOG_ERROR, "%f %f\n", get_std_framerate(j) / 12.0/1001, error);
    ...
    if(i!=ic->nb_streams){
        ...
        for(i=0;i<ic->nb_streams;i++) {
            st = ic->streams[i];
            if (codec_identified[st->index]) 
                av_seek_frame2(ic, st->index, 0.0, 0, cb, userdata);                    
            st->cur_dts= st->first_dts;
        }
...
{
    return av_find_stream_info2(ic, NULL, NULL);
}
...
static void dump_stream_format(AVFormatContext *ic, int i, int index, int is_output)
{
    ...
    if (st->language_type > 1 && st->language_type < 4)
        av_log(NULL, AV_LOG_INFO, " %s", ltypes[st->language_type]);
...
void new_url_split(char *proto, int proto_size,
               char *authorization, int authorization_size,
               char *hostname, int hostname_size,
               int *port_ptr,
               char *path, int path_size,
