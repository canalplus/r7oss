...
int raw_read_partial_packet(AVFormatContext *s, AVPacket *pkt)
{
    int ret, size;
    ...
    for(; buf < end; buf++) {
        ...
        for(frames = 0; buf2 < end; frames++) {
            ...
            if(buf2 + hdr.frame_size > end ||
               av_crc(av_crc8005, 0, buf2 + 2, hdr.frame_size - 2))
                break;
            buf2 += hdr.frame_size;
        }
...
AVInputFormat h264_demuxer = {
    ...
    raw_read_close,
    .flags= AVFMT_GENERIC_INDEX,
    .extensions = "h26l,h264,264,26l", //FIXME remove after h264_probe works properly
    .value = CODEC_ID_H264,
};
