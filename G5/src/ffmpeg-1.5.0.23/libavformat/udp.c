 ...
 *         'consume=1'   : if set, then read will attempt to get the
 *                         requested amount of data (minus up to the max packet size)
 *
...
static int udp_open(URLContext *h, const char *uri, int flags)
{
    ...
    if (p) {
        ...
        s->consume = find_info_tag(buf, sizeof(buf), "consume", p);
...
static int udp_read(URLContext *h, uint8_t *buf, int size)
{
    ...
    for(;;) {
        len = recv(s->udp_fd, buf, size - total, 0);
        if (len < 0) {
            ...
            total += len;
            buf += len;
            if (!s->consume || len == 0 || size - total < h->max_packet_size)
                break;
        }
    }
    return total;
