...
/* For a kernel that doesn't implement the reboot of the container, to re-initialize
 * a container, LXC fallback to a mode without CAP_SYS_BOOT and an inotify on utmp to
 * listen lxc-stop order instead of listening a signal. We still want these two modes
 * except the CAP_SYS_BOOT drop to be able to reboot the system from a container.
	#if HAVE_SYS_CAPABILITY_H
	if (handler->conf->need_utmp_watch) {
