...
static int setup_shared_fstab_entries(const char *fstab, struct lxc_list *entries);
static int clear_shared_fstab_entries(struct lxc_conf *c);
static int in_shared_fstab_list(const struct lxc_conf *lxc_conf, const char *fsname);
static int is_shared(const char *fsname);
...
static int exec_script(const char *name, const char *section,
                      const char *script, ...)
{
	...
	va_start(ap, script);
        f = lxc_popen_exec(script, name, section, ap);
	va_end(ap);
        if (!f) {
...
static int __attribute__((__unused__)) run_script(const char *name, const char *section,
		      const char *script, ...)
{
	int ret;
	char *buffer, *p;
...
static int setup_rootfs_pivot_root_cb(char *buffer, void *data)
{
    ...
    /* if keep shared mounts flag is set, ignore shared mountpoints in fstab */
    if (lxc_conf->keep_shared_mounts && in_shared_fstab_list(lxc_conf, mountpoint + strlen(pivotdir))) {
        INFO("%s matches a shared fstab entry => ignoring it", mountpoint);
        return 0;
    }
...
static int umount_oldrootfs(const struct lxc_conf *lxc_conf, const char *oldrootfs)
{
	char path[MAXPATHLEN];
	void *cbparm[3];
...
static int setup_rootfs_pivot_root(const struct lxc_conf *lxc_conf, const char *rootfs, const char *pivotdir)
{
	char path[MAXPATHLEN];
	...
	if (umount_oldrootfs(lxc_conf, pivotdir))
		return -1;
...
static int setup_pivot_root(const struct lxc_conf *lxc_conf, const struct lxc_rootfs *rootfs)
{
	if (!rootfs->path)
		return 0;
	...
	if (detect_ramfs_rootfs()) {
		if (prepare_ramfs_root(rootfs->mount))
			return -1;
	} else if (setup_rootfs_pivot_root(lxc_conf, rootfs->mount, rootfs->pivot)) {
...
static int is_shared(const char *fsname)
{
    ...
    /* walk /proc/mounts to check if fsname is a shared entry */
    ...
    while (getline(&line, &len, f) != -1) {
        ...
        if (strcmp(target, fsname) != 0)
            continue;
        null_endofword(opts);
        ret = (strstr(opts, "shared")) ? 1 : 0;
        break;
    }
    fclose(f);
    if (line)
...
static int setup_shared_fstab_entries(const char *fstab, struct lxc_list *entries)
{
    DEBUG("fetching shared fstab entries from %s", fstab);

    /* walk fstab to find shared entries */
    FILE *file;
    struct mntent mntent;
    char buf[4096];
    ...
    char *fsname;
    struct lxc_list *entry;

    while (getmntent_r(file, &mntent, buf, sizeof(buf))) {
        DEBUG("considering %s", mntent.mnt_fsname);

        /* check if fstab entry is shared */
        if (is_shared(mntent.mnt_fsname)) {
            DEBUG("%s is shared", mntent.mnt_fsname);

            /* add shared fstab entry */
            entry = malloc(sizeof(*entry));
            if (!entry)
                return -1;

            fsname = strdup(mntent.mnt_fsname);
            if (!fsname) {
                free(entry);
                return -1;
            }
            ...
            lxc_list_add_tail(entries, entry);
        }
    }
...
static int in_shared_fstab_list(const struct lxc_conf *lxc_conf, const char *fsname)
{
    /* walk shared fstab list to check if fsname is present */
    struct lxc_list *iterator;

    if (!fsname)
        return 0;

    lxc_list_for_each(iterator, &lxc_conf->shared_fstab_list) {
        if (strcmp((char*)iterator->elem, fsname) == 0) {
            DEBUG("%s is a shared fstab entry", fsname);
            return 1;
        }
    }
...
static int clear_shared_fstab_entries(struct lxc_conf *c)
{
    ...
    lxc_list_for_each_safe(it, &c->shared_fstab_list, next) {
        lxc_list_del(it);
        free(it->elem);
        free(it);
    }
...
struct lxc_conf *lxc_conf_init(void)
{
    ...
    lxc_list_init(&new->shared_fstab_list);
...
static int instantiate_veth(struct lxc_handler *handler, struct lxc_netdev *netdev)
{
	...
	if (netdev->upscript) {
		err = exec_script(handler->name, "net", netdev->upscript, "up",
...
static int shutdown_veth(struct lxc_handler *handler, struct lxc_netdev *netdev)
{
	...
	if (netdev->downscript) {
		err = exec_script(handler->name, "net", netdev->downscript,
...
static int instantiate_phys(struct lxc_handler *handler, struct lxc_netdev *netdev)
{
	...
	if (netdev->upscript) {
		int err;
		err = exec_script(handler->name, "net", netdev->upscript,
...
void remount_all_slave(const struct lxc_conf *lxc_conf)
{
	...
	while (getline(&line, &len, f) != -1) {
		...
		null_endofword(target);
	    /* if keep shared mounts flag is set, ignore shared mountpoints in fstab */
		if (lxc_conf->keep_shared_mounts && in_shared_fstab_list(lxc_conf, target)) {
            INFO("%s is a shared fstab entry => not remounting it as slave", target);
		    continue;
		}
...
int lxc_setup(struct lxc_handler *handler)
{
    ...
    if (setup_shared_fstab_entries(lxc_conf->fstab, &lxc_conf->shared_fstab_list)) {
        ERROR("failed to setup shared fstab entries");
        return -1;
    }
	...
	if (setup_pivot_root(lxc_conf, &lxc_conf->rootfs)) {
...
void lxc_conf_free(struct lxc_conf *conf)
{
    ...
    clear_shared_fstab_entries(conf);
	free(conf);
}
