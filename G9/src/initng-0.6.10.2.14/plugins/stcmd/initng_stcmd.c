...
static char *cmd_get_depends_on_graph(char *arg);
static char *cmd_get_depends_on_dot(char *arg);
...
s_command DEPENDS_ON_GRAPH = { 'G', "service_dep_on_graph", STRING_COMMAND, ADVANCHED_COMMAND,
	REQUIRES_OPT,
	{(void *) &cmd_get_depends_on_graph},
	"Print what services me depends on (graph)"
};
s_command DEPENDS_ON_DOT = { 'D', "service_dep_on_dot", STRING_COMMAND, ADVANCHED_COMMAND,
	REQUIRES_OPT,
	{(void *) &cmd_get_depends_on_dot},
	"Print what services me depends on (dot graph format)"
};
...
static int do_deps_graph(char **string, active_db_h *on, int level)
{
	active_db_h *current = NULL;
	...
	for(i = 0; i < level; i++)
		mprintf(string, "  ");
	mprintf(string, "%s [%s]%s\n", on->name, on->current_state->state_name, on->seen ? "..." : "");
	...
	while_active_db(current)
	{
		...
		if (initng_depend(on, current) == TRUE)
		{
			if (current->current_state && current->current_state->state_name)
			{
                do_deps_graph(string, current, level + 1);
			}
		}
	}
	return 0;
...
static char *cmd_get_depends_on_graph(char *arg)
{
	...
	do_deps_graph(&string, on, 0);

	return (string);
}

static int do_deps_dot(char **string, active_db_h *on, int level)
{
	active_db_h *current = NULL;
	...
	while_active_db(current)
	{
		...
		if (initng_depend(on, current) == TRUE)
		{
			if (current->current_state && current->current_state->state_name)
			{
				...
				do_deps_dot(string, current, level + 1);
			}
		}
	}
    return 0;
...
static char *cmd_get_depends_on_dot(char *arg)
{
	...
	mprintf(&string, "digraph dependencies {\n");
	do_deps_dot(&string, on, 0);
	mprintf(&string, "}\n");
...
int module_init(int api_version)
{
	...
	initng_command_register(&DEPENDS_ON_GRAPH);
	initng_command_register(&DEPENDS_ON_DOT);
...
void module_unload(void)
{
	...
	initng_command_unregister(&DEPENDS_ON_GRAPH);
	initng_command_unregister(&DEPENDS_ON_DOT);
