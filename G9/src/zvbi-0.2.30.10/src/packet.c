...
store_lop(vbi_decoder *vbi, const cache_page *vtp)
{
	 ...
	 */
	/* DEACTIVATED, it wrongly detects channel switch on some TDT Premium streams, and
	 * we always know when we switch channel anyway */
	if (FALSE && event.ev.ttx_page.roll_header) {
		int r;
	...
	/*
	  more flag parsing, for unclear reason to me, cache_network_get_ttx_page_stat does not seem to do the job 
        */             
	if( vtp->flags & (C5_NEWSFLASH | C7_SUPPRESS_HEADER) ){
		ps->page_type = VBI_SUBTITLE_PAGE;
	}
...
//			printf("Hamming error in packet 0 page number\n");
		...
		if (cvtp->function == PAGE_FUNCTION_UNKNOWN) {
			...
			default:
				/* Bugzilla #9599: support hexadecimal page in teletext
				   There is no reason to exclude the pages with an hexadecimal number.
				*/
