...
/**
 * @brief Helper class for the encoding/decoding of a D-Bus object path.
 *
 * This is simply a custom name for a `std::string` required by message encoding/decoding implementation.
 */
...
/**
 * @brief Helper class for the encoding/decoding of a D-Bus method signature.
 *
 * This is simply a custom name for a `std::string` required by message encoding/decoding implementation.
 */
...
/**
 * @brief Wrapper class for the encoding/decoding of a file descriptor.
 *
 * Wraps a standard Linux file descriptor as a C++ type. Additionally, it supports the
 * auto closing of the inner file descriptor on destruction.
 */
class DXXAPI UnixFD
{
public:
    UnixFD() : _fd(-1), _close(false) {}
    UnixFD(int fd, bool close = false) : _fd(fd), _close(close) {}
    UnixFD &operator = (int fd)
    {
        _fd = fd;
        return *this;
    }
    ...
    void set(int fd, bool close = false)
    {
        _fd = fd;
        _close = close;
    }
...
/**
 * @brief Helper class for the encoding/decoding of a D-Bus variant type
 */
...
template <> struct type<UnixFD>         { static std::string sig(){ return "h"; } };
...
template <
	...
	typename T9
>
struct type< Struct<T1,T2,T3,T4,T5,T6,T7,T8,T9> >
{
	static std::string sig()
	{
		return "("
...
inline DBus::MessageIter &operator << (DBus::MessageIter &iter, const DBus::UnixFD & fd)
{
    iter.append_fd(fd.get());
    return iter;
}

template<typename Iterable>
inline DBus::MessageIter &fromIterable(DBus::MessageIter &iter, const Iterable &val)
{
	const std::string sig = DBus::type<typename Iterable::value_type>::sig();
	DBus::MessageIter ait = iter.new_array(sig.c_str());

	typename Iterable::const_iterator vit;
...
{
	return fromIterable<std::vector<E> >(iter, val);
}
...
template <typename E>
inline DBus::MessageIter& operator << (DBus::MessageIter &iter, const std::list<E> &val)
{
	return fromIterable<std::list<E> >(iter, val);
}

template <typename E>
inline DBus::MessageIter& operator << (DBus::MessageIter &iter, const std::set<E> &val)
{
	return fromIterable<std::set<E> >(iter, val);
}
...
template <
	...
	typename T9
>
inline DBus::MessageIter &operator << (DBus::MessageIter &iter, const DBus::Struct<T1,T2,T3,T4,T5,T6,T7,T8,T9>& val)
{
...
*/
	...
	sit << val._1 << val._2 << val._3 << val._4 << val._5 << val._6 << val._7 << val._8 << val._9;
...
template<typename E>
inline DBus::MessageIter& operator >> (DBus::MessageIter &iter, std::list<E> &val)
{
	if(!iter.is_array())
		throw DBus::ErrorInvalidArgs("array expected");
...
template<typename E>
inline DBus::MessageIter& operator >> (DBus::MessageIter &iter, std::set<E> &val)
{
	if(!iter.is_array())
		throw DBus::ErrorInvalidArgs("array expected");
...
template <
	...
	typename T9
>
inline DBus::MessageIter &operator >> (DBus::MessageIter &iter, DBus::Struct<T1,T2,T3,T4,T5,T6,T7,T8,T9>& val)
{
