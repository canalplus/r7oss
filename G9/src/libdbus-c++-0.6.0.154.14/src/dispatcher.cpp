...
void Dispatcher::queue_connection(DBusConnection *cp)
{
	_mutex_p.lock();
	dbus_connection_ref(cp);
...
bool Dispatcher::has_something_to_dispatch()
{
	...
	for(Connection::PrivatePList::iterator it = _pending_queue.begin();
		...
		++it)
	{
		has_something = dbus_connection_get_dispatch_status(*it) == DBUS_DISPATCH_DATA_REMAINS;
	}
...
{
	_mutex_p.lock();
	Connection::PrivatePList queue;
	queue.swap(_pending_queue);
	debug_log("%i connections in queue", queue.size());
	_mutex_p.unlock();
	...
	while (!queue.empty())
	{
		Connection::PrivatePList::iterator it;
		for (it = queue.begin(); it != queue.end(); )
		{
			if (dbus_connection_dispatch(*it) != DBUS_DISPATCH_DATA_REMAINS)
			{
				dbus_connection_unref(*it);
				queue.erase(it++);
			}
