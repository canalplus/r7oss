...
void parse_dbus_introspect_xml(Xml::Node &, const char *, bool, ostringstream &, vector<string> &);

void generate_adaptor(Xml::Document & doc, const char * filename, const char * suffix, bool pass_message)
{
    ...
    ostringstream head;
    {
        ...
        cond_comp = "__dbusxx__" + filestring + "__ADAPTOR_MARSHAL_H";
...
//                cerr << tab << tab << (*node_it)->name << ": \"" << (*node_it)->get("name") << "\"" << endl;
                parse_dbus_introspect_xml(**node_it, suffix, pass_message, body, include_vector);
                body << endl;
            }
        }
    }
    else {
        parse_dbus_introspect_xml(*(doc.root), suffix, pass_message, body, include_vector);
    }

    {
        ...
        head << endl;
        head << "/* undefine macros that could clash with generated code */" << endl;
        head << "#undef register_method" << endl;
        head << "#undef bind_property" << endl;
        head << "#undef connect_signal" << endl;
        head << "#undef connect_reply" << endl;
        head << endl;

        body << endl;
        body << "/* reset previously-undefined macros */" << endl;
        body << "#define register_method(interface, method, callback) \\" << endl;
        body << "    dbusxx_register_method(interface, method, callback)" << endl;
        body << "#define bind_property(variable, type, can_read, can_write) \\" << endl;
        body << "    dbusxx_bind_property(variable, type, can_read, can_write)" << endl;
        body << "#define connect_signal(interface, signal, callback) \\" << endl;
        body << "    dbusxx_connect_signal(interface, signal, callback)" << endl;
        body << "#define connect_reply(interface, async_call, callback) \\" << endl;
        body << "    dbusxx_connect_reply(interface, async_call, callback)" << endl;
        body << endl;
...
/*! Generate adaptor code for a XML introspection
  */
void parse_dbus_introspect_xml(Xml::Node & root, const char *suffix, bool pass_message,
        ostringstream & body, vector<string> & include_vector)
{
	...
	for (Xml::Nodes::iterator i = interfaces.begin(); i != interfaces.end(); ++i)
	{
		...
		for (Xml::Nodes::iterator pi = properties.begin(); pi != properties.end(); ++pi)
		{
			...
			body << tab << tab << "dbusxx_bind_property("
			<< property.get("name") << ", "
			<< "\"" << property.get("type") << "\", "
		...
		for (Xml::Nodes::iterator mi = methods.begin(); mi != methods.end(); ++mi)
		{
			...
			body << tab << tab << "dbusxx_register_method("
		...
		for (Xml::Nodes::iterator mi = methods.begin(); mi != methods.end(); ++mi)
		{
			...
			// pass the original message if requested
			bool prepend_param_sep;

			if (pass_message)
			{
				body << "const ::DBus::CallMessage & _msg_";
