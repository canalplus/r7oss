...
typedef Callback_Base<void, PendingCall&> PendingCallCallback;
 ...
 * @class Connection
 *
 * @brief Provides an abstraction of a D-Bus connection.
 */
...
public:
     ...
     */
    /**
     * @brief Creates a new connection on one of the known buses.
     *
     * @param[in] disp Pointer to a dispatcher where to subscribe the new connection
     *
     * @return The newly created connection
     */
	static Connection SystemBus( Dispatcher* disp = NULL );

	static Connection SessionBus( Dispatcher* disp = NULL );

	static Connection ActivationBus( Dispatcher* disp = NULL );
	/**
	 * @}
	 */
	...
	typedef std::list<DBusConnection*> PrivatePList;

	/**
	 * @name Initialization/Destruction
	 *
	 * A Connection object works in conjunction with a Dispatcher object, therefore
	 * the construction or initialization requires a Dispatcher object pointer
	 * @{
	 */
	Connection( Private*, Dispatcher* disp = NULL );
	 ...
	 *
	 * \param[in] s The MessageSlot to add.
	 */
	 ...
	 * the unique name is set, you can get it with unique_name().
	 *
	...
	/*!
	 * \brief Sets whether _exit() should be called when the connection receives a
	 *        disconnect signal.
	 *
	...
	/*!
	 *
	 * \brief backward compatibility method
	 * \deprecated 0.6.0.154.9
	 */
	Message send_blocking_wyplay( Message& msg, int timeout = -1);
	...
	PendingCall send_async_wyplay( Message &, PendingCallCallback *, AsyncContext *, int timeout = -1);
