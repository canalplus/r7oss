...
void InterfaceAdaptor::get_all_properties(std::map<std::string, Variant> & ret) const
{
    for (PropertyTable::const_iterator it = _properties.begin() ; it != _properties.end() ; ++it) {
        if (!it->second.read) {
            throw ErrorAccessDenied("one of the properties is not readable");
        }
...
bool InterfaceProxy::dispatch_reply(const ReturnMessage & msg, const std::string & method, void * udata)
{
	debug_log("dispatching reply interface(%s) method(%s)", name().c_str(), method.c_str());

	ReplyCallbacksTable::iterator ri = _replies.find(method);
    if (_replies.end() == ri) {
        debug_log("cannot dispatch reply because no reply callback found for interface(%s) method(%s)", name().c_str(), method.c_str());
        return false;
    }

	ri->second.call(msg, udata);
...
bool InterfaceProxy::invoke_method_async(const CallMessage & call, void * udata, bool on_thread_pool)
{
	CallMessage & call2 = const_cast<CallMessage &>(call);
	...
	return _invoke_method_async(call2, udata, on_thread_pool);
}

void InterfaceProxy::enable_signal(const char * member)
{
    _enable_signal(*this, member);
}

void InterfaceProxy::disable_signal(const char * member)
{
    _disable_signal(*this, member);
}

void InterfaceProxy::pre_enabled_signals_add(const char * member)
{
    _pre_enabled_signals.push_back(member);
}

const EnabledSignalsList & InterfaceProxy::get_pre_enabled_signals()
{
    return _pre_enabled_signals;
}

bool InterfaceProxy::is_legacy_mode() const
{
    return _is_legacy_mode;
}

void InterfaceProxy::disable_legacy_mode()
{
    _is_legacy_mode = false;
}
