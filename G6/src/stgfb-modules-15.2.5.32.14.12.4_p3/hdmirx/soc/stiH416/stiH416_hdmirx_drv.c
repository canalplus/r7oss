/***********************************************************************
 *
 * File: hdmirx/soc/stiH416/stiH416_hdmirx_drv.c
...
uint32_t hdmirx_initialisation(hdmirx_handle_t Handle)
{
  ...
  if (hdmirx_configure_vtac())
    {
      HDMI_PRINT(" hdmirx_configure_vtac: failed, error%d\n", ErrorCode);
      return -EINVAL;
    }
  return 0;
}
...
#ifndef CONFIG_OF
  Port_handle->Pad_Config.gpios_num = 4;

  Port_handle->Pad_Config.gpios = (struct stm_pad_gpio *) stm_hdmirx_malloc(sizeof(struct stm_pad_gpio)*Port_handle->Pad_Config.gpios_num);
  if (Port_handle->Pad_Config.gpios == NULL) return -EINVAL;

  Port_handle->Pad_Config.gpios[0]= (struct stm_pad_gpio)STM_PAD_PIO_OUT((stm_gpio_port(Port_handle->Hpd_Pio)),(stm_gpio_pin(Port_handle->Hpd_Pio)),1);
  Port_handle->Pad_Config.gpios[1]= (struct stm_pad_gpio)STM_PAD_PIO_IN((stm_gpio_port(Port_handle->Pd_Pio)), (stm_gpio_pin(Port_handle->Pd_Pio )),1);
  Port_handle->Pad_Config.gpios[2]= (struct stm_pad_gpio)STM_PAD_PIO_OUT((stm_gpio_port(Port_handle->SCL_Pio)),(stm_gpio_pin(Port_handle->SCL_Pio)),1);
  Port_handle->Pad_Config.gpios[3]= (struct stm_pad_gpio)STM_PAD_PIO_OUT((stm_gpio_port(Port_handle->SDA_Pio)),(stm_gpio_pin(Port_handle->SDA_Pio)),1);
