...
#define POLL_TIMEOUT_DELAY 60000
...
const char *attributes[ATTR_MAX] = {
	...
	[ATTR_HDMI_RXSENSE_STATUS] = DISPLAY HDMI "rxsense_status",
};
...
bool wait_for_hotplug(bool connected, bool quiet)
{
	...
	if (sz < 0 || attr[1] != '\n')
		...
		res = poll(&pollfd, 1, POLL_TIMEOUT_DELAY);

	close(pollfd.fd);

	if (!quiet)
		printf("%s\n", (res < 0 ? "FAILED" : (res == 0 ? "TIMEOUT" :"OK")));
	if (res < 0)
...
bool wait_for_hdmi_status(bool running, bool quiet)
{
	struct pollfd pollfd;
	char attrname[256];
	long page_size = sysconf(_SC_PAGESIZE);
	...
	snprintf(attrname, 256, attributes[ATTR_HDMI_TMDS_STATUS],  device_number, device_number, subdevice_number);

	pollfd.fd = open(attrname, O_RDONLY);
	if (pollfd.fd < 0) {
		fprintf(stderr, "ERROR: Unable to wait for HDMI Status\n");
		return false;
	}
	...
	if (!quiet) {
		printf("Waiting for HDMI to be %s ... ", running ? "Running" : "Stopped");
		fflush(stdout);
	}

	p = malloc(page_size + 1);
	if (!p) {
		return -1;
	}

	ssize_t sz = read(pollfd.fd, p, page_size);

	int res;
	if (sz < 0)
		res = -1;
	else if ((0 == strcmp("Running\n", p)) == running)
		res = 1;
	else
		res = poll(&pollfd, 1, POLL_TIMEOUT_DELAY);
	...
	if (!quiet)
		printf("%s\n", (res < 0 ? "FAILED" : (res == 0 ? "TIMEOUT" :"OK")));
	if (res < 0)
		fprintf(stderr, "ERROR: Cannot wait for HDMI status\n");

	return 1 == res;
}

bool wait_for_rxsense_status(bool active, bool quiet)
{
	struct pollfd pollfd;
	char attrname[256];
	long page_size = sysconf(_SC_PAGESIZE);
	...
	snprintf(attrname, 256, attributes[ATTR_HDMI_RXSENSE_STATUS],  device_number, device_number, subdevice_number);

	pollfd.fd = open(attrname, O_RDONLY);
	if (pollfd.fd < 0) {
		fprintf(stderr, "ERROR: Unable to wait for HDMI RX-Sense\n");
		return false;
	}
	...
	if (!quiet) {
		printf("Waiting for Connected device to be %s ... ", active ? "Active" : "Inactive");
		fflush(stdout);
	}
	...
	if (!quiet)
		printf("%s\n", (res < 0 ? "FAILED" : (res == 0 ? "TIMEOUT" :"OK")));
	if (res < 0)
		fprintf(stderr, "ERROR: Cannot wait for Connected device status\n");
...
bool show_rxsense_status(void)
{
	char *s = get_attribute(ATTR_HDMI_RXSENSE_STATUS);
	if(!s) {
		return false;
	}
	printf("Rx-Sense Status: %s",s);
	return true;
}
...
static void issue_help_message(void)
{
    ...
    printf("  -r, --rxsense-status     Show the TV status.\n");
    printf("  -R, --no-rxsense-status\n"
           "                           Don't show the TV status.\n");
...
int main(int argc, char *argv[])
{
	...
	int  wait_for_plug_in = -1;
	int  wait_for_hdmi_enabled = -1;
	int  wait_for_connected_device = -1;
	...
	while ((option = getopt_long (argc, argv, "cCd:lLsStThmMnNqvw:", long_options, NULL)) != -1) {
		...
		case 'L':
			...
			break;
		case 'r':
			show[ATTR_HDMI_RXSENSE_STATUS] = true;
			break;
		case 'R':
			show[ATTR_HDMI_RXSENSE_STATUS] = false;
			break;
		case 's':
		...
		case 'w':
			wait_on_entry = true;
			show[ATTR_HDMI_HOTPLUG] = false;
			if (0 == strcmp("inactive", optarg))
				wait_for_connected_device = 0;
			else if(0 == strcmp("active", optarg))
				wait_for_connected_device = 1;
			else if (0 == strcmp("unplugged", optarg))
				wait_for_plug_in = 0;
			else if(0 == strcmp("plugged", optarg))
				wait_for_plug_in = 1;
			else if(0 == strcmp("false", optarg))
				wait_for_hdmi_enabled = 0;
			else if (0 == strcmp("true", optarg))
				wait_for_hdmi_enabled = 1;
			else
				wait_for_plug_in = (0 == strcmp("0", optarg));
			break;
		default:
	...
	if (wait_on_entry)
	{
		if(wait_for_hdmi_enabled != -1)
			keep_going = wait_for_hdmi_status(wait_for_hdmi_enabled, quiet);
		else if (wait_for_connected_device != -1)
			keep_going = wait_for_rxsense_status(wait_for_connected_device, quiet);
		else if(wait_for_plug_in != -1)
	...
	if (keep_going && show[ATTR_HDMI_RXSENSE_STATUS])
		keep_going = show_rxsense_status();
