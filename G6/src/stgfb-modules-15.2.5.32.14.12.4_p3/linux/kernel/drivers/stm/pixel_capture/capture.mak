...
KBUILD_CPPFLAGS += -I$(STG_TOPDIR)/linux/kernel/drivers/stm/pixel_capture

# Common stm_pixelcaptureLinux specific module files
...
GENERICSRCS := $(addprefix $(SRC_TOPDIR)/pixel_capture/generic/,       \
			CaptureDevice.cpp                                          \
			Capture.cpp                                                \
			Queue.cpp)

STM_CAPTURE_SRC_FILES := $(DEVICESOURCEFILES) $(GENERICSRCS)

# Pull in all the new IP objects. Eventually a lot of the above will disappear
# into here as well.
include $(STG_TOPDIR)/pixel_capture/ip/ip.mak
...
CAPTURE_VERSION := $(shell cd $(CAPTURE_ORIGINAL_SOURCE_PATH) && git describe --long --dirty --tags --always 2>/dev/null)
# If our git-describe is unsuccessful, we should fall back to an RPMBuild generated file
ifeq ($(CAPTURE_VERSION),)
CAPTURE_VERSION := $(shell cat $(STG_TOPDIR)/VERSION)
endif
