/***********************************************************************
 *
 * File: linux/kernel/drivers/stm/hdmi/hdmidev.c
...
struct of_device_id stmhdmi_match[] = {
	{ .compatible = "st,hdmi-repeater" },
	{},
};

extern int stmhdmi_repeater_manager(void *data);
...
int stmhdmi_destroy(struct stm_hdmi *hdmi)
{
  ...
  if (hdmi->route)
  {
    stm_hdmirx_route_close(hdmi->route);
    hdmi->route =NULL;
  }
  if (hdmi->port)
  {
    stm_hdmirx_port_close(hdmi->port);
    hdmi->port =NULL;
  }
  if (hdmi->hdmirx_device)
  {
    stm_hdmirx_device_close(hdmi->hdmirx_device);
    hdmi->hdmirx_device =NULL;
  }
  if(hdmi->hdmi_output)
  {
...
void *stmhdmi_get_device_tree_data(struct platform_device *pdev)
{
  ...
  hdmi_root_node_p = of_find_compatible_node(NULL, NULL, "st,hdmi-repeater");
  if ( !hdmi_root_node_p)
   return NULL;
  ...
  hdmi_platform_data->device_id = of_alias_get_id(pdev->dev.of_node, "hdmi-repeater");
...
#endif
{
  ...
  if(!platform_data)
  {
    printk(KERN_ALERT "platform data pointer is NULL\n");
    return -EINVAL;
  }
 ...
 if(stmcore_get_display_pipeline(platform_data->pipeline_id, &display_pipeline)<0)
  ...
  /*
   * Open HDMI Rx connection
   */

  if (stm_hdmirx_device_open (&hdmi->hdmirx_device, 0) <0)
  {
    printk(KERN_ERR "HDMI Rx Device open failed\n");
    goto exit_nodev;
  }

  if(stm_hdmirx_port_open(hdmi->hdmirx_device, HDMIRX_INPUT_PORT, &hdmi->port)<0)
  {
    printk(KERN_ERR "HdmiRx Port Open failed \n");
    goto exit_nodev;
  }

  if (stm_hdmirx_route_open(hdmi->hdmirx_device, 0, &hdmi->route) <0)
  {
    printk(KERN_ERR "HdmiRx Route Open failed \n");
    goto exit_nodev;
  }
  ...
  hdmi->thread = kthread_create(stmhdmi_repeater_manager, hdmi, thread_name);
  if (hdmi->thread == NULL)
  {
...
{
  if(alloc_chrdev_region(&firstdevice,0,1,"hdmi-repeater")<0)
  {
