/***********************************************************************
 *
 * File: linux/kernel/drivers/stm/hdmi/hdmirepeater/hdmirepkthread.c
...
#define HDMI_REPEATER_EVT_NUMBER  (4)    /* HDMI repeater events Number:route HDCP Rx, HPD, Rx sense and HDCP Tx events */
...
static int stmhdmi_authenticate_repeater (struct stm_hdmi *hdmi, bool repeater_ready)
{
  uint16_t status;
  int retVal  = 0;
  uint8_t ksvs[127*5];
  stm_hdmirx_hdcp_downstream_status_t downstream_status;

  memset(&downstream_status, 0, sizeof(stm_hdmirx_hdcp_downstream_status_t));


  // to be done in context of An, Aksv writing
  if ((retVal = stm_hdmirx_route_set_ctrl(hdmi->route, STM_HDMIRX_ROUTE_CTRL_REPEATER_STATUS_READY, 0))!=0)
  {
    printk(KERN_ERR "Error can't set route control: STM_HDMIRX_ROUTE_CTRL_REPEATER_STATUS_READY '0'\n");
    return retVal;
  }
  //Enable repeater mode
  if ((retVal = stm_hdmirx_route_set_ctrl(hdmi->route, STM_HDMIRX_ROUTE_CTRL_REPEATER_MODE_ENABLE, 1))!=0)
  {
    printk(KERN_ERR "Error can't set route control: STM_HDMIRX_ROUTE_CTRL_REPEATER_MODE_ENABLE '1'\n");
    return retVal;
  }
 // to be done in context of C5 state.
  if ((retVal =stm_display_link_hdcp_get_sink_status_info(hdmi->link, &status, 0)) !=0)
  {
    printk(KERN_ERR "Error can't get HDCP sink Bstatus\n");
    return retVal;
  }

  downstream_status.depth = (status & 0x700)>>8;
  downstream_status.device_count = status & 0x7F;
  downstream_status.max_devs_exceeded = ((status& 0x80) == 0x80);
  downstream_status.max_cascade_exceeded = ((status & 0x0800) == 0x0800);

  printk(KERN_ALERT "$$$$$ Read BStatus from downstream : depth = %d ; device = %d ; MaxDevExBit = %d ; MaxCasExBit = %d\n",
      downstream_status.depth,
      downstream_status.device_count,
      downstream_status.max_devs_exceeded,
      downstream_status.max_cascade_exceeded);


  if (downstream_status.depth >= 7) {
    downstream_status.max_cascade_exceeded = true;
  }
  else {
    downstream_status.depth++;
  }

  if (downstream_status.device_count  >= 127) {
    downstream_status.max_devs_exceeded = true;
  }
  else {
    downstream_status.device_count++;
  }

  printk(KERN_ALERT "$$$$$ Provide BStatus to upstream : depth = %d ; device = %d ; MaxDevExBit = %d ; MaxCasExBit = %d\n",
        downstream_status.depth,
        downstream_status.device_count,
        downstream_status.max_devs_exceeded,
        downstream_status.max_cascade_exceeded);

  if ((retVal = stm_hdmirx_route_set_hdcp_downstream_status(hdmi->route, &downstream_status))!=0)
  {
    printk(KERN_ERR "Error can't set hdcp downstream status\n");
    return retVal;
  }

  if (repeater_ready){
    if (stm_display_link_hdcp_get_downstream_ksv_list(hdmi->link, &ksvs[0], downstream_status.max_devs_exceeded?127:downstream_status.device_count) == 0) {
      if ((retVal = stm_hdmirx_route_set_hdcp_downstream_ksv_list(hdmi->route, &ksvs[0], (downstream_status.max_devs_exceeded?127:downstream_status.device_count)*5))!=0)
      {
        printk(KERN_ERR "Error can't set route control :STM_DISPLAY_LINK_CTRL_HDCP_ENABLE '0'\n");
        return retVal;
      }
    }
  printk(KERN_ALERT "$$$$$ Set Repeater Ready\n");
  if ((retVal =stm_hdmirx_route_set_ctrl(hdmi->route, STM_HDMIRX_ROUTE_CTRL_REPEATER_STATUS_READY, 1)) !=0)
  {
    printk(KERN_ERR "Error can't set route control :STM_DISPLAY_LINK_CTRL_HDCP_ENABLE '0'\n");
    return retVal;
  }
...
void stmhdmi_generate_hpdrx_pulse(struct stm_hdmi * hdmi)
{
  if (hdmi->hdcp_enable)
  {
    stm_hdmirx_port_set_hpd_signal(hdmi->port, STM_HDMIRX_PORT_HPD_STATE_LOW);
    msleep(100);
    stm_hdmirx_port_set_hpd_signal(hdmi->port, STM_HDMIRX_PORT_HPD_STATE_HIGH);
  }
}

int stmhdmi_repeater_manager(void *data)
{
  stm_event_info_t                       hdmi_repeater_event[HDMI_REPEATER_EVT_NUMBER];
 ...
 /*
  * Subscribe to port, route and display link events
  */
  subscription_p = &subscription_entry[number_of_entries];
  subscription_p->cookie = NULL;
  subscription_p->object = (stm_object_h)hdmi->route;
  subscription_p->event_mask = STM_HDMIRX_ROUTE_HDCP_AUTHENTICATION_DETECTED_EVT;
  number_of_entries++;
 ...
 if (stm_event_subscription_create(subscription_entry,
                        number_of_entries,
                        &(hdmi->subscription)) != 0)
 {
  ...
  if ((retVal = stm_event_set_wait_queue(hdmi->subscription, &hdmi->status_wait_queue, true))!=0)
  {
  ...
  while(1)
  {
    wait_event_freezable(hdmi->status_wait_queue,
                                ((stm_event_wait(hdmi->subscription, 0 , 0, NULL, NULL)!= 0) ||
    ...
    if ((retVal = stm_event_wait(hdmi->subscription, 0 , HDMI_REPEATER_EVT_NUMBER, &event_count, hdmi_repeater_event))==0)
    {
      printk(KERN_INFO " An event is retrieved :HDCP Rx/HPD/HDCP Tx/RXsense event\n");
    }
    ...
    */
    ...
    {
      if (hdmi_repeater_event[n].event.object== hdmi->route)
      {
        if (hdmi_repeater_event[n].event.event_id == STM_HDMIRX_ROUTE_HDCP_AUTHENTICATION_DETECTED_EVT)
        {
          if ((retVal = stm_hdmirx_route_get_hdcp_status(hdmi->route, &hdmi->hdmirx_hdcp_status)) != 0)
          {
          ...
          DPRINTK("An event is retrieved :HDMI Rx route HDCP event : %d\n", hdmi->hdmirx_hdcp_status);
          if (((hdmi->hdcp_status == STM_DISPLAY_LINK_HDCP_STATUS_SUCCESS_ENCRYPTING) ||
              ...
              (hdmi->hdmirx_hdcp_status == STM_HDMIRX_ROUTE_HDCP_STATUS_AUTHENTICATION_DETECTED))
          {
            stmhdmi_authenticate_repeater(hdmi, true);
          }
          else if (((hdmi->hdcp_status == STM_DISPLAY_LINK_HDCP_STATUS_FAIL_MAX_DEVICE_COUNT_EXCEEDED) ||
                    ...
                    (hdmi->hdcp_status == STM_DISPLAY_LINK_HDCP_STATUS_FAIL_KSV_REVOKED))&&
                    (hdmi->hdmirx_hdcp_status == STM_HDMIRX_ROUTE_HDCP_STATUS_AUTHENTICATION_DETECTED))
          {
            stmhdmi_authenticate_repeater(hdmi, false);
          }
          else if ((hdmi->hdmirx_hdcp_status == STM_HDMIRX_ROUTE_HDCP_STATUS_NOT_AUTHENTICATED)||
                   (hdmi->hdmirx_hdcp_status == STM_HDMIRX_ROUTE_HDCP_STATUS_NOISE_DETECTED))
          {
            stm_hdmirx_route_set_ctrl(hdmi->route, STM_HDMIRX_ROUTE_CTRL_REPEATER_MODE_ENABLE, 0);
          }
        }
      }
      else if (hdmi_repeater_event[n].event.object== hdmi->link)
      {
        switch(hdmi_repeater_event[n].event.event_id)
        {
          case STM_DISPLAY_LINK_HPD_STATE_CHANGE_EVT:
          {
            ...
            {
              stm_hdmirx_route_set_ctrl(hdmi->route, STM_HDMIRX_ROUTE_CTRL_REPEATER_MODE_ENABLE, 1);
              ...
              stmhdmi_generate_hpdrx_pulse(hdmi);
            }
            else
            {
    ...
    if (((event_count ==0)||(hdmi_repeater_event[0].event.event_id==STM_DISPLAY_LINK_HDCP_STATUS_CHANGE_EVT)) && likely(hdmi->status_changed) && !is_suspended)
    {
  ...
  if ((retVal = stm_event_subscription_delete(hdmi->subscription))!=0)
  {
