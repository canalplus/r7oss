/*******************************************************************************
 *
 * File: display/ip/videoPlug/VideoPlug.cpp
 ...
 */
static const uint32_t mpr601_table[4] = {
  0x0a800000,
...
static const uint32_t mpr709_table[4] = {
  0x0a800000,
...
CVideoPlug::CVideoPlug(CDisplayDevice *pDev, uint32_t plugOffset, bool bEnablePSI, bool bUseMPR)
{
...
void CVideoPlug::updateColorKeyState (stm_color_key_config_t       * const dst,
                               ...
                               uint32_t                      transparency)
{
  ...
  TRC( TRC_ID_UNCLASSIFIED, "VPO = %#08x, VPS = %#08x", plugSetup.VIDn_VPO, plugSetup.VIDn_VPS );

  plugSetup.VIDn_CTL = 0;

  if(pFrame->src.flags & STM_BUFFER_SRC_COLORSPACE_709)
    {
      TRC( TRC_ID_UNCLASSIFIED, "colorspace set to ITU-R.709" );
      if(m_bUseMPR)
    ...
    {
      TRC( TRC_ID_UNCLASSIFIED, "colorspace set to ITU-R.601" );
      if(m_bUseMPR)
  ...
  if (!(m_ColorKeyConfig.flags & SCKCF_MINVAL)
      || !CDisplayPlane::GetRGBYCbCrKey (ucRCR, ucGY, ucBCB,
                                         ...
                                         m_ColorKeyConfig.format == SCKCVF_RGB))
    {
      TRC( TRC_ID_MAIN_INFO, "Min key value not obtained" );
      return false;
    }
  ...
  if (!(m_ColorKeyConfig.flags & SCKCF_MAXVAL)
      || !CDisplayPlane::GetRGBYCbCrKey (ucRCR, ucGY, ucBCB,
                                         ...
                                         m_ColorKeyConfig.format == SCKCVF_RGB))
    {
      TRC( TRC_ID_MAIN_INFO, "Max key value not obtained" );
      return false;
    }
  ...
  if(m_bUseMPR)
    {
      if(setup.mpr == NULL)
      {
        TRC( TRC_ID_ERROR, "setup.mpr is NULL!!!" );
      }
      else
      {
