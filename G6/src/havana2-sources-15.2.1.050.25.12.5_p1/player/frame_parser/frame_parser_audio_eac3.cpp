                                                              ...
                                                              int GivenFrameSize)
{
    ...
    bool InvalidBSID = false;
    unsigned int  DeltaLength = 0;

    memset(ParsedFrameHeader, 0, sizeof(EAc3AudioParsedFrameHeader_t));

    int SelectedSubStreamId = Player->PolicyValue(Playback, Stream, PolicyAudioSubstreamId);

    do
    {
        // At this point we got the independant substream or stream length, now search for
        ...
        if (Status !=  FrameParserNoError)
        {
            ...
            if (InvalidBSID == true)
            {
                // we had an invalid bsid at the last block
                //try to check at the delta length
                StreamIndex += DeltaLength;
                FrameSize += DeltaLength;
                DeltaLength = 0;
                InvalidBSID = false;
            }
            else
        ...
        {
            DeltaLength = NextParsedFrameHeader.DeltaLength;
            InvalidBSID = ((NextParsedFrameHeader.Type == TypeInValidBSID)) ? true : false;

            if (((NextParsedFrameHeader.Type == TypeEac3Ind) &&
                 ((NextParsedFrameHeader.SubStreamId == 0) || (NextParsedFrameHeader.SubStreamId == SelectedSubStreamId)))
                || (NextParsedFrameHeader.Type == TypeAc3)
                || (NextParsedFrameHeader.Type == TypeInValidBSID))
            {
                // we are an independent  substream check if selected sub stream occur before primary [invalid case]
                if ((NumberOfIndependantSubStreams == 0) && (SelectedSubStreamId != 0) && (NextParsedFrameHeader.SubStreamId != 0))
                {
                    SE_ERROR("Independent substream[%d] found before independant primary one, should not occur\n", SelectedSubStreamId);
                    /* we met a independent nth substream first, this is a frame parsing error ....*/
                    return FrameParserError;
                }

                if (NumberOfIndependantSubStreams == SelectedSubStreamId)
                {
                    /* get the first user selected independant stream properties */
                ...
                if ((NextParsedFrameHeader.SubStreamId == SelectedSubStreamId)
                    || (NextParsedFrameHeader.Type == TypeAc3)
                    || (NextParsedFrameHeader.Type == TypeInValidBSID))
                {
                    NumberOfSamples += NextParsedFrameHeader.NumberOfSamples;    // NumberOfSamples to be increase only for the user selected substream
                }
            }
            else if (NextParsedFrameHeader.Type == TypeEac3Dep)
            {
                ...
                {
                    SE_ERROR("Dependant subframe found before independant one, should not occur\n");
            ...
            else if ((NextParsedFrameHeader.Type == TypeEac3Ind) &&
                     ((NextParsedFrameHeader.SubStreamId != 0) || (NextParsedFrameHeader.SubStreamId != SelectedSubStreamId)))
            {
                // we are an independent substream other than I0 and user selected substream
                if (NumberOfIndependantSubStreams == 0)
                {
                    SE_ERROR("Independent secondary substream found before independant primary one, should not occur\n");
                    /* we met a independent secondary substream first, this is a frame parsing error ....*/
                    return FrameParserError;
                }
    ...
    {
        SE_ERROR("Given frame size mismatch: %d (expected:%d)\n", FrameSize, GivenFrameSize);
        return FrameParserError;
    }

    if (NumberOfSamples != EAC3_NBSAMPLES_NEEDED)
    {
        SE_DEBUG(group_frameparser_audio, "Number of samples mismatch: %d (expected:%d)\n", NumberOfSamples, EAC3_BYTES_NEEDED);
        /// TODO : Condition the playback of frames that can't be transcoded to DD+ ISO profile
        ///        MS1x profile shall drop these frames.
        // return FrameParserError;
    }
    ...
    SE_DEBUG(group_frameparser_audio, "SamplingFrequency %d, FrameSize %d, Indp substreams: %d, Dep substreams: %d\n",
    ...
    if (FirstTime)
    {
        ...
        {
            SE_INFO(group_frameparser_audio, "AC3 stream properties: SamplingFrequency %d, FrameSize %d\n",
                    ParsedFrameHeader->SamplingFrequency,
                    ParsedFrameHeader->Length);
        }
        else
        {
            SE_INFO(group_frameparser_audio, "DD+ stream properties: SamplingFrequency %d, FrameSize %d, Indp substreams: %d, Dep substreams: %d\n",
                    ParsedFrameHeader->SamplingFrequency,
                    ParsedFrameHeader->Length,
                    NumberOfIndependantSubStreams,
                    NumberOfDependantSubStreams);
                                                                    ...
                                                                    bool SearchForConvSync)
{
    ...
    unsigned int    AlternateFrameSize = 0;
    ...
    if (SyncWord != EAC3_START_CODE)
    {
        ...
        {
            SE_ERROR("Invalid start code 0x%04x\n", SyncWord);
        }
    ...
    if (Bsid <= 8)                                // bsid <= 8 and bsid >= 0
    {
        ...
        if (FsCode >= 3)
        {
            SE_ERROR("Invalid frequency code\n");
            return FrameParserError;
        }
    ...
    else if ((Bsid > 10) && (Bsid <= 16))                              // bsid >10 and bsid <= 16
    {
        ...
        if ((StrmType == 0))// Sub stream type 2 is not supported fron DD+ version 3.0 so reject it
        {
        ...
        {
            /* Sub stream type 2 or 3 that are to be dropped*/
            Type =  TypeEac3Reserved;
        }
        ...
        else
        {
            ...
            if ((fscod2_numblk != 3) && (SearchForConvSync))
            {
                ...
                SE_DEBUG(group_frameparser_audio, "StrmType: %d, substream id: %d, FrameSize: %d, fscod: %d, fscod2_numblk: %d\n",
                         StrmType, ParsedFrameHeader->SubStreamId, FrameSize, fscod, fscod2_numblk);
                SE_DEBUG(group_frameparser_audio, "acmod: %d, lfeon: %d, bsid: %d, dialnorm: %d\n",
                         acmod, lfeon, bsid, dialnorm);

                if (Bits.Get(1))  // compre
                {
                    SE_DEBUG(group_frameparser_audio, "compre on\n");
                ...
                {
                    SE_DEBUG(group_frameparser_audio, "mixmdat enabled\n");
                ...
                {
                    SE_DEBUG(group_frameparser_audio, "infomdat enabled\n");
                ...
                if ((StrmType == 0) && (fscod2_numblk != 3))
                {
                    // we finally got it!!!!!!!
                    convsync = Bits.Get(1);
                    SE_DEBUG(group_frameparser_audio, "Reached convsync: value = %d\n", convsync);
                }
            }
        }

        NbSamples =  256 * EAC3Blocks[fscod2_numblk];
        // This header says it is an I0 frame and has 6 blocks in it and convsync not set
        // So if we have previous frame that is being accumulated with less than 6 blocks we need to push it
        // and send the current frame as a full frame to be pushed independenly later

        // If the number of samples of the currenly accumulated frame is still less than 1536
        // setting the convsync i.e. "FirstBlockForTranscoding" is a way of telling the collator that
        // whatever has been accumulated needs to de delivered as we are at the start of a new frame.
        // See 3.1.8 : Blocks-per-frame Switching (Dolby_Intrinsics_Implementation_Manual.pdf)
        if (EAC3Blocks[fscod2_numblk] == 6 && (StrmType == 0) && (ParsedFrameHeader->SubStreamId == 0))
        {
            convsync = 1;// Fake it
        }
    }
    ...
    {
        /* This is a wrong BSID. Calculation of Frame Size may not be correct.So we guess it as we can't do any thing.
            So calculate first using AC3 type if any error in AC3 Type then calculate using DD+ type framesize calculation*/
        unsigned int   RateCode;
        unsigned int   FsCode;
        unsigned int   BitRate;
        // bool AC3Error = false;
        ...
        if (RateCode >= 38)
        {
            // Cap the RateCode to the maximum offset of the array AC3Rate[]
            RateCode = 36;
            //AC3Error = true;
        }

        //if(AC3Error == false) // No error in the FsCode of RateCode so calculate frame size as AC3
        {
            BitRate = AC3Rate[RateCode >> 1];
            SamplingFrequency = EAC3SamplingFreq[FsCode];
            FrameSize = 2 * ((FsCode == 1) ? ((320 * BitRate / 147 + (RateCode & 1))) : (EAC3FsCodeToFrameSize[FsCode] * BitRate));
        }
        // Now find the size of the DD+ Frame also
        //else // Error in the FsCode or RateCode so calculate frame size as DD+
        {
            Bits.SetPointer(&FrameHeaderBytes[2]);
            int fscod;
            int fscod2_numblk;
            unsigned int DDPSamplingFrequency;
            Bits.Get(2); // Skip stream-type
            Bits.Get(3); // Skip SubStream ID

            if (FrameSize)
            {
                AlternateFrameSize = (Bits.Get(11) + 1) * 2;
            }
            else
            {
                FrameSize = (Bits.Get(11) + 1) * 2;
            ...
            if (fscod == 3)
            {
                DDPSamplingFrequency = EAC3SamplingFreq[fscod2_numblk] >> 1;
                fscod2_numblk = 3; /* six blocks per frame */
            }
            else
            {
                DDPSamplingFrequency =  EAC3SamplingFreq[fscod];
            }

            if (0 != DDPSamplingFrequency)
            {
                SamplingFrequency = DDPSamplingFrequency;
            }
        }
        Type = TypeInValidBSID;
        NbSamples =  1536;
        SE_DEBUG(group_frameparser_audio, "Invalid BSID FOUND : continue decoding FW will take care of this BSID = %d FrameSize = %d,AltFrame = %d\n", Bsid, FrameSize, AlternateFrameSize);
    }

    SE_DEBUG(group_frameparser_audio, "SamplingFrequency: %d, FrameSize: %d, Frame type: % d, NbSamples: %d, convsync: %d\n",
    ...
    //Store the minumum of the two valid sizes
    ParsedFrameHeader->Length = (AlternateFrameSize) ? min(FrameSize, AlternateFrameSize) : FrameSize;
    ParsedFrameHeader->FirstBlockForTranscoding = convsync;
    // Notify the extra size that may need to be jumped.
    // We have already accounted for 1st estimate in ParsedFrameHeader->Length
    ParsedFrameHeader->DeltaLength = (AlternateFrameSize) ? (max(FrameSize, AlternateFrameSize) - ParsedFrameHeader->Length) : 0;
    //
    return FrameParserNoError;
}
...
FrameParser_AudioEAc3_c::FrameParser_AudioEAc3_c(void)
    ...
    , ParsedFrameHeader()
    , FirstTime(true)
    , LastOriginalEncoding(AudioOriginalEncodingUnknown)
    , StreamParameters(NULL)
    , CurrentStreamParameters()
    , FrameParameters(NULL)
    , PreviousSamplingFrequency(48000)
{
...
///
FrameParserStatus_t   FrameParser_AudioEAc3_c::Connect(Port_c *Port)
{
    //
...
FrameParserStatus_t   FrameParser_AudioEAc3_c::ReadHeaders(void)
{
    ...
    if (0 != ParsedFrameHeader.SamplingFrequency)
    {
        PreviousSamplingFrequency = ParsedFrameHeader.SamplingFrequency;
    }
    else
    {
        SE_DEBUG(group_frameparser_audio, "SamplingFrequency %d, %d\n", ParsedFrameHeader.SamplingFrequency, PreviousSamplingFrequency);
        ParsedFrameHeader.SamplingFrequency = PreviousSamplingFrequency;
    }
    ...
    ParsedFrameParameters->NewStreamParameters = (ParsedAudioParameters->OriginalEncoding != LastOriginalEncoding);
    LastOriginalEncoding = ParsedAudioParameters->OriginalEncoding;
