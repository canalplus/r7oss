                                                               ...
                                                               int GivenFrameSize)
{
    ...
    do
    {
        ...
        else
        {
            ...
            SE_ERROR("Unknown substream type: %d\n", NextParsedFrameHeader.Type);
            return FrameParserError;
        }

        if (NextParsedFrameHeader.Length)
        {
    ...
    ParsedFrameHeader->Length  =  FrameSize;
    {
        ...
        if (FirstTime)
        {
            SE_INFO(group_frameparser_audio, "DTS stream properties: SamplingFreq: %d Hz,  FrameSize %d, Nb Samples: %d\n",
                    ParsedFrameHeader->SamplingFrequency,
                    ParsedFrameHeader->Length,
                    ParsedFrameHeader->NumberOfSamples);
            SE_INFO(group_frameparser_audio, "Core substreams: %d, Extension substreams: %d\n", nbCore, nbSub);

            if (nbCore && nbSub)
            {
                SE_INFO(group_frameparser_audio, "DTS core substream properties: SamplingFreq: %d Hz,  FrameSize %d, Nb Samples: %d\n",
            ...
            if (nbSub)
            {
                SE_INFO(group_frameparser_audio, "Number of assets in the extension %d: %d\n",
                        DecodeExtensionId, ParsedFrameHeader->NumberOfAssets);

                for (int astIdx = 0; astIdx < ParsedFrameHeader->NumberOfAssets; astIdx++)
                {
                    SE_INFO(group_frameparser_audio, "Type of Coding Component present in asset %d:\n", astIdx);
                    ...
                    for (int id = CodeDtshdExSSCore, idx = 0; id <= CodeDtshdExtSSXLL; id <<= 1, idx ++)
                    {
                        if (Code & id)
                        {
                            SE_INFO(group_frameparser_audio, " %s\n", DTSExtSubStreamTypeName[idx]);
                        }
                    }
                } // end of asset loop
            }
            ...
            {
                SE_INFO(group_frameparser_audio, "Type of extension present in core: %s\n", DTSCoreSubStreamExtTypeName[ParsedFrameHeader->CoreExtensionType]);
            }
                                                                     ...
                                                                     unsigned char                  SelectedAudioPresentation)
{
    ...
    else if (SyncWord == DTSHD_START_CODE_SUBSTREAM)
    {
        ...
        if (ParseModel.ParseType == ParseForSynchro)
        {
            ...
            {
                SE_DEBUG(group_frameparser_audio, "Not enough header bytes to compute checksum %d vs %d required\n",
            ...
            else
            {
                ...
                if (crc_ref != crc)
                {
                    SE_ERROR("bad checksum in extension substream: 0x%x vs 0x%x\n", crc_ref, crc);
                    return FrameParserError;
                }
            }
        }
                 ...
                 ((ParseModel.ParseType == ParseSmart) && (ParseModel.ParseExtendedIndex == nuExtSSIndex)))
        {
            ...
            FrameParser_AudioDtshd_c::ParseExtensionSubstreamAssetHeader(Bits, &SamplingFrequency, ParsedFrameHeader,
                                                                         nuBits4ExSSFsize, SelectedAudioPresentation);
    ...
    {
        SE_ERROR("Unrecognized frame synchronization: 0x%x\n", SyncWord);
        return FrameParserError;
    }
                                                              ...
                                                              unsigned int SyncWord)
{
    ...
    {
        SE_ERROR("NumberOfPcmSampleBlocks out of range (%d)\n",
    ...
    if (0 == ParsedFrameHeader->CoreSamplingFrequency)
    {
        SE_INFO(group_frameparser_audio, "CoreSamplingFrequency 0 -- using default\n");
        ParsedFrameHeader->CoreSamplingFrequency = 48000;
    }
...
void FrameParser_AudioDtshd_c::ExtractMixData(BitStreamClass_c *Bits, unsigned int *nuNumMixOutCh,
                                              unsigned int *nDecCh, unsigned int ns, unsigned int nE)
{
    ...
    for (unsigned int nCh = 0; nCh < nDecCh[nE]; nCh++)
    {
        ...
        unsigned int nuNumMixCoefs;
        nuNumMixCoefs = FrameParser_AudioDtshd_c::CountBitsSet_to_1(nuMixMapMask[ns][nE][nCh]);

        for (unsigned int nC = 0; nC < nuNumMixCoefs; nC++)
        {
...
void FrameParser_AudioDtshd_c::ParsingDynamicMetadata(BitStreamClass_c *Bits,
...
{
    SE_DEBUG(group_frameparser_audio, "Mix metadata present\n");
    ...
    for (unsigned int ns = 0; ns < nuNumMixOutConfigs; ns++)
    {
        ...
        for (unsigned int nE = 0; nE < nEmDM; nE++)
        {
            ...
            ExtractMixData(Bits, nuNumMixOutCh, nDecCh, ns, nE);
        } // End supplemental channel loop
    } // End configuration loop
} // end of dynamic metadata parsing

void FrameParser_AudioDtshd_c::ParsingNavigationData(BitStreamClass_c *Bits,
                                                     unsigned int       nAst,
...
void FrameParser_AudioDtshd_c::IsCompatibleCorePresent(BitStreamClass_c               *Bits,
                                                       DtshdAudioParsedFrameHeader_t *ParsedFrameHeader,
                                                       unsigned char                   SelectedAudioPresentation,
...
void FrameParser_AudioDtshd_c::ParseNumAssets(BitStreamClass_c *Bits,
                                              DtshdAudioParsedFrameHeader_t *ParsedFrameHeader,
                                              bool           bStaticFieldsPresent,
                                              unsigned int *nuNumMixOutConfigs,
                                              unsigned int *nuNumMixOutCh,
                                              unsigned int *NumAssets,
                                              unsigned int *NumAudioPresent, bool *bMixMetadataEnable)
{
    ...
    {
        SE_DEBUG(group_frameparser_audio, "Static Field present\n");
        unsigned int Temp;
    ...
    * bMixMetadataEnable = bMixMetadataEnabl;
    * NumAudioPresent    = nuNumAudioPresnt;
    * NumAssets          = nuNumAssets;
}
                                                                  ...
                                                                  unsigned char SelectedAudioPresentation)
{
    ...
    ParseNumAssets(Bits               , ParsedFrameHeader , bStaticFieldsPresent,
                   &nuNumMixOutConfigs, nuNumMixOutCh,
                   &nuNumAudioPresnt  , &nuNumAssets      , &bMixMetadataEnabl);
    SE_DEBUG(group_frameparser_audio, "Number of audio presentations: %d, assets: %d\n", nuNumAudioPresnt, nuNumAssets);
    // set default sampling freq in case parsing fails
    *SamplingFrequency = 48000;
    ...
    for (unsigned int nAst = 0; nAst < nuNumAssets; nAst++)
    {
        ...
        {
            ParsingDynamicMetadata(Bits,
                                   bMixMetadataEnabl , bMixMetadataPresent, bEmbededStereoFlag, bEmbededSixChFlag,
                                   nuNumMixOutConfigs, nuNumMixOutCh      , nuTotalNumChs);
        }

        ParsingNavigationData(Bits, nAst,
                              bOne2OneMapChannels2Speakers, bMixMetadataEnabl,  bMixMetadataPresent,
                              nuNumMixOutConfigs          , nuNumMixOutCh    , ParsedFrameHeader,
                              nuBits4ExSSFsize);
    ...
    IsCompatibleCorePresent(Bits, ParsedFrameHeader, SelectedAudioPresentation, nuNumAudioPresnt);
}
                                                               ...
                                                               unsigned char *FrameHeaderBytes)
{
    ...
    case DTSHD_START_CODE_LBR:
    {
        ...
        if (nLBRSampleRateCode >= 16)
        {
            SE_INFO(group_frameparser_audio, "Found invalid nLBRSampleRateCode:%d; forcing 12\n", nLBRSampleRateCode);
            nLBRSampleRateCode = 12;
        }
    ...
    default:
        SE_ERROR("Found coding component different from xll, lbr or core in extension-only substream (Sync: 0x%x)!\n", Sync);
        break;
    }
}
...
///
FrameParserStatus_t   FrameParser_AudioDtshd_c::Connect(Port_c *Port)
{
    //
