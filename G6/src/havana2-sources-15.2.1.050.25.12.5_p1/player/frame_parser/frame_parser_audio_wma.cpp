                                                              ...
                                                              bool                         Verbose)
{
    ...
    if (!FrameData)
    {
        SE_ERROR("Failed to decode ASF stream properties object\n");
    ...
    if (!FrameData)
    {
        SE_ERROR("Failed to decode audio stream's type specific data (WaveFormatEx)\n");
        return FrameParserError;
    }
    ...
    if (!FrameData)
    {
        SE_ERROR("Failed to decode audio stream's type specific data (WMA)\n");
        return FrameParserError;
    }
    ...
    if (Verbose)
    {
        SE_INFO(group_frameparser_audio, "SamplesPerFrame = %d\n", StreamParameters->SamplesPerFrame);
    }
    ...
    SE_DEBUG(group_frameparser_audio, "Average frame duration is %dms\n", FrameDuration);
    ...
    if (Verbose)
    {
        SE_INFO(group_frameparser_audio, "SamplesPerBlock = %d\n", StreamParameters->SamplesPerBlock);
    }
...
///
FrameParserStatus_t   FrameParser_AudioWma_c::Connect(Port_c *Port)
{
    //
...
FrameParserStatus_t   FrameParser_AudioWma_c::ReadHeaders(void)
{
    ...
    //SE_DEBUG(group_frameparser_audio, "READHEADERS: Len-%d\n", BufferLength);
    ...
    else
    {
        ...
        ParsedAudioParameters->SampleCount                  = WMA_BLOCK_WISE_SAMPLE_COUNT; //Fixed to 1024 for WMA blockwise
        ParsedAudioParameters->Organisation                 = 0; // filled in by codec
    }

    Stream->Statistics().FrameParserAudioSampleRate = CurrentStreamParameters.SamplingFrequency;
    // Reporting the actual frame size is rather unhelpful for WMA. This is partly because WMA
    // is frequently VBR but mostly because we don't know where the frame boundaries are until
    // the data goes to the decoder anyway (frame boundaries only become apparant *after* the
    // huffman decode).
    Stream->Statistics().FrameParserAudioFrameSize = 0;
    return FrameParserNoError;
}
...
FrameParserStatus_t   FrameParser_AudioWma_c::ResetReferenceFrameList(void)
{
    ...
    Stream->GetParseToDecodeEdge()->CallInSequence(SequenceTypeImmediate, TIME_NOT_APPLICABLE, CodecFnReleaseReferenceFrame, CODEC_RELEASE_ALL);
    return FrameParserNoError;
}
...
FrameParserStatus_t   FrameParser_AudioWma_c::GeneratePostDecodeParameterSettings(void)
{
    ...
    {
        SE_DEBUG(group_frameparser_audio, "No timestamp (collator did not provide one)\n");
    }
    ...
    if (Status != FrameParserNoError)
    {
        SE_INFO(group_frameparser_audio,  "POSTDECODE: HandleCurrentFrameNormalizedPlaybackTime failed\n");
        SE_ERROR("POSTDECODE: HandleCurrentFrameNormalizedPlaybackTime failed\n");
        return Status;
    }
