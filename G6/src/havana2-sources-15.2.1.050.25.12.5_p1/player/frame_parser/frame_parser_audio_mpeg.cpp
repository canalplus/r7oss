...
// these are former members of the Player1 MPEG frame analyser class
    ...
    default:        SE_ERROR("Bad MPEG selection\n");
        return FrameParserError;
        break;
    }
    ...
    if (BitRate == 0)
    {
        SE_ERROR("Invalid bit rate, %x, %x, %x, %x\n",
    ...
    {
        SE_ERROR("Invalid frequency, %x, %x, %x\n",
    ...
    //
    SE_DEBUG(group_frameparser_audio,  "Header %8x, MPEG%dL%d, BitRate %3d, Frequency %5d, FrameSize %d, Padding %d\n",
...
FrameParserStatus_t FrameParser_AudioMpeg_c::ParseExtensionHeader(unsigned char *ExtensionHeader, unsigned int *ExtensionLength)
{
    ...
    if (0x7ff != frameSync)
    {
        // not an error because this method is called speculatively
        SE_DEBUG(group_frameparser_audio,  "Invalid start code %x\n", frameSync);
        return FrameParserError;
    }
    ...
    if (0 != id)
    {
        SE_ERROR("Invalid (reserved) ID %x\n", id);
        return FrameParserError;
    }

    SE_DEBUG(group_frameparser_audio,  "FrameSync %03x  CRC %04x  FrameSize %d  ID %d\n", frameSync, crc, lengthInBytes, id);
...
///
FrameParserStatus_t   FrameParser_AudioMpeg_c::Connect(Port_c *Port)
{
    //
...
FrameParserStatus_t   FrameParser_AudioMpeg_c::ReadHeaders(void)
{
    ...
    Stream->Statistics().FrameParserAudioFrameSize = FrameParameters->FrameSize;
    return FrameParserNoError;
}
...
FrameParserStatus_t   FrameParser_AudioMpeg_c::ResetReferenceFrameList(void)
{
    ...
    Stream->GetParseToDecodeEdge()->CallInSequence(SequenceTypeImmediate, TIME_NOT_APPLICABLE,
