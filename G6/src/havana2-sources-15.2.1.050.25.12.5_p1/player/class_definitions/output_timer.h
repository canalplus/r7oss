...
enum
{
    ...
    OutputTimerDropFrame            = BASE_OUTPUT_TIMER,
    OutputTimerDropFrameSingleGroupPlayback,
    OutputTimerDropFrameKeyFramesOnly,
    OutputTimerDropFrameReferenceFramesOnly,
    ...
    OutputTimerDropFrameTooEarlyForManifestation
};
...
enum
{
    OutputTimerFnResetTimeMapping = BASE_OUTPUT_TIMER,
};
...
public:
    ...
    virtual OutputTimerStatus_t   ResetOnStreamSwitch(void) = 0;

    virtual OutputTimerStatus_t   ResetTimingContext(unsigned int ManifestationIndex = INVALID_INDEX) = 0;
    ...
    virtual void                  FillOutManifestationTimings(Buffer_t Buffer) = 0;
    ...
    virtual void                  MonitorGroupStructure(ParsedFrameParameters_t  *ParsedFrameParameters) = 0;
    virtual OutputTimerStatus_t   SetSmoothReverseSupport(bool SmoothReverseSupportFromParser) = 0;
    virtual OutputTimerStatus_t   TrickModeControl(void) = 0;
...
/*! \fn OutputTimerStatus_t OutputTimer_c::ResetTimingContext(  unsigned int      ManifestationIndex );

    \brief Reset the timing context for a particular manifestation.

    This function clears out the state associated with a particular output timing
    sequence for one manifestation. It does not impact the time mapping for the whole stream.

    \param ManifestationIndex The index in the output timing structure and
           output surface descriptor list, that is impacted.
