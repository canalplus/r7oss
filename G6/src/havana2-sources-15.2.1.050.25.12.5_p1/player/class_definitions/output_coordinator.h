...
enum
{
    ...
    OutputCoordinatorOffsetsNotEstablished
};
...
{
public:
    virtual void DrainLivePlayback(void) = 0;
    ...
    virtual OutputCoordinatorStatus_t   SynchronizeStreams(OutputCoordinatorContext_t  Context,
                                                           ...
                                                           unsigned long long         *SystemTime,
                                                           void                       *ParsedAudioVideoDataParameters) = 0;

    virtual OutputCoordinatorStatus_t   PerformEntryIntoDecodeWindowWait(
        OutputCoordinatorContext_t Context,
        unsigned long long         NormalizedDecodeTime,
        unsigned long long         DecodeWindowPorch) = 0;
    ...
    virtual OutputCoordinatorStatus_t   HandlePlaybackTimeDeltas(
        ...
        long long int               DeltaCollationTime) = 0;

    virtual OutputCoordinatorStatus_t   CalculateOutputRateAdjustments(
        OutputCoordinatorContext_t    Context,
    ...
    virtual OutputCoordinatorStatus_t   RestartOutputRateIntegration(
        ...
        unsigned int                TimingIndex = INVALID_INDEX) = 0;

    virtual OutputCoordinatorStatus_t   ResetTimingContext(OutputCoordinatorContext_t Context,
                                                           unsigned int               TimingIndex = INVALID_INDEX) = 0;

    virtual OutputCoordinatorStatus_t   AdjustMappingBase(OutputCoordinatorContext_t Context,
                                                          long long                  Adjustment,
                                                          bool                       ResetVSyncOffsets) = 0;
    ...
    virtual OutputCoordinatorStatus_t   MonitorVsyncOffsets(OutputCoordinatorContext_t   Context,
                                                            OutputTiming_t              *OutputTiming,
                                                            long long int              **VsyncOffsets) = 0;

    virtual OutputCoordinatorStatus_t   ClockRecoveryInitialize(PlayerTimeFormat_t    SourceTimeFormat) = 0;
    virtual OutputCoordinatorStatus_t   ClockRecoveryIsInitialized(bool              *Initialized) = 0;
...
/*! \fn OutputCoordinatorStatus_t   OutputCoordinator_c::HandlePlaybackTimeDeltas(
    ...
    \param DeltaCollationTime The delta in collation times (can be negative after frame re-ordering).

    \return OutputCoordinator status code, OutputCoordinatorNoError indicates success.
*/

/*! \fn OutputCoordinatorStatus_t   OutputCoordinator_c::CalculateOutputRateAdjustments(
                                OutputCoordinatorContext_t    Context,
                                OutputTiming_t            OutputTiming,
                                Rational_t          **OutputRateAdjustments,
    ...
    \param Context Stream context identifier.
    \param OutputTiming containing the returned information from the manifestors.
    \param OutputRateAdjustments Pointer to a variable to hold the address of the array of output rate adjustments (held within the coordinator)..
    \param SystemClockAdjustment The current value of the system clock adjustment factor.

    \return OutputCoordinator status code, OutputCoordinatorNoError indicates success.
*/

/*! \fn OutputCoordinatorStatus_t   OutputCoordinator_c::RestartOutputRateIntegration( OutputCoordinatorContext_t     Context );
...
/*! \fn OutputCoordinatorStatus_t   OutputCoordinator_c::ResetTimingContext(     OutputCoordinatorContext_t        Context,
                                                                unsigned int                      TimingIndex = INVALID_INDEX );
    \brief Reset a timing context

    Completely reset a timing context, used during reset, and when a new manifestation is connected to an output.

    \param Context Stream context identifier.
    \param TimingIndex The index of the surface descriptor for this timing context.
...
/*! \fn     OutputCoordinatorStatus_t   OutputCoordinator_c::MonitorVsyncOffsets(   OutputCoordinatorContext_t    Context,
                                            OutputTiming_t            OutputTiming,
                                            long long int           **VsyncOffsets );

    \brief Monitor vsync offsets.

    This function is provided to monitor the vsync offsets from the expected values. Used to support dvp control
    when not using vsync locking, and to provide information for the video synchronization.
    In addition to monitoring the vsync offsets, the function will adjust the time mapping to minimise them,
    when we have more than one video manifestation.

    \param Context Stream context identifier.
    \param OutputTiming Returned values from manifestation.
    \param VsyncOffsets Pointer to a variable to hold the address of the array of offset values (held within the coordinator).
...
/*! \fn OutputCoordinatorStatus_t   OutputCoordinator_c::ClockRecoveryIsInitialized(
                            bool    *Initialized )
\brief Checking if clock recovery is initialized

\param Initialized true when clock recovery is initialized
