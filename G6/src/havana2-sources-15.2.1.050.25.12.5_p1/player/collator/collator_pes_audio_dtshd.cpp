...
Collator_PesAudioDtshd_c::Collator_PesAudioDtshd_c(void)
    : Collator_PesAudioDvd_c(DTSHD_FRAME_HEADER_SIZE)
    , CoreFrameSize(0)
    , GotCoreFrameSize(false)
    , EightChannelsRequired(false) // by default do only 5.1...
    , SyncFrameHeader()
    , Bits()
{
...
CollatorStatus_t Collator_PesAudioDtshd_c::DecideCollatorNextStateAndGetLength(unsigned int *FrameLength)
{
    ...
    if (!GotCoreFrameSize)
    {
        ...
        if (Status == CollatorNoError)
        {
            ...
            // negative.
            SE_ASSERT(((int) FrameHeaderLength + CodeOffset) >= 0);
...
CollatorStatus_t Collator_PesAudioDtshd_c::HandlePesPrivateData(unsigned char *PesPrivateData)
{
    ...
    if (CollatorState == SeekingSyncWord)
    {
        ...
        BitsLoc.SetPointer(PesPrivateData);
        unsigned int SubStreamId = BitsLoc.Get(8);
        unsigned int NumberOfFrameHeaders = BitsLoc.Get(8);
        unsigned int FirstAccessUnitPointer = BitsLoc.Get(16);
