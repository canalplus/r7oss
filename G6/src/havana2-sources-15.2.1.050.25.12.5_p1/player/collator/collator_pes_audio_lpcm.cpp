...
Collator_PesAudioLpcm_c::Collator_PesAudioLpcm_c(LpcmStreamType_t GivenStreamType)
    : Collator_PesAudio_c(LPCM_FRAME_HEADER_SIZE)
...
CollatorStatus_t Collator_PesAudioLpcm_c::DecideCollatorNextStateAndGetLength(unsigned int *FrameLength)
{
    ...
    if (IsFirstPacket)
    {
        ...
        *FrameLength = PesPrivateToSkip +
        ...
        PesPrivateToSkip = 0;
        SE_INFO(group_collator_audio, "First packet: Skipping %d bytes\n", *FrameLength);
        return (Status);
    }
    ...
    if (AccumulatePrivateDataArea)
    {
        ...
        AccumulatePrivateDataArea = false;
        SE_DEBUG(group_collator_audio, "Accumulate PDA of length %d\n", Configuration.ExtendedHeaderLength);
    }
    ...
    else if (PesPrivateToSkip > 0)
    {
        ...
        *FrameLength = min(PesPrivateToSkip, RemainingElementaryLength);
        ...
        SE_DEBUG(group_collator_audio, "Skipping %d bytes of pda\n", *FrameLength);
    }
             ...
             IsPesPrivateDataAreaNew)
    {
        ...
        SE_DEBUG(group_collator_audio, "Flush after %d audio frames\n", AccumulatedFrameNumber);
        AccumulatedFrameNumber = 0;
        ...
        *FrameLength = 0;
        ...
        SE_DEBUG(group_collator_audio, "PlaybackTime: %llx\n", PlaybackTime);
    }
    else
    {
        ...
        SE_ASSERT(0 == RemainingDataLength);
        SE_ASSERT(0 == PesPrivateToSkip);
        ...
        SE_DEBUG(group_collator_audio, "Read frame of size %d (total frames in this chunk: %d)\n",
                 *FrameLength,
                 AccumulatedFrameNumber);
    }
    ...
    else
    {
        ...
        else
        {
            if ((ParsedFrameHeader.SubStreamId != NextParsedFrameHeader.SubStreamId) ||
                ...
                ((StreamType == TypeLpcmSPDIFIN) && (ParsedFrameHeader.NumberOfSamples != NextParsedFrameHeader.NumberOfSamples)))
            {
        ...
        {
            SE_DEBUG(group_collator_audio, "Uncorrect FirstAccessUnitPointer (%d. vs expected %d)\n", NextParsedFrameHeader.FirstAccessUnitPointer, GuessedNextFirstAccessUnit);
            GuessedNextFirstAccessUnit = 0;
        }
