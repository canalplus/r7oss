...
/* Content of this structure is to be sent as PES payload, it would be metadata PES payload which SE would receive
   if PES_EXTENSION_DATA_PRESENT is enabled in PES header. It is used in MPEG4P2 to initialize width/height. this would
   be used in case VOL header is not received before VOP */
typedef struct MPEG4MetaData_s
{
    unsigned int Width;
    unsigned int Height;
} MPEG4MetaData_t;
