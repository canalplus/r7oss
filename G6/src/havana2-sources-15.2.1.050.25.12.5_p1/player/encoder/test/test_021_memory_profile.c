...
#define WISHED_VIDEO_ENCODE_PROFILE STM_SE_CTRL_VALUE_ENCODE_CIF_PROFILE
...
{
    int retval = 0;
    stm_se_capture_buffer_t *p_capture_buffer;
    u32 crc = 0;
    int loop = 0;
    ...
    //
    p_capture_buffer = (stm_se_capture_buffer_t *)memsinkDescriptor->m_Buffer;
    p_capture_buffer->physical_address = NULL;
    p_capture_buffer->virtual_address  = &memsinkDescriptor->m_Buffer      [sizeof(stm_se_capture_buffer_t)];
    p_capture_buffer->buffer_length    = COMPRESSED_BUFFER_SIZE           - sizeof(stm_se_capture_buffer_t);

    retval = stm_memsink_pull_data(memsinkDescriptor->obj,
                                   p_capture_buffer,
                                   p_capture_buffer->buffer_length,
    ...
    // Check for EOS
    if (p_capture_buffer->u.compressed.discontinuity & STM_SE_DISCONTINUITY_EOS)
    {
        pr_info("%s EOS Detected! discontinuity 0x%x\n", __func__, p_capture_buffer->u.compressed.discontinuity);
        return 1;
    }

#ifdef DUMP_OUTPUT_BUFFER
    print_hex_dump(KERN_INFO, "OUTPUT BUFFER DUMP IN HEXADECIMAL: ", DUMP_PREFIX_OFFSET, 16, 1,
                   p_capture_buffer->virtual_address, min((unsigned) 64, memsinkDescriptor->m_availableLen), true);
#endif

    crc = crc32_be(crc, p_capture_buffer->virtual_address, memsinkDescriptor->m_availableLen);
...
static int StaticImageEncode(EncoderContext *context, pp_descriptor *sink)
{
    ...
    for (i = 0; i < (int)(VIDEO_FRAME_RATE_NUM / VIDEO_FRAME_RATE_DEN); i++)
    {
...
static int MemoryProfileTest(EncoderTestArg *arg)
{
    EncoderContext context = {0};
    int MemoryProfile = 0xBEEF;
    int result = 0;
    stm_se_picture_resolution_t resolution;

    pr_info("Start MemoryProfileTest\n");
    ...
    // Apply suitable memory profile: CIF (352x288) as test frame is 320x240

    pr_info("call profile set control\n");
    result = stm_se_encode_set_control(context.encode, STM_SE_CTRL_VIDEO_ENCODE_MEMORY_PROFILE, WISHED_VIDEO_ENCODE_PROFILE);
    if (result < 0)
    {
    ...
    // Check applied memory profile

    pr_info("call profile get control\n");
    result = stm_se_encode_get_control(context.encode, STM_SE_CTRL_VIDEO_ENCODE_MEMORY_PROFILE, &MemoryProfile);
    if (result < 0)
    {
        pr_err("Error: %s Failed to get memory profile control on encode\n", __func__);
        EncoderTestCleanup(&context);
        return result;
    }
    if (MemoryProfile != WISHED_VIDEO_ENCODE_PROFILE)
    {
        pr_err("Error: %s Failed to get expected memory profile on encode - %d (expected %d)\n", __func__, MemoryProfile, WISHED_VIDEO_ENCODE_PROFILE);
        EncoderTestCleanup(&context);
        return -1;
    }
    ...
    pr_info("call encode stream new\n");
    ...
    // Check non suitable encode resolution request is denied (resolution > profile)

    resolution.width  = VIDEO_WIDTH;
    resolution.height = VIDEO_HEIGHT + 100;
    result = stm_se_encode_stream_set_compound_control(context.video_stream[0], STM_SE_CTRL_VIDEO_ENCODE_STREAM_RESOLUTION, (void *)&resolution);
    if (result >= 0)
    {
        pr_err("Error: %s Expected this STM_SE_CTRL_VIDEO_ENCODE_STREAM_RESOLUTION control to fail because of memory profile\n", __func__);
        EncoderTestCleanup(&context);
        return -1;
    }

    resolution.width  = VIDEO_WIDTH + 100;
    resolution.height = VIDEO_HEIGHT;
    result = stm_se_encode_stream_set_compound_control(context.video_stream[0], STM_SE_CTRL_VIDEO_ENCODE_STREAM_RESOLUTION, (void *)&resolution);
    if (result >= 0)
    {
    ...
    if (result < 0)
    {
        pr_info("%s: stm_se_encode_delete returned %d\n", __func__, result);
        return result;
    }
...
ENCODER_TEST(MemoryProfileTest, "Test encoding using memory profiles", BASIC_TEST);
