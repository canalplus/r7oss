...
static int encode_event_test(EncoderTestArg *arg)
{
    ...
    result = stm_se_encode_stream_new("EncodeStream0", context.encode, STM_SE_ENCODE_STREAM_ENCODING_AUDIO_AAC, &context.audio_stream[0]);

    if (result < 0)
    {
        /* Try AC-3 */
        result = stm_se_encode_stream_new("EncodeStream0", context.encode, STM_SE_ENCODE_STREAM_ENCODING_AUDIO_AC3, &context.audio_stream[0]);

        if (result < 0)
        {
            /* Try MP3 */
            result = stm_se_encode_stream_new("EncodeStream0", context.encode, STM_SE_ENCODE_STREAM_ENCODING_AUDIO_MP3, &context.audio_stream[0]);

            if (result < 0)
            {
                pr_err("Error: %s Error: Could not create an encoder audio stream of any kind\n", __func__);
                EncoderTestCleanup(&context);
                return result;
            }
            else
            {
                pr_warn("%s: Warning: encoder audio stream : replaced AAC with MP3\n", __func__);
                gAudioTest.ExpectedTestResult = NULL;
            }
        }
        else
        {
            pr_warn("%s: Warning: encoder audio stream : replaced AAC with AC3\n", __func__);
            gAudioTest.ExpectedTestResult = NULL;
        }
    }
    ...
    if (result < 0)
    {
        ...
        pr_err("Error: %s Check your companion rev is >= BL34_8p1: AAC ADTS header changed\n", __func__);
...
ENCODER_TEST(encode_event_test, "Test frame encoded event", BASIC_TEST);
