...
#define DEFAULT_FREE_MEMORY_TARGET (2*1024*1024) // 2 MBytes
...
static int video_encode_memory_full(EncoderTestArg *arg)
{
    ...
    // Fill buffer context
    pContext->raw_video_frame[0] = raw_video_frame;
    ...
    pContext->encode_id = 0;
    result = encode(pContext);
    if (result != 0)
    {
        encode_error = 1;
    }
    ...
    pContext->raw_video_frame[0].virtual = NULL;
    pContext->raw_video_frame[0].physical = 0;
    pContext->raw_video_frame[0].size = 0;
...
ENCODER_TEST(video_encode_memory_full, "Video encode until memory full", BASIC_TEST);
