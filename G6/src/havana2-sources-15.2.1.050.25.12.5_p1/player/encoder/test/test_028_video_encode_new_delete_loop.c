...
static int inject(EncoderContext *pContext)
{
    int iteration = 0;
    int result = 0;
    ...
    for (iteration = 0 ; iteration < INJECT_COUNT ; iteration++)
    {
    ...
    if (gVideoTest[pContext->stream_id].FatalErrorReceived > 0)
    {
        pr_err("Error: %s Fatal error event, FatalErrorReceived = %u\n", __func__, gVideoTest[pContext->stream_id].FatalErrorReceived);
        result = -1;
        goto inject_fail;
    }

    gVideoTest[pContext->stream_id].FatalErrorReceived = 0;
...
static int video_encode_new_delete_loop(EncoderTestArg *arg)
{
    EncoderContext *pContext;
    int result = 0;
    int iteration = 0;
    int encode_new_iteration = 0;
    int init_stream_iteration = 0;
    int encode_iteration = 0;
    int stream_iteration = 0;
    int deinit_stream_iteration = 0;
    int encode_delete_iteration = 0;
    char encode_name[32];
    int duration = 0;
    ...
    do
    {
        ...
        for (encode_new_iteration = 0 ; encode_new_iteration < TEST_MAX_ENCODE ; encode_new_iteration++)
        {
            pContext = &gContext[encode_new_iteration];
            pContext->encode_id = encode_new_iteration;
            sprintf(encode_name, "Encode%02d", encode_new_iteration);

            // Create an Encode Object
            result = stm_se_encode_new(encode_name, &pContext->encode);
            if (result < 0)
            {
            ...
            for (init_stream_iteration = 0 ; init_stream_iteration < TEST_MAX_ENCODE_STREAM ; init_stream_iteration++)
            {
                pContext = &gContext[encode_new_iteration];
                pContext->stream_id = init_stream_iteration;
        ...
        for (encode_delete_iteration = 0 ; encode_delete_iteration < TEST_MAX_ENCODE ; encode_delete_iteration++)
        {
            pContext = &gContext[encode_delete_iteration];
            pContext->encode_id = encode_delete_iteration;
            for (deinit_stream_iteration = 0 ; deinit_stream_iteration < TEST_MAX_ENCODE_STREAM ; deinit_stream_iteration++)
            {
                pContext->stream_id = deinit_stream_iteration;
...
init_fail:
set_control_fail:
    for (encode_delete_iteration = 0 ; encode_delete_iteration < encode_new_iteration ; encode_delete_iteration++)
    {
        pContext = &gContext[encode_delete_iteration];
        pContext->encode_id = encode_delete_iteration;
        for (deinit_stream_iteration = 0 ; deinit_stream_iteration < init_stream_iteration ; deinit_stream_iteration++)
        {
            pContext->stream_id = deinit_stream_iteration;
            (void) deinit_stream(pContext);
        }
    }
new_fail:
deinit_fail:
    ...
    {
        pContext = &gContext[encode_delete_iteration];
        (void) stm_se_encode_delete(pContext->encode);
    }
...
ENCODER_TEST(video_encode_new_delete_loop, "Video encode creation/destruction loop", BASIC_TEST);
