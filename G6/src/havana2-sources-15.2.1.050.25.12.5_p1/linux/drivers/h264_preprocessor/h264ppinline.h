                                                    ...
                                                    void               *OutputBufferPhysicalAddress)
{
	...
	if (Slice->first_mb_in_slice != 0) {
		SE_ERROR("Attempt to process a buffer with a non-first slice\n");
		return h264pp_error;
	}
	...
	Parameters.MaxOutputBufferSize         = MaxOutputBufferSize;
	...
	if (Status != OSDEV_NoError) {
		SE_ERROR("Failed to queue a buffer for processing\n");
		return h264pp_error;
	}
                                                            ...
                                                            unsigned int       *ErrorMask)
{
	...
	if (Status != OSDEV_NoError) {
		SE_ERROR("Failed to get a pre-processed buffer\n");
		return h264pp_error;
	}
