...
static struct stm_plat_blit_data plat_blit_7540[] = {
	[0] = {
...
static struct platform_device bdisp_7540[] = {
	[0] = {
	.name = "stm-bdispII",
	.id = 0,
	.num_resources = 7,
...
static struct platform_device *fli7540_devices[] __initdata = {
	&bdisp_7540[0],
	&bdisp_7540[1],
};


static int __init stm_blit_fli7540_init(void)
{
	int n_devices;
	...
	n_devices = 1;
#else
	n_devices = ARRAY_SIZE(fli7540_devices);
	...
	return platform_add_devices(fli7540_devices, n_devices);
}

static void __exit stm_blit_fli7540_exit(void)
{
	stm_blit_printd(0, "%s\n", __func__);
	...
	if (cpu_data->cut_major > 1)
		platform_device_unregister(&bdisp_7540[1]);
#endif
	platform_device_unregister(&bdisp_7540[0]);
}
...
{
	return stm_blit_fli7540_init();
}

void __exit stm_blit_glue_exit(void)
{
	stm_blit_fli7540_exit();
}

#else /* UNIFIED_DRIVER */

MODULE_AUTHOR("André Draszik <andre.draszik@st.com");
MODULE_DESCRIPTION("STMicroelectronics FLi7540 BDispII driver glue");
MODULE_LICENSE("GPL");

module_init(stm_blit_fli7540_init);
module_exit(stm_blit_fli7540_exit);
