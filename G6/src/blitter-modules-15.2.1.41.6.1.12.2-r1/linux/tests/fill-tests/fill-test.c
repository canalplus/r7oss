 ...
 *
 * Test to exercise Fill Rectangles Operations using the blitter hw
 */
...
#define FILL_TEST_AUTHOR "Akram BEN BELGACEM <akram.ben-belgacem@st.com>"
#define FILL_TEST_DESC   "A sample fill rectangles test operations using the blitter driver"

#define DONT_ALLOCATE_DESTINATION_BUFFER

/* Test module parameters */
static int number_of_loops = 1;
...
static int __init fill_test_init(void)
{
	...
	stm_blitter_surface_h 				surface			= NULL;
	...
	stm_blitter_rect_t 				   *fill_rects		= NULL;
	int									i				= 0;
	...
	printk(KERN_INFO "Starting Fill Rectangle tests\n");
#ifdef DONT_ALLOCATE_DESTINATION_BUFFER
  ...
  Dest.memory         = fbuffer;
	...
	if((stm_blitter_surface_get(format,colorspace,&buffer_address,buffer_size,&size,pitch, &surface)) < 0)
	{
		printk(KERN_ERR "Failed to get a blitter device!\n");
	...
	printk(KERN_INFO "Starting Fill Rectangle tests (First Implemented API)..\n");
	color_p->b = 0x80;
	stm_blitter_surface_clear (blitter, surface, color_p);
	...
	for (i = 0; i < 256; i++)
	{
		...
		/* Fill ops */
		if(stm_blitter_surface_fill_rect(blitter, surface, &dst_rect) < 0)
		{
			printk(KERN_ERR "FillRectangle ops failed!\n");
			stm_blitter_surface_put(surface);
			stm_blitter_put(blitter);
			goto out_surface;
		}
	...
	save_gam_file("/root/dumped_fill_t1_dest.gam", &Dest);

	printk(KERN_INFO "Starting Fill Rectangle tests (Second Implemented API)..\n");
	color_p->a = 0xff;
	color_p->r = 0x00;
	color_p->g = 0x00;
	color_p->b = 0xff;
	...
	for (i = 0; i < 256; i++)
	{
		...
		/* Fill ops */
		if(stm_blitter_surface_fill_rect2(blitter, surface, dst_rect.position.x, dst_rect.position.y, dst_rect.size.w, dst_rect.size.h) < 0)
		{
			printk(KERN_ERR "FillRectangle ops failed!\n");
			stm_blitter_surface_put(surface);
			stm_blitter_put(blitter);
	...
	save_gam_file("/root/dumped_fill_t2_dest.gam", &Dest);

	printk(KERN_INFO "Starting Fill Rectangles tests (Single API call)..\n");
	color_p->a = 0xff;
	color_p->r = 0x00;
	color_p->g = 0xff;
	color_p->b = 0xff;
	...
	/* Allocate memory for surface buffer */
	fill_rects = kmalloc(256*sizeof(stm_blitter_rect_t), GFP_KERNEL);
	if(!fill_rects)
	{
		printk(KERN_ERR "Failed to allocate memory!\n");
		goto out_surface;
	...
	for (i = 0; i < 256; i++)
	{
		fill_rects[i].size.w = 300;
		fill_rects[i].size.h = 200;
		fill_rects[i].position.x = i + ((i * fill_rects[i].size.w)%BITMAP_HEIGHT);
		fill_rects[i].position.y = i + ((i * fill_rects[i].size.h)%BITMAP_WIDTH);
		printk ("rect: %d %d %d %d\n", (int)fill_rects[i].position.x, (int)fill_rects[i].position.y,
			  (int)fill_rects[i].size.w, (int)fill_rects[i].size.h);
	}

	/* Fill ops */
	if(stm_blitter_surface_fill_rects(blitter, surface, fill_rects, i-1) < 0)
	{
		printk(KERN_ERR "FillRectangles ops failed!\n");
		stm_blitter_surface_put(surface);
		stm_blitter_put(blitter);
		kfree(fill_rects);
		goto out_surface;
	...
	save_gam_file("/root/dumped_fill_t3_dest.gam", &Dest);
	...
	printk(KERN_INFO " Fill Rectangles tests finished\n");
...
static void __exit fill_test_exit(void)
{
	printk(KERN_INFO " Fill Rectangles tests module removed.\n");
}

module_init(fill_test_init);
module_exit(fill_test_exit);

MODULE_LICENSE("GPL");
MODULE_AUTHOR(FILL_TEST_AUTHOR);
MODULE_DESCRIPTION(FILL_TEST_DESC);
