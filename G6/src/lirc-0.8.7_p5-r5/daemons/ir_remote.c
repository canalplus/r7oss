			    ...
			    lirc_t min_remaining_gap, lirc_t max_remaining_gap)
{
	...
	struct timeval prev_release_time;
	...
	// Hack to force a key release if the new press happens after previous event + gap but was not already taken into account.
	if(timerisset(&remote->last_send))
	{
		int secs;
		prev_release_time.tv_usec = remote->last_send.tv_usec + upper_limit(remote, remote->max_total_signal_length)+5000;
		secs = prev_release_time.tv_usec / 1000000;
		prev_release_time.tv_usec -= secs*1000000;
		prev_release_time.tv_sec = remote->last_send.tv_sec + secs;
		if(timercmp(&current, &prev_release_time, >))
		{
			reset_release_time();
			repeat_flag = 0;
		}
