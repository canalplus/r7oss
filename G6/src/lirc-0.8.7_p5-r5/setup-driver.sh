...
# DO NOT EDIT
    ...
    if test "$?" = "0"; then
        ...
        if false; then :
			...
			5 "ITE IT8704/05/12/18/20 CIR port" \
			6 "ITE8709 CIR port" \
			...
			w "Streamzap PC Remote" \
			x "TechnoTrend USB IR receiver" \
			y "USB-UIRT" \
			z "VLSystem MPlay Blast" \
			A "VLSystem MPlay Mini" \
			B "Windows Media Center Transceivers/Remotes (all)" 2> $TEMP
            if test "$?" = "0"; then
                {
                set `cat $TEMP`
                if false; then :
                ...
                elif test "$1" = "w"; then LIRC_DRIVER=streamzap;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "x"; then LIRC_DRIVER=ttusbir;		DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "y"; then LIRC_DRIVER=usb_uirt_raw;	DRIVER_PARAMETER=ttyUSB1; DRIVER_PARAM_TYPE=ttyUSB;
                elif test "$1" = "z"; then LIRC_DRIVER=mplay;		DRIVER_PARAMETER=ttyUSB1; DRIVER_PARAM_TYPE=ttyUSB;
                elif test "$1" = "A"; then LIRC_DRIVER=mplay;		DRIVER_PARAMETER=ttyUSB1; DRIVER_PARAM_TYPE=ttyUSB;
                elif test "$1" = "B"; then LIRC_DRIVER=mceusb;		DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                fi
                }
                else
                    return;
