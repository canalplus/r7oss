...
int get_arg(void **, infra_type_t , void *, uint8_t );
...
infra_error_code_t get_param_data(char *, void *);
...
int get_number_of_tokens(char *data_p)
{
	...
	size = strlen(data_p);
	data_copy_p = infra_os_malloc(size);
...
EXPORT_SYMBOL(get_number_of_tokens);
...
EXPORT_SYMBOL(get_arg_size);

int
get_arg(void **data_pp, infra_type_t type, void *argument_p, uint8_t arg_size)
{
	...
	infra_test_debug("\tSeparated Token : %s", token_p);
	...
	*data_pp = (void *)data_copy_p;
	...
	switch(type){
		case STRING :
			...
			infra_test_debug("\tChecking for String");
			error = check_for_string(token_p, argument_p, size);
			infra_test_debug( "\tcase STRING: Error (%d) ", error);
			if (error == 0)
				return size;
			else
				return error;
		...
		case INTEGER :
			...
			infra_test_debug("\tChecking %s  for Integer Type",
					(char*)token_p);
			error = check_for_integer(token_p, argument_p);
			infra_test_debug( "\tcase INTEGER: Error (%d) ", error);
			return error;
		...
		case HEX :
			...
			infra_test_debug("\tChecking %s  for Integer Type",
					(char*)token_p);
			error = check_for_integer(token_p, argument_p);
			infra_test_debug( "\tcase HEX: Error (%d) ", error);
			return error;

		case UNKNOWN :
			error = -EINVAL;
			infra_test_debug( "\tcase UNKNOWN: Error (%d) ", error);
			return -EINVAL;
	}
...
int check_for_string(char *data, void *argument, uint8_t size)
{
	...
	infra_test_debug(" %s>>>> %d>>> ",__func__, __LINE__);
	len = strlen(data);
	infra_test_debug( " Data '%s' Recieved",data);
	infra_test_debug("\n Length of String is : \t %d ", len);

	for(i=0; i<len; i++){
		infra_test_debug("Checking %c \t", data[i]);
	...
	infra_test_debug("Checked data %s", data);
...
int check_for_integer(char *data, void *argument)
{
	...
	if((data[0] == '0') && ( data[1] == 'x' || data[1] == 'X') ){
		...
		infra_test_debug("\tChecking for hex");
		for(i=0; i<len; i++){
			switch(data[i]){
				case '0' ... '9':
				case  '\n' 	:
	...
	infra_test_debug( "\tConverted String ' %s ' to integer ' %d '" , data,
			integer_value);
...
EXPORT_SYMBOL(check_for_integer);
...
EXPORT_SYMBOL(get_integer_from_ascii);
