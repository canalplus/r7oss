...
static struct notifier_block clk_rate_change_nb;
...
static void gt_clockevent_update_freq(void *info)
{
	unsigned long *clk_rate = info;

	clockevents_update_freq(this_cpu_ptr(gt_evt), *clk_rate);
}

static int arm_global_timer_clockevent_cb(struct notifier_block *nb,
		unsigned long event, void *data)
{
	struct clk_notifier_data *ndata = data;

	if (event == POST_RATE_CHANGE) {
		gt_clk_rate = ndata->new_rate;
		/* Update clock events */
		on_each_cpu(gt_clockevent_update_freq, &gt_clk_rate, 1);
	}
