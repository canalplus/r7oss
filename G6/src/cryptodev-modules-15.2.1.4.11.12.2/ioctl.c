...
crypto_create_session(struct fcrypt *fcr, struct session_op *sop)
{
	...
	if (alg_name) {
		...
		if (ret < 0) {
			...
			/* Copy key data back to userspace, in case cipher has
			 * updated it (this can be the case with stm custom
			 * cipher) */
			if (unlikely(copy_to_user(sop->key, keyp, sop->keylen
							))) {
				ret = -EFAULT;
				goto error_cipher;
			}
