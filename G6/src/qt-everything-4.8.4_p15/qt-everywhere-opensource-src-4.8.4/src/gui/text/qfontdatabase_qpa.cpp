...
static void registerFont(QFontDatabasePrivate::ApplicationFont *fnt)
{
    ...
    for (int i = 0; i < fnt->families.size(); i ++) {
        if (!db->fallbackFamilies.contains(fnt->families.at(i))) {
            db->fallbackFamilies += fnt->families.at(i);
        }
    }
...
{
    return QApplicationPrivate::platformIntegration()->fontDatabase()->resolveFontFamilyAlias(family);
}
