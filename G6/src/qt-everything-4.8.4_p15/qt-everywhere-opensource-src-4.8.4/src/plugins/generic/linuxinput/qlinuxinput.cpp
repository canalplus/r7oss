    ...
    : m_notify(0), m_x(0), m_y(0), m_prevx(0), m_prevy(0), m_xoffset(0), m_yoffset(0), m_buttons(0), d(0)
{
    ...
    QString dev = QLatin1String("/dev/input/event2");
    m_compression = true;
    m_smooth = false;
    int jitterLimit = 0;
...
QLinuxInputKeyboardHandler::QLinuxInputKeyboardHandler(const QString &key, const QString &specification)
    : m_handler(0), m_fd(-1)
{
    qWarning() << "QLinuxInputKeyboardHandler::QLinuxInputKeyboardHandler() constructor" << key << specification;
    ...
    qWarning() << "Opening keyboard at" << dev;
    m_handler = new QLinuxInputKeycodeHandler(dev + (!keymap.isEmpty() ? (":keymap="+keymap) : "") );
...
void QLinuxInputKeyboardHandler::readKeycode()
{
    ...
    for (int i = 0; i < n; ++i) {
        ...
        QLinuxInputKeycodeHandler::KeycodeAction ka;
        ka = m_handler->processKeycode(code, value != 0, value == 2);

        switch (ka) {
        case QLinuxInputKeycodeHandler::CapsLockOn:
        case QLinuxInputKeycodeHandler::CapsLockOff:
            switchLed(LED_CAPSL, ka == QLinuxInputKeycodeHandler::CapsLockOn);
            break;

        case QLinuxInputKeycodeHandler::NumLockOn:
        case QLinuxInputKeycodeHandler::NumLockOff:
            switchLed(LED_NUML, ka == QLinuxInputKeycodeHandler::NumLockOn);
            break;

        case QLinuxInputKeycodeHandler::ScrollLockOn:
        case QLinuxInputKeycodeHandler::ScrollLockOff:
            switchLed(LED_SCROLLL, ka == QLinuxInputKeycodeHandler::ScrollLockOn);
            break;
