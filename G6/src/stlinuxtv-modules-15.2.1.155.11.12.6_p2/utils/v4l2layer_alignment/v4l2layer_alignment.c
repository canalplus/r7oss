...
{
    fprintf(stderr,"Usage: v4l2layer_alignment [options] layer1 layer2\n");
    fprintf(stderr,"\t-a,--alpha [0-255]       global alpha for layer2\n");
...
int main(int argc, char **argv)
{
  ...
  if((v4lfd1 = v4l2_open_by_name(V4L2_DISPLAY_DRIVER_NAME, V4L2_DISPLAY_CARD_NAME, O_RDWR)) < 0)
  {
  ...
  if (v4l2_set_output_by_name (v4lfd1, argv[optind]) == -1)
  {
    perror("Unable to set first output");
    goto exit;
  }

  if((v4lfd2 = v4l2_open_by_name(V4L2_DISPLAY_DRIVER_NAME, V4L2_DISPLAY_CARD_NAME, O_RDWR)) < 0)
  {
    perror("Unable to open video device");
    goto exit;
  }

  if (v4l2_set_output_by_name (v4lfd2, argv[optind+1]) == -1)
  {
    perror("Unable to set second output");
    goto exit;
  }
