...
#define AUDIO_CAPTURE_PCM 3
#warning Need to check this value
#elif defined(CONFIG_CPU_SUBTYPE_STX7105)  || defined(CONFIG_CPU_SUBTYPE_STX7106) || defined(CONFIG_CPU_SUBTYPE_STX7108) || defined(CONFIG_CPU_SUBTYPE_FLI7510)
#define AUDIO_CAPTURE_PCM 3
#warning Need to check this value
...
extern struct device *player_sysfs_get_class_device(void *playback,
						    void *stream);
extern void player_sysfs_new_attribute_notification(struct device *stream_dev);
extern int player_sysfs_get_stream_id(struct device *stream_dev);

static enum eFsRange TranslateIntegerSamplingFrequencyToRange(unsigned int
							      IntegerFrequency);
static enum eAccFsCode TranslateIntegerSamplingFrequencyToDiscrete(unsigned int
								   IntegerFrequency);
static unsigned int TranslateDiscreteSamplingFrequencyToInteger(enum eAccFsCode
								DiscreteFrequency);

static const char *LookupAudioMode(enum eAccAcMode DiscreteMode);
static struct snd_pseudo_mixer_channel_assignment
TranslateAudioModeToChannelAssignment(enum eAccAcMode AudioMode);
static enum eAccAcMode TranslateChannelAssignmentToAudioMode(struct
							     snd_pseudo_mixer_channel_assignment
							     ChannelAssignment);
...
static unsigned int FillOutDevicePcmParameters(avr_v4l2_audio_handle_t *
					       AudioContext,
...
static int
ProcessSpecializedTransformStatus(MME_LowlatencySpecializedTransformStatus_t *
				  TransformStatus);

static int InitLLTransformer(avr_v4l2_audio_handle_t * AudioContext);

static int CreateSysFsStreamAndAttributes(avr_v4l2_audio_handle_t *
					  AudioContext);

static int CheckPostMortemTransformerCapability(avr_v4l2_audio_handle_t *
						AudioContext);
...
static int CreateBufferPool(BufferPool_t * PoolPtr, BufferEntry_t * EntryPtr,
			    unsigned int NbElt, unsigned int EltSize);
...
static void DiscloseLatencyTargets(avr_v4l2_audio_handle_t * AudioContext,
				   const char *Why)
{
	avr_v4l2_shared_handle_t *SharedContext = AudioContext->SharedContext;

	DVB_TRACE
	    ("[%s] %dms latency with %lums Vsync compensation and [%d, %d, %d, %d] extra per output\n",
	     Why,
	     (int)SharedContext->target_latency +
	     AudioContext->MasterLatencyWhenInitialized,
...
static int InsertPrivateDataHeader(unsigned char *data, int payload_size,
...
static int InsertPesHeader(unsigned char *data, int size,
			   unsigned char stream_id, unsigned long long int pts,
			   int pic_start_code)
{
	BitPacker_t ld2 = { data, 0, 32 };
...
/*{{{  store_xxx_attribute*/
static ssize_t store_decode_errors(struct device *class_dev,
				   struct device_attribute *attr,
				   const char *buf, size_t count)
{
	avr_v4l2_audio_handle_t *AudioContext =
	    container_of(class_dev, avr_v4l2_audio_handle_t, StreamClassDevice);
...
static ssize_t show_input_format(struct device *class_dev,
				 struct device_attribute *attr, char *buf)
{
	...
	attr_size =
	    sprintf(buf, "%s",
		    TranslatePcToEncoding(AudioContext->
					  LLDecoderStatus.CurrentSpdifStatus.
					  PC));

#if SPDIFIN_API_VERSION >= 0x100319
	if ((AudioContext->LLDecoderStatus.CurrentSpdifStatus.PC ==
	     SPDIFIN_DTS1)
		...
		switch (AudioContext->LLDecoderStatus.
			CurrentSpdifStatus.Display & 0x3) {
		case DTS:
#ifndef DISABLE_BAD_X96_EXTENSION_REPORTING
			{
				/* firmware doesn't correctly report the presence of X96 for some
				 * stream types. however we can infer its presence from the reported
				 * sampling frequency.
				 */
				int sfreq =
				    TranslateDiscreteSamplingFrequencyToInteger((enum eAccFsCode)AudioContext->LLDecoderStatus.CurrentSpdifStatus.SamplingFreq);
				if (sfreq > 48000) {
					DVB_TRACE
					    ("Deploying DTS 96/24 reporting workaround (sfreq %d)\n",
					     sfreq);
					attr_size +=
					    sprintf(buf + attr_size, " 96/24");
					break;
				}
			}
#endif
			...
			case DTS_61_MATRIX:
				attr_size +=
				    sprintf(buf + attr_size, " ES 6.1 Matrix");
				break;
			case DTS_61_DISCRETE:
				attr_size +=
				    sprintf(buf + attr_size,
					    " ES 6.1 Discrete");
				break;
...
/* creation of show_xx methods */
static ssize_t show_decode_errors(struct device *class_dev,
				  struct device_attribute *attr, char *buf)
{
...
#define E(t, f, b, l, m) [SND_PSEUDO_MIXER_CHANNEL_PAIR_ ## t] = { f, b, l, m }
	E(DEFAULT, 0, 0, 0, 0),	// DEFAULT is a special case that must be handled in code
	    ...
	    E(DSTEREO_LsRs, 2, 0, 0, 0), E(NOT_CONNECTED, 0, 0, 0, 0),
#undef E
};

void count_speakers(struct snd_pseudo_mixer_channel_assignment
		    channel_assignment, int *pfront, int *pback, int *plfe,
		    int *pmisc)
{
	int i;
	...
	for (i = 0; i < 5; i++) {
		enum snd_pseudo_mixer_channel_pair pair =
		    0 == i ? channel_assignment.pair0 : 1 ==
		    i ? channel_assignment.pair1 : 2 ==
		    i ? channel_assignment.pair2 : 3 ==
		    i ? channel_assignment.pair3 : channel_assignment.pair4;

		if (SND_PSEUDO_MIXER_CHANNEL_PAIR_DEFAULT == pair) {
			// treat specially (interpreted differently based on position with a pair)
			switch (i) {
			case 0:
				front += 2;
				break;
...
void report_audio(tMMESpdifinStatus * spdifin_status, int *pfront, int *pback,
		  int *plfe, int *pmisc)
{
...
/* creation of show_xx methods */
static ssize_t show_number_channels(struct device *class_dev,
				    struct device_attribute *attr, char *buf)
{
	// get the private context 
	avr_v4l2_audio_handle_t *AudioContext =
	...
	report_audio(&AudioContext->LLDecoderStatus.CurrentSpdifStatus,
		     &firmware_front, &firmware_back, &firmware_lfe,
		     &firmware_misc);
	...
	if (nchan <= 0 || nchan > 8) {
		...
		ChannelAssignment =
		    TranslateAudioModeToChannelAssignment(AudioMode);
	...
	DVB_TRACE("%s (0x%02x) %d/%d+%d.%d -> %d/%d.%d\n",
		  ...
		  firmware_front, firmware_back, firmware_misc, firmware_lfe,
		  front, back + misc, lfe);
	return sprintf(buf, "%d/%d.%d\n", front, back + misc, lfe);
}

/* creation of show_xx methods */
static ssize_t show_sample_frequency(struct device *class_dev,
				     struct device_attribute *attr, char *buf)
{
	// get the private context 
	avr_v4l2_audio_handle_t *AudioContext =
...
#if SPDIFIN_API_VERSION >= 0x090623
	enum eAccFsCode fs =
	    (enum eAccFsCode)AudioContext->LLDecoderStatus.
	    CurrentSpdifStatus.SamplingFreq;
#else
	enum eAccFsCode fs =
	    (enum eAccFsCode)AudioContext->
	    LLDecoderStatus.CurrentDecoderFrequency;
#endif
...
static bool is_inconsistant(avr_v4l2_audio_handle_t * AudioContext)
{
...
#if SPDIFIN_API_VERSION >= 0x090623
	{
		int fs1 =
		    ...
		    (AudioContext->LLDecoderStatus.CurrentSpdifStatus.SamplingFreq);
		int fs2 =
		    TranslateDiscreteSamplingFrequencyToInteger
...
{
	MME_LxAudioDecoderHDInfo_t *Capability =
	    &AudioContext->AudioDecoderTransformCapability;

	switch (AudioContext->LLDecoderStatus.CurrentSpdifStatus.PC) {
	case SPDIFIN_AC3:
	case SPDIFIN_DDPLUS:
	case SPDIFIN_DTRUEHD:
		return ACC_DECODER_CAPABILITY_EXT_FLAGS(Capability,
							ACC_SPDIFIN) & (1 <<
									ACC_SPDIFIN_DD);
...
{
	int sfreq = TranslateDiscreteSamplingFrequencyToInteger((enum
								 eAccFsCode)
								AudioContext->LLDecoderStatus.CurrentDecoderFrequency);
...
/* creation of show_xx methods */
static ssize_t show_supported_input_format(struct device *class_dev,
					   struct device_attribute *attr,
					   char *buf)
{
	// get the private context 
...
/* creation of show_xx methods */
static ssize_t show_supported_input_format_in_game_mode(struct device
							*class_dev, struct device_attribute
							*attr, char *buf)
{
	// get the private context 
	avr_v4l2_audio_handle_t *AudioContext =
...
/* creation of show_xx methods */
static ssize_t show_number_of_samples_processed(struct device *class_dev,
						struct device_attribute *attr,
						char *buf)
{
	// get the private context 
...
static ssize_t store_emergency_mute(struct device *class_dev,
				    struct device_attribute *attr,
				    const char *buf, size_t count)
{
	avr_v4l2_audio_handle_t *AudioContext =
	...
	if (count >= 1) {
		...
		case 'n':
		case 'N':
			AvrAudioSetEmergencyMuteReason(AudioContext,
...
static ssize_t show_emergency_mute(struct device *class_dev,
				   struct device_attribute *attr, char *buf)
{
	// get the private context 
	avr_v4l2_audio_handle_t *AudioContext =
...
static ssize_t store_post_mortem(struct device *class_dev,
				 struct device_attribute *attr,
				 const char *buf, size_t count)
{
	avr_v4l2_audio_handle_t *AudioContext =
...
ssize_t show_post_mortem(struct device * class_dev,
			 struct device_attribute * attr, char *buf)
{
	avr_v4l2_audio_handle_t *AudioContext =
	    container_of(class_dev, avr_v4l2_audio_handle_t, StreamClassDevice);
	...
	if (!AudioContext->PostMortemAlreadyIssued) {
		if (0 != CheckPostMortemStatus(AudioContext)) {
			DVB_TRACE
			    ("Firmware issued post-mortem but drivers have not yet observed this\n");
...
static DEVICE_ATTR(input_format, S_IRUGO, show_input_format, NULL);
static DEVICE_ATTR(supported_input_format, S_IRUGO, show_supported_input_format,
		   NULL);
static DEVICE_ATTR(supported_input_format_in_game_mode, S_IRUGO,
		   show_supported_input_format_in_game_mode, NULL);
static DEVICE_ATTR(decode_errors, S_IWUSR | S_IRUGO, show_decode_errors,
		   store_decode_errors);
static DEVICE_ATTR(sample_frequency, S_IRUGO, show_sample_frequency, NULL);
static DEVICE_ATTR(number_channels, S_IRUGO, show_number_channels, NULL);
static DEVICE_ATTR(number_of_samples_processed, S_IRUGO,
		   show_number_of_samples_processed, NULL);
static DEVICE_ATTR(emergency_mute, S_IWUSR | S_IRUGO, show_emergency_mute,
		   store_emergency_mute);
static DEVICE_ATTR(post_mortem, S_IWUSR | S_IRUGO, show_post_mortem,
		   store_post_mortem);

static const struct device_attribute *attribute_array[] = {
	&dev_attr_input_format,
	&dev_attr_supported_input_format,
	&dev_attr_supported_input_format_in_game_mode,
	&dev_attr_decode_errors,
	&dev_attr_sample_frequency,
	&dev_attr_number_channels,
	&dev_attr_number_of_samples_processed,
	&dev_attr_emergency_mute,
	&dev_attr_post_mortem
};

static const int g_attrCount =
    sizeof(attribute_array) / sizeof(struct device_attribute *);
					  ...
					  AudioContext)
{
	...
	int result, attr_idx;
	struct device *stream_class_device = &AudioContext->StreamClassDevice;
	struct class *player2_class;
	stm_se_playback_h playerplayback;

	player2_class = player_sysfs_get_player_class();

	memset(stream_class_device, 0, sizeof(struct device));

	// retrieve the already created playback class device
	result =
	    DvbPlaybackGetPlayerEnvironment(AudioContext->
					    DeviceContext->Playback,
					    &playerplayback);
	if (result < 0) {
		DVB_ERROR("DvbPlaybackGetPlayerEnvironment failed\n");
		return -ENODEV;
	}

	AudioContext->PlaybackClassDevice =
	    player_sysfs_get_class_device(playerplayback, NULL);

	if (AudioContext->PlaybackClassDevice == NULL) {
		DVB_ERROR("Playback class device does not exist\n");
		return -ENODEV;
	}
#warning "TODO2.4 sort out initialisation of device."
	...
	dev_set_drvdata(stream_class_device, AudioContext);	/*stream_class_device->class_data = AudioContext; */
	// TODO modify this with audio0 in the future...
	/*snprintf(stream_class_device->class_id, BUS_ID_SIZE, "audio%d", 0); */

	result = device_register(stream_class_device);	/*result = class_device_register(stream_class_device); */
	...
	for (attr_idx = 0; attr_idx < g_attrCount; attr_idx++) {
		/* Create attribute */
		result =
		    device_create_file(&AudioContext->StreamClassDevice,
				       (struct device_attribute *)
				       attribute_array[attr_idx]);
		if (result) {
			DVB_ERROR
			    ("class_device_create_file failed (%d) - attr %d\n",
			     result, attr_idx);
			return -ENODEV;
		}
	}
...
static int AvrAudioSysfsCreateEmergencyMute(avr_v4l2_audio_handle_t *
					    AudioContext)
{
	int Result = 0;
	const struct device_attribute *attribute = NULL;
	stm_se_playback_h playerplayback = NULL;
	stm_se_play_stream_h playerstream = NULL;
	int streamid;

	attribute = &dev_attr_emergency_mute;

	Result =
	    DvbStreamGetPlayerEnvironment(AudioContext->
					  DeviceContext->AudioStream,
					  &playerplayback, &playerstream);
	if (Result < 0) {
		DVB_ERROR("DvbStreamGetPlayerEnvironment failed\n");
		return -1;
	}

	AudioContext->EmergencyMuteClassDevice =
	    player_sysfs_get_class_device(playerplayback, playerstream);
	if (AudioContext->EmergencyMuteClassDevice == NULL) {
		DVB_ERROR
		    ("get_class_device failed -> cannot create attribute \n");
		return -1;
	}

	streamid =
	    player_sysfs_get_stream_id(AudioContext->EmergencyMuteClassDevice);
	...
	Result =
	    device_create_file(AudioContext->EmergencyMuteClassDevice,
			       (struct device_attribute *)attribute);
	if (Result) {
		DVB_ERROR("Error in %s: device_create_file failed (%d)\n",
			  __FUNCTION__, Result);
		return -1;
	}
...
static int AvrAudioCalculateSampleRate(avr_v4l2_audio_handle_t * AudioContext,
				       snd_pcm_sframes_t initial_delay,
				       snd_pcm_sframes_t final_delay,
...
//    }
	...
	SampleRate =
	    (((unsigned long long)(AUDIO_PERIOD_FRAMES - initial_delay +
				   final_delay)) * 1000000ull) / (final_time -
								  initial_time);
	...
	if (AudioContext->PreviousSampleRate != SampleRate) {
		...
		if (AudioContext->EmergencyMuteReason ==
		...
		}
		{
			...
			MonitorSignalEvent
			    (MONITOR_EVENT_AUDIO_SAMPLING_RATE_CHANGE,
			     Parameters, "New sample rate detected");
		}
	}
...
static int AvrAudioSendPacketToPlayer(avr_v4l2_audio_handle_t * AudioContext,
				      ...
				      unsigned long long time_average)
{
	...
	AudioData =
	    capture_areas[0].addr + capture_areas[0].first / 8 +
	    capture_offset * capture_areas[0].step / 8;
	AudioDataSize =
	    capture_frames * (AUDIO_SAMPLE_DEPTH * AUDIO_CHANNELS / 8);

	// Calculate audioPts

	DelayInMicroSeconds =
	    (DelayInSamples * 1000000ull) / AudioContext->SampleRate;
	...
	Result =
	    avr_set_external_time_mapping(AudioContext->SharedContext,
					  AudioContext->
					  DeviceContext->AudioStream,
	...
	HeaderLength =
	    InsertPesHeader(PesHeader, AudioDataSize + 4, PES_PRIVATE_STREAM1,
			    audioPts, 0);
	HeaderLength +=
	    InsertPrivateDataHeader(&PesHeader[HeaderLength], AudioDataSize,
				    AudioContext->DiscreteSampleRate);
	Result =
	    DvbStreamInject(AudioContext->DeviceContext->AudioStream, PesHeader,
			    HeaderLength);
	if (Result < 0) {
	...
	Result =
	    DvbStreamInject(AudioContext->DeviceContext->AudioStream, AudioData,
			    AudioDataSize);
	if (Result < 0) {
...
static int AvrAudioInjectorThreadCleanup(avr_v4l2_audio_handle_t * AudioContext)
{
	...
	if (AudioContext->DeviceContext->AudioStream != NULL) {
		ret =
		    DvbStreamDrain(AudioContext->DeviceContext->AudioStream,
				   true);
		if (ret != 0) {
			DVB_ERROR("DvbStreamDrain failed\n");
			return -EINVAL;
		}

		ret =
		    dvb_playback_remove_stream(AudioContext->
					    DeviceContext->Playback,
					    AudioContext->
					    DeviceContext->AudioStream);
		if (ret != 0) {
			DVB_ERROR("dvb_playback_remove_stream failed\n");
			return -EINVAL;
		}
...
static int AvrAudioInjectorThread(void *data)
{
	...
	Result = dvb_playback_add_stream(Context->Playback,
				      STM_SE_MEDIA_AUDIO,
				      STM_SE_STREAM_ENCODING_AUDIO_SPDIFIN,
				      0,  /* EncodingFlags */
				      Context->DvbContext->DvbAdapter->num,
				      INVALID_MEDIA_ID,
				      Context->Id, &Context->AudioStream);
	if (Result < 0) {
		DVB_ERROR("dvb_playback_add_stream failed with %d\n", Result);
		goto err2;
	} else {
	...
	if (Result) {
		DVB_ERROR
		    ("Error in %s %d: create_emergency_mute_attribute failed\n",
	...
	Result =
	    DvbStreamSetOption(Context->AudioStream, DVB_OPTION_AV_SYNC, 1);
	if (Result < 0) {
		DVB_ERROR("DVB_OPTION_AV_SYNC set failed\n");
		goto err2;
	}
	// Audio capture init

	Result =
	    ksnd_pcm_open(&capture_handle, AUDIO_CAPTURE_CARD,
			  AUDIO_CAPTURE_PCM, SND_PCM_STREAM_CAPTURE);
	if (Result < 0) {
	...
	Result =
	    ksnd_pcm_set_params(capture_handle, AUDIO_CHANNELS,
				AUDIO_SAMPLE_DEPTH, AUDIO_DEFAULT_SAMPLE_RATE,
	...
	while (AudioContext->ThreadShouldStop == false) {
		...
		while (capture_frames < AUDIO_PERIOD_FRAMES) {
			...
			if (Result <= 0) {
				DVB_ERROR
				    ("Failed to wait for capture period expiry: %d)\n",
				     Result);
				...
				if (ret < 0)
					DVB_ERROR
					    ("dvp_deploy_emergency_mute failed\n");

				if (Result < 0) {
					Result =
					    AvrAudioXrunRecovery(capture_handle,
								 Result);
					if (Result < 0) {
						DVB_ERROR
						    ("Cannot recovery from xrun after waiting for period expiry\n");
						break;
					}
					ksnd_pcm_start(capture_handle);
				}
		...
		Result =
		    ksnd_pcm_mtimestamp(capture_handle, &SamplesAvailable,
					&time_stamp_as_timespec);
		if (Result < 0) {
		...
		time_average =
		    time_stamp_as_timespec.tv_sec * 1000000ll +
		    time_stamp_as_timespec.tv_nsec / 1000;
		DelayInSamples = SamplesAvailable;

		capture_frames = AUDIO_PERIOD_FRAMES;
		Result =
		    ksnd_pcm_mmap_begin(capture_handle, &capture_areas,
					&capture_offset, &capture_frames);
		if (Result < 0) {
		...
		if (packet_number == 1) {
			...
			Result =
			    AvrAudioCalculateSampleRate(AudioContext,
							DelayInSamples0,
							DelayInSamples1, t0,
							t1);
			if (Result == 0) {
				...
				Result =
				    AvrAudioSendPacketToPlayer(AudioContext,
							       capture_frames0,
							       capture_offset0,
							       capture_areas0,
							       DelayInSamples0,
				...
				if (Result < 0) {
					DVB_ERROR
					    ("dvp_send_audio_packet_to_player failed \n");
					goto err2;
				}
		...
		commited_frames =
		    ksnd_pcm_mmap_commit(capture_handle, capture_offset,
					 capture_frames);
		if (commited_frames < 0
		    || (snd_pcm_uframes_t) commited_frames != capture_frames) {
			//DVB_DEBUG("Capture XRUN! (commit %ld captured %ld)\n" , commited_frames,capture_frames);
			Result =
			    AvrAudioXrunRecovery(capture_handle,
						 commited_frames >=
						 0 ? -EPIPE : commited_frames);
			if (Result < 0) {
...
int AvrAudioLLThread(void *data)
{
	...
	if (CheckPostMortemTransformerCapability(AudioContext)) {
		DVB_ERROR
		    ("Cannot find post-mortem data - post-mortem trace is disabled\n");
	}
	...
	memset(&AudioContext->LLDecoderStatus, 0,
	       sizeof(AudioContext->LLDecoderStatus));
	...
	while (AudioContext->ThreadShouldStop == false) {
		...
		int result =
		    wait_event_interruptible_timeout(AudioContext->WaitQueue,
						     (AudioContext->ThreadShouldStop || AudioContext->UpdateMixerSettings || AudioContext->GotTransformCommandCallback || AudioContext->GotSetGlobalCommandCallback || AudioContext->GotSendBufferCommandCallback),
						     (HZ * 5));
		...
		if (result == 0) {
			if (CheckPostMortemStatus(AudioContext)) {
				DVB_ERROR
				    ("Transformer %s does not seem alive any more...\n",
				     LL_MT_NAME);
				IssuePostMortemDiagnostics(AudioContext);
				continue;
			}
		...
		if (AudioContext->ThreadShouldStop) {
			// cleanup thread
			DVB_TRACE
			    ("ThreadShouldStop is asserted - terminating\n");
			AvrAudioLLThreadCleanup(AudioContext);
			return 0;
		}
		...
		if (AudioContext->UpdateMixerSettings) {
			...
			case MME_COMMAND_STILL_EXECUTING:
				AudioContext->NeedAnotherSetGlobalCommand =
				    true;
				break;
			default:
				// unrecoverable errro
		...
		if (AudioContext->GotSendBufferCommandCallback) {
			...
			for (i = 0; i < AVR_LOW_LATENCY_MAX_TOTAL_SEND_BUFFERS;
			     i++) {
				BufferEntry_t *TheEntry =
				    AudioContext->BufferEntries[i];

				if (TheEntry && TheEntry->IsFree
				    && TheEntry->BytesUsed) {
					unsigned int relay_id;
					unsigned int stream_nb =
					    (i /
					     AVR_LOW_LATENCY_MAX_SIMULTANEOUS_BUFFERS);

					//DVB_DEBUG("relay %d bytes to the buffer %d\n", TheEntry->BytesUsed, i);
					// relay this buffer!
					if (stream_nb <
					    AVR_LOW_LATENCY_MAX_INPUT_CARDS) {
						relay_id =
					...
					AudioContext->
					    SendBufferCommands[i].DataBuffers_p
					    [0]->ScatterPages_p[0].BytesUsed =
					    0;
					...
					MMEStatus =
					    ...
					    (AudioContext->LLTransformerHandle,
					     &AudioContext->SendBufferCommands
					     [i]);

#if 0
					DVB_DEBUG
					    ("****Dumping MME comand %x,%d ****\n",
					     (unsigned int)
					     &AudioContext->SendBufferCommands
					     [i], i);
					DumpMMECommand
					...
					if (MMEStatus != MME_SUCCESS) {
						DVB_ERROR
						    ("Error: Call to MME_SendCommand %s (MME_SEND_BUFFER) returned %d\n",
						     LL_MT_NAME, MMEStatus);
					} else {
						TheEntry->IsFree = false;
						atomic_inc(&
							   (AudioContext->CommandsInFlight));
						DVB_TRACE
						    ("Issued command %08x (MME_SEND_BUFFERS) - %d in flight\n",
						     AudioContext->SendBufferCommands
						     [i].CmdStatus.CmdId,
						     atomic_read(&
								 (AudioContext->CommandsInFlight)));
		...
		if (AudioContext->GotTransformCommandCallback) {
			...
			bool CodecChange, MuteChange, FrequencyChange,
			    TopologyChange;

			AudioContext->GotTransformCommandCallback = false;

			if (AudioContext->TransformCommand.CmdStatus.Error !=
			    MME_SUCCESS) {
				DVB_ERROR
				    ("Firmware reports transform command failure. This is impossible!!!\n");
			}
			// re-arrange the transform status so we can use the strongly typed structures
			if (0 !=
			    ProcessSpecializedTransformStatus
			    (&AudioContext->TransformStatus))
				DVB_ERROR
				    ("Failed to extract firmware's status. Expect driver to make bad decisions.\n");

			//examine the status of the transform command
			DecoderStatus =
			    &AudioContext->TransformStatus.DecoderFrameStatus;
			PrimaryLimiterStatus =
			    &AudioContext->TransformStatus.LimiterStatus[0];
			LLDecoderStatus = &AudioContext->LLDecoderStatus;
			SpdifStatus =
			    (tMMESpdifinStatus *) &
			    DecoderStatus->FrameStatus[0];
			CurrentSpdifStatus =
			    &LLDecoderStatus->CurrentSpdifStatus;

#if SPDIFIN_API_VERSION >= 0x100415
			NewSilence =
			    (enum eMulticomSpdifinState)SpdifStatus->Silence;
			...
			if (DecoderStatus->DecStatus != MME_SUCCESS) {
				...
				LLDecoderStatus->NumberOfProcessedSamples +=
				    DecoderStatus->NbOutSamples;
			}

			up(&AudioContext->DecoderStatusSemaphore);

			CodecChange = (OldSilence != NewSilence)
			    || (OldState != NewState) || (OldPC != NewPC)
			    || (OldDisplay != NewDisplay);
			MuteChange = (OldMute.State == 2 && NewMute.State != 2);
			FrequencyChange =
			    (LLDecoderStatus->CurrentDecoderFrequency !=
			     DecoderStatus->DecSamplingFreq);
			TopologyChange =
			    (LLDecoderStatus->CurrentDecoderAudioMode !=
			     DecoderStatus->DecAudioMode);

			if (CodecChange || MuteChange || FrequencyChange
			    || TopologyChange) {
				down(&AudioContext->DecoderStatusSemaphore);
				...
				LLDecoderStatus->CurrentDecoderAudioMode =
				    DecoderStatus->DecAudioMode;
				memcpy(CurrentSpdifStatus, SpdifStatus,
				       sizeof(tMMESpdifinStatus));
				up(&AudioContext->DecoderStatusSemaphore);

				DVB_TRACE
				    ("LL thread: Firmware /%s%s%s%s change: /%s%s-%s/%d/%s/\n",
				     (CodecChange ? "Codec/" : ""),
				     (MuteChange ? "Mute/" : ""),
				...
				sysfs_notify(&AudioContext->
					     StreamClassDevice.kobj, NULL,
					     "input_format");

				// notify the format changes to sysfs
				if (CodecChange) {
					sysfs_notify
					    (&AudioContext->StreamClassDevice.
					     kobj, NULL,
				...
				if (MuteChange) {
					AvrAudioSetEmergencyMuteReason
					    (AudioContext,
					     (FrequencyChange ?
					      EMERGENCY_MUTE_REASON_SAMPLE_RATE_CHANGE
					      : EMERGENCY_MUTE_REASON_ERROR));
				...
				if (FrequencyChange) {
					sysfs_notify
					    (&AudioContext->StreamClassDevice.
					     kobj, NULL, "sample_frequency");
					if (!CodecChange)
						sysfs_notify
						    (&AudioContext->StreamClassDevice.kobj,
						     NULL,
						     "supported_input_format_in_game_mode");
				}
				if (TopologyChange) {
			...
			// notify the format changes to sysfs
			sysfs_notify(&AudioContext->StreamClassDevice.kobj,
				     NULL, "decode_errors");
			sysfs_notify(&AudioContext->StreamClassDevice.kobj,
				     NULL, "number_of_samples_processed");

			if (IsVeryFirstTransformCommand) {
				IsVeryFirstTransformCommand = false;
			} else if (LLDecoderStatus->CurrentDecoderFrequency !=
				   DecoderStatus->DecSamplingFreq) {
...
void SendLogBuffers(avr_v4l2_audio_handle_t * AudioContext)
{
	...
	if (ll_input_log_enable) {
		...
		for (i =
		     (AudioContext->AudioInputId *
		      AVR_LOW_LATENCY_MAX_SIMULTANEOUS_BUFFERS);
		     i < AVR_LOW_LATENCY_MAX_SIMULTANEOUS_BUFFERS; i++) {
			MMEStatus =
			    MME_SendCommand(AudioContext->LLTransformerHandle,
					    &AudioContext->SendBufferCommands
					    [i]);
			...
			if (MMEStatus != MME_SUCCESS) {
				...
				DVB_TRACE
				    ("Issued command %x (MME_SEND_BUFFERS) for input log - %d in flight\n",
				     AudioContext->
				     SendBufferCommands[i].CmdStatus.CmdId,
				     atomic_read(&
						 (AudioContext->CommandsInFlight)));
			}
		}
	...
	if (ll_output_log_enable) {
		...
		for (i = AVR_LOW_LATENCY_MAX_LOG_INPUT_SEND_BUFFERS;
		     i < AVR_LOW_LATENCY_MAX_TOTAL_SEND_BUFFERS; i++) {
			MMEStatus =
			    MME_SendCommand(AudioContext->LLTransformerHandle,
					    &AudioContext->SendBufferCommands
					    [i]);
			...
			if (MMEStatus != MME_SUCCESS) {
				...
				AudioContext->BufferEntries[i]->IsFree = false;
				DVB_TRACE
				    ("Issued command %x (MME_SEND_BUFFERS) for output log - %d in flight\n",
				     AudioContext->
				     SendBufferCommands[i].CmdStatus.CmdId,
				     atomic_read(&
						 (AudioContext->CommandsInFlight)));
...
static int CheckSinglePostMortemTransformerCapability(const char *Name, unsigned long
						      *PhysicalAddress,
						      ...
						      Buffer)
{
	...
	*Buffer = NULL;
	...
	if (Info.StructSize != sizeof(Info)
	    || 0 == Info.PostMortemPhysicalAddress) {
	...
	if (Capability.Version != DRV_MULTICOM_PERFLOG_VERSION) {
		DVB_TRACE
		    ("Warning: Version skew (0x%x vs 0x%x) detected in post-mortem trace on %s\n",
	...
	*Buffer = ioremap(Info.PostMortemPhysicalAddress, sizeof(**Buffer));
	if (!*Buffer) {
		DVB_TRACE
		    ("Found post-mortem buffer for %s but could not map for IO\n",
		     Name);
		return -EINVAL;
	}
...
static int CheckPostMortemStatus(avr_v4l2_audio_handle_t * AudioContext)
{
	...
	if (AudioContext->SecondaryPostMortemBuffer
	    && AudioContext->SecondaryPostMortemBuffer->Status !=
	    POSTMORTEM_RUNNING)
		return -ETIMEDOUT;
...
static void PostMortemReportOutstandingCommand(avr_v4l2_audio_handle_t *
					       AudioContext,
					       MME_Command_t * Cmd)
{
	...
	if (MME_COMMAND_COMPLETED != State && MME_COMMAND_FAILED != State)
		printk("  0x%08x (%s)\n",
		       Cmd->CmdStatus.CmdId,
		       LookupMmeCommandCode(Cmd->CmdCode));
}

static void IssueSinglePostMortemDiagnostics(MME_TimeLogPostMortem_t * pm,
					     const char *name)
{
	int i;
	...
	if (pm->StructSize != sizeof(MME_TimeLogPostMortem_t) ||
		...
		printk
		    ("WARNING: Version skew detected - contemplate report with great care\n");

	printk("Identity: %s\n", name);
	printk("Status:   %s (%d)\n", LookupPostMortemStatus(pm->Status),
	       pm->Status);
	printk("Message:  %s%s", pm->Message,
	       (pm->Message[strlen(pm->Message) - 1] == '\n' ? "" : "\n"));
	printk("Release:  %s\n", pm->FirmwareRelease);
	printk("Built by: %s@%s at %s %s\n", pm->UserName, pm->BuildMachine,
	       pm->CompileTime, pm->CompileDate);
		...
		for (i = 1; i < 60; i += 4)
			printk
			    ("R%-2d  0x%08x    R%-2d  0x%08x    R%-2d  0x%08x    R%-2d  0x%08x\n",
			     i, pm->Regs[i], i + 1, pm->Regs[i + 1], i + 2,
			     pm->Regs[i + 2], i + 3, pm->Regs[i + 3]);
		printk("R61  0x%08x    R62  0x%08x    R63  0x%08x\n\n",
		       pm->Regs[61], pm->Regs[62], pm->Regs[63]);
		printk
		    ("BR0  0x%08x    BR1  0x%08x    BR2  0x%08x    BR3  0x%08x\n",
		     (pm->BranchBits & 0x01 ? 1 : 0),
		     (pm->BranchBits & 0x02 ? 1 : 0),
		     (pm->BranchBits & 0x04 ? 1 : 0),
		     (pm->BranchBits & 0x08 ? 1 : 0));
		printk
		    ("BR4  0x%08x    BR5  0x%08x    BR6  0x%08x    BR7  0x%08x\n",
		     (pm->BranchBits & 0x10 ? 1 : 0),
		     (pm->BranchBits & 0x20 ? 1 : 0),
		     (pm->BranchBits & 0x40 ? 1 : 0),
		     (pm->BranchBits & 0x80 ? 1 : 0));

		printk("\nCall trace:\n");
		if (pm->BackTrace[0]) {
			printk("  0x%08x\n  0x%08x\n", pm->PC, pm->LINK);
			for (i = 0;
			     i < lengthof(pm->BackTrace) && pm->BackTrace[i];
			     i++)
				printk("  0x%08x\n", pm->BackTrace[i]);
		} else {
			printk
			    ("  No call trace available (?bad stack pointer?)\n");
		}
	}
}
...
static void IssuePostMortemDiagnostics(avr_v4l2_audio_handle_t * AudioContext)
{
	...
	if (!pm[0]
	    || 0 != test_and_set_bit(0, &AudioContext->PostMortemAlreadyIssued)) {
	...
	printk
	    ("\nFound %d crashed processors and killed %d more (guru meditation #%x/%x/%x)\n",
	     hweight8(dead_mask), hweight8(kill_mask), dead_mask, kill_mask,
	     trap_mask);
	...
	for (i = 0; i < AVR_LOW_LATENCY_MAX_TOTAL_SEND_BUFFERS; i++)
		PostMortemReportOutstandingCommand(AudioContext,
						   &AudioContext->SendBufferCommands
						   [i]);
	PostMortemReportOutstandingCommand(AudioContext,
...
static int CheckLLTransformerCapability(avr_v4l2_audio_handle_t * AudioContext)
{
	...
	DVB_DEBUG("%s capabilities:%s%s%s%s\n", LL_MT_NAME,
		  (ACC_DECODER_CAPABILITY_EXT_FLAGS(LLInfo, ACC_SPDIFIN) &
		   (1 << ACC_SPDIFIN_DD) ? " DD" : ""),
		  (ACC_DECODER_CAPABILITY_EXT_FLAGS(LLInfo, ACC_SPDIFIN) &
		   (1 << ACC_SPDIFIN_DTS) ? " DTS" : ""),
		  (ACC_DECODER_CAPABILITY_EXT_FLAGS(LLInfo, ACC_SPDIFIN) &
		   (1 << ACC_SPDIFIN_MPG) ? " MP2AAC" : ""),
		  (ACC_DECODER_CAPABILITY_EXT_FLAGS(LLInfo, ACC_SPDIFIN) &
		   (1 << ACC_SPDIFIN_WMA) ? " WMA" : ""));
...
static int GetMasterLatencyOffset(avr_v4l2_audio_handle_t * AudioContext)
{
	...
	if (SND_PSEUDO_MIXER_MAGIC == SharedContext->mixer_settings.magic) {
		int master_latency =
		    SharedContext->mixer_settings.master_latency;
...
static int InitLLTransformer(avr_v4l2_audio_handle_t * AudioContext)
{
	...
	memset(&AudioContext->SetGlobalParams, 0,
	       sizeof(MME_LowlatencySpecializedGlobalParams_t));
	...
	InitParams.TransformerInitParamsSize =
	    sizeof(MME_LowlatencySpecializedInitParams_t);
	...
	LLInitParams.InitParams.Features.Latency =
	    SharedContext->target_latency + MasterLatency;

#ifdef LOWLATENCY_API_EPOCH
#if LOWLATENCY_API_VERSION >= 0x100430
	LLInitParams.InitParams.Features.SilenceThresh =
	    AudioContext->SilenceThreshold;
	LLInitParams.InitParams.Features.SilencePeriod =
	    AudioContext->SilenceDuration;
#endif
#endif
	...
	if (0 !=
	    SetGlobalTransformParameters(AudioContext,
					 &LLInitParams.GlobalInitParams)) {
		DVB_ERROR
		    ("Could not set global transformer parameters (resource conflict?)\n");
		return -EINVAL;
	}
...
{
	avr_v4l2_audio_handle_t *AudioContext =
	    (avr_v4l2_audio_handle_t *) UserData;

	atomic_dec(&(AudioContext->CommandsInFlight));
	DVB_TRACE("Event %d for CmdId %08x (%s) - %d in flight\n",
		  Event, CallbackData->CmdStatus.CmdId,
		  LookupMmeCommandCode(CallbackData->CmdCode),
		  atomic_read(&(AudioContext->CommandsInFlight)));
		...
		case MME_SEND_BUFFERS:
			{
				...
				MME_DataBuffer_t *DataBuffer =
				    CallbackData->DataBuffers_p[0];
				unsigned int Size =
				    DataBuffer->ScatterPages_p[0].BytesUsed;
				int BufferIndex = (int)DataBuffer->UserData_p;

				TheEntry =
				    AudioContext->BufferEntries[BufferIndex];
				...
				if (TheEntry) {
					...
					if (Size) {
						...
						AudioContext->GotSendBufferCommandCallback
						    = true;
						wake_up
						    (&AudioContext->WaitQueue);
					} else {
						DVB_ERROR
						    ("Strange: Got a SendBuffer callback but buffer (%d) is empty\n",
						     BufferIndex);
					}
				} else {
					DVB_ERROR
					    ("Strange: Got a SendBuffer callback but buffer has a wrong index (%d - %x)\n",
					     BufferIndex,
	...
	default:
		...
		atomic_inc(&(AudioContext->CommandsInFlight));
		DVB_ERROR
		    ("Low latency MMe callback: Unexpected MME event (%d)\n",
		     Event);
		break;
	}
	return;
...
static int LaunchLLSetGlobalCommand(avr_v4l2_audio_handle_t * AudioContext)
{
	...
	if (AudioContext->SetGlobalCommand.CmdStatus.State !=
	    MME_COMMAND_COMPLETED
	    && AudioContext->SetGlobalCommand.CmdStatus.State !=
	    MME_COMMAND_FAILED) {
		DVB_TRACE
		    ("Refusing to reuse SetGlobalCommand structure (CmdId %08x still pending)\n",
	...
	if (MMEStatus != MME_SUCCESS) {
		...
		atomic_inc(&(AudioContext->CommandsInFlight));
		DVB_TRACE
		    ("Issued command %x (MME_SET_GLOBAL_TRANSFORM_PARAMS) - %d in flight\n",
		     AudioContext->SetGlobalCommand.CmdStatus.CmdId,
		     atomic_read(&(AudioContext->CommandsInFlight)));
	}
...
static int LaunchLLTransformCommand(avr_v4l2_audio_handle_t * AudioContext)
{
	...
	if (AudioContext->TransformCommand.CmdStatus.State !=
	    MME_COMMAND_COMPLETED
	    && AudioContext->TransformCommand.CmdStatus.State !=
	    MME_COMMAND_FAILED) {
					...
					* GlobalParams)
{
	...
	SpdifInFlags.Bits.HDMIAudioMode =
	    (ll_force_amode) ? HDMI_AMODE(ll_force_amode) :
	    AudioContext->HdmiAudioMode;
#endif

#if SPDIFIN_API_VERSION >= 0x091005
	SpdifInFlags.Bits.DisableDetection = (ll_disable_detection
					      || ll_bitexactness_testing);
	SpdifInFlags.Bits.DisableFade = (ll_disable_spdifinfade
					 || ll_bitexactness_testing);
#endif
	...
	if (SND_PSEUDO_MIXER_MAGIC == SharedContext->mixer_settings.magic) {
		...
		if (mixer_settings->drc_enable) {
			...
			SpdifInConfig->DecConfig[DD_COMPRESS_MODE] =
			    mixer_settings->drc_type;
			SpdifInConfig->DecConfig[DD_HDR] = mixer_settings->hdr;
			SpdifInConfig->DecConfig[DD_LDR] = mixer_settings->ldr;
		}

		for (inp_idx = 0; inp_idx < AVR_LOW_LATENCY_MAX_INPUT_CARDS;
		     inp_idx++) {
...
#if DRV_MULTICOM_AUDIO_DECODER_VERSION >= 0x090128
			mch_enable = ((AudioContext->HdmiLayout != HDMI_LAYOUT0)
				      && (NbChannels[inp_idx] ==
					  AUDIO_CHANNELS_HDMI));
#endif
			IoCfg->Params.NbChannels =
			    (mch_enable) ? AUDIO_CHANNELS_HDMI : AUDIO_CHANNELS;
			IoCfg->Params.WordSize = ACC_WS32;
			IoCfg->Params.AudioMode =
			    (mch_enable) ? AudioContext->HdmiAudioMode :
			    ACC_MODE_ID;
			...
			mch_enabled |=
			    (inp_idx ==
			     AudioContext->AudioInputId) ? mch_enable : 0;

			// mark the first input as connected
			IoCfg->Connect.Connect =
			    (inp_idx ==
			     AudioContext->AudioInputId) ? IO_CONNECT :
			    IO_DISCONNECT;
...
#endif
			DVB_DEBUG
			    ("Input card %sconnected card hw:%d,%d (source %d)\n",
			     IoCfg->Connect.Connect == IO_CONNECT ? "" : "dis",
			     IoCfg->ID.CardId, IoCfg->ID.DevId,
			     IoCfg->Connect.Source);
			IoCfg++;
		}
		...
		for (out_idx = 0; out_idx < ActualOutputCards; out_idx++) {
			...
			if (6 != strlen(alsaname)
			    || 0 != strncmp(alsaname, "hw:", 3)
			    || alsaname[3] < '0' || alsaname[4] != ','
			    || alsaname[5] < '0') {
				CardId = 0;
				DevId = 0;
				...
				DVB_ERROR
				    ("Cannot find '%s', try 'hw:X,Y' instead\n",
				     alsaname);
			} else {
				CardId = alsaname[3] - '0';
				DevId = alsaname[5] - '0';
			...
			IoCfg->Params.SamplingFreq =
			    TranslateIntegerSamplingFrequencyToDiscrete
			    (Card->max_freq);
			...
			if (AudioContext->SoundcardHandle[out_idx] != NULL) {
				ksnd_pcm_close(AudioContext->SoundcardHandle
					       [out_idx]);
				AudioContext->SoundcardHandle[out_idx] = NULL;
			}

			if (0 !=
			    ksnd_pcm_open(&AudioContext->SoundcardHandle
					  [out_idx], CardId, DevId,
				...
				DVB_DEBUG
				    ("Configured output card %s (idx %d) DevId %d CardId %d, NbChan %d SamplingFreq %d Connected %d\n",
				     alsaname, out_idx, IoCfg->ID.DevId,
				     IoCfg->ID.CardId, IoCfg->Params.NbChannels,
				     IoCfg->Params.SamplingFreq,
				     IoCfg->Connect.Connect);
			}
		...
		for (out_idx = 0;
		     out_idx <
		     (AVR_LOW_LATENCY_MAX_OUTPUT_CARDS - ActualOutputCards);
		     out_idx++) {
			// unconnected outputs...
			IoCfg->ID.MTId = ACC_LOWLATENCY_MT;
			IoCfg->ID.ExtId = 0;
			...
			AudioContext->SoundcardHandle[out_idx +
						      ActualOutputCards] = NULL;

			DVB_DEBUG
			    ("Configured output card (idx %d) DevId %d CardId %d, NbChan %d SamplingFreq %d Connected %d\n",
			     out_idx, IoCfg->ID.DevId, IoCfg->ID.CardId,
			     IoCfg->Params.NbChannels,
			     IoCfg->Params.SamplingFreq,
			     IoCfg->Connect.Connect);
			IoCfg++;
		...
		{
			unsigned int IoCfgSize =
			    sizeof(MME_LowLatencyIO_t) *
			    (AVR_LOW_LATENCY_MAX_INPUT_CARDS +
			     AVR_LOW_LATENCY_MAX_OUTPUT_CARDS);
			MME_LLChainPcmProcessingGlobalParams_t
			    * PcmProcessingsCfg =
			    (MME_LLChainPcmProcessingGlobalParams_t
			     *) ((unsigned int)IoCfg +
				 sizeof(MME_LLPcmProcessingGlobalParams_t));
			MME_LLPcmProcessingGlobalParams_t *PcmParams =
			    (MME_LLPcmProcessingGlobalParams_t *) IoCfg;
			...
			for (n = 0; n < ActualOutputCards; n++) {
				PcmParamsSize +=
				    FillOutDevicePcmParameters(AudioContext,
							       &PcmProcessingsCfg
							       [n], n,
							       &OutputEncoding,
							       (AudioContext->EmergencyMuteReason
								!=
								EMERGENCY_MUTE_REASON_NONE));
		...
		DVB_ERROR
		    ("No valid mixer settings... Set Global will be simple...!\n");
	}

	// Set SpdifInFlags  according to the connected IOs.
#ifdef(SPDIFIN_API_VERSION) && (SPDIFIN_API_VERSION >= 0x090228)
	SpdifInFlags.Bits.Layout = (mch_enabled)
	    ? (((ll_bitexactness_testing)
		&& (AudioContext->HdmiLayout !=
		    HDMI_LAYOUT0)) ? HDMI_LAYOUT1 : AudioContext->HdmiLayout) :
	    HDMI_LAYOUT0;
#else
#if DRV_MULTICOM_AUDIO_DECODER_VERSION >= 0x090128
	SpdifInFlags.Bits.NbChannels =
	    (mch_enabled) ? AudioContext->HdmiLayout : HDMI_LAYOUT0;
...
//XXX( ACC_MODE53_LFE,          ...                                                             ), // Not 8ch
	    ...
	    E(ACC_MODE20, L_R, NOT_CONNECTED, NOT_CONNECTED, NOT_CONNECTED), DUPE(ACC_HDMI_MODE20, L_R, NOT_CONNECTED, NOT_CONNECTED, NOT_CONNECTED), E(ACC_MODE20_LFE, L_R, 0 _LFE1, NOT_CONNECTED, NOT_CONNECTED), DUPE(ACC_HDMI_MODE20_LFE, L_R, 0 _LFE1, NOT_CONNECTED, NOT_CONNECTED), E(ACC_MODE30, L_R, CNTR_0, NOT_CONNECTED, NOT_CONNECTED), DUPE(ACC_HDMI_MODE30, L_R, CNTR_0, NOT_CONNECTED, NOT_CONNECTED), E(ACC_MODE30_LFE, L_R, CNTR_LFE1, NOT_CONNECTED, NOT_CONNECTED), DUPE(ACC_HDMI_MODE30_LFE, L_R, CNTR_LFE1, NOT_CONNECTED, NOT_CONNECTED), E(ACC_MODE21, L_R, NOT_CONNECTED, CSURR_0, NOT_CONNECTED), DUPE(ACC_HDMI_MODE21, L_R, NOT_CONNECTED, CSURR_0, NOT_CONNECTED), E(ACC_MODE21_LFE, L_R, 0 _LFE1, CSURR_0, NOT_CONNECTED), DUPE(ACC_HDMI_MODE21_LFE, L_R, 0 _LFE1, CSURR_0, NOT_CONNECTED), E(ACC_MODE31, L_R, CNTR_0, CSURR_0, NOT_CONNECTED), DUPE(ACC_HDMI_MODE31, L_R, CNTR_0, CSURR_0, NOT_CONNECTED), E(ACC_MODE31_LFE, L_R, CNTR_LFE1, CSURR_0, NOT_CONNECTED), DUPE(ACC_HDMI_MODE31_LFE, L_R, CNTR_LFE1, CSURR_0, NOT_CONNECTED), E(ACC_MODE22, L_R, NOT_CONNECTED, LSUR_RSUR, NOT_CONNECTED), DUPE(ACC_HDMI_MODE22, L_R, NOT_CONNECTED, LSUR_RSUR, NOT_CONNECTED), E(ACC_MODE22_LFE, L_R, 0 _LFE1, LSUR_RSUR, NOT_CONNECTED), DUPE(ACC_HDMI_MODE22_LFE, L_R, 0 _LFE1, LSUR_RSUR, NOT_CONNECTED), E(ACC_MODE32, L_R, CNTR_0, LSUR_RSUR, NOT_CONNECTED), DUPE(ACC_HDMI_MODE32, L_R, CNTR_0, LSUR_RSUR, NOT_CONNECTED), E(ACC_MODE32_LFE, L_R, CNTR_LFE1, LSUR_RSUR, NOT_CONNECTED), DUPE(ACC_HDMI_MODE32_LFE, L_R, CNTR_LFE1, LSUR_RSUR, NOT_CONNECTED), XXX(ACC_MODE23, L_R, NOT_CONNECTED, LSUR_RSUR, CSURR_0),	// Bug 4518
	    DUPE(ACC_HDMI_MODE23, L_R, NOT_CONNECTED, LSUR_RSUR, CSURR_0), XXX(ACC_MODE23_LFE, L_R, 0 _LFE1, LSUR_RSUR, CSURR_0),	// Bug 4518
	    DUPE(ACC_HDMI_MODE23_LFE, L_R, 0 _LFE1, LSUR_RSUR, CSURR_0), E(ACC_MODE33, L_R, CNTR_0, LSUR_RSUR, CSURR_0),	// Bug 4518
	    DUPE(ACC_HDMI_MODE33, L_R, CNTR_0, LSUR_RSUR, CSURR_0), E(ACC_MODE33_LFE, L_R, CNTR_LFE1, LSUR_RSUR, CSURR_0),	// Bug 4518
	    DUPE(ACC_HDMI_MODE33_LFE, L_R, CNTR_LFE1, LSUR_RSUR, CSURR_0), XXX(ACC_MODE24, L_R, NOT_CONNECTED, LSUR_RSUR, LSURREAR_RSURREAR),	//Bug 4518
	    DUPE(ACC_HDMI_MODE24, L_R, NOT_CONNECTED, LSUR_RSUR, LSURREAR_RSURREAR), XXX(ACC_MODE24_LFE, L_R, 0 _LFE1, LSUR_RSUR, LSURREAR_RSURREAR),	//Bug 4518
	    DUPE(ACC_HDMI_MODE24_LFE, L_R, 0 _LFE1, LSUR_RSUR, LSURREAR_RSURREAR), E(ACC_MODE34, L_R, CNTR_0, LSUR_RSUR, LSURREAR_RSURREAR), DUPE(ACC_HDMI_MODE34, L_R, CNTR_0, LSUR_RSUR, LSURREAR_RSURREAR), E(ACC_MODE34_LFE, L_R, CNTR_LFE1, LSUR_RSUR, LSURREAR_RSURREAR), DUPE(ACC_HDMI_MODE34_LFE, L_R, CNTR_LFE1, LSUR_RSUR, LSURREAR_RSURREAR), XXX(ACC_HDMI_MODE40, L_R, NOT_CONNECTED, NOT_CONNECTED, CNTRL_CNTRR),	// Bug 4518
	    XXX(ACC_HDMI_MODE40_LFE, L_R, 0 _LFE1, NOT_CONNECTED, CNTRL_CNTRR),	// Bug 4518
	    ...
	    XXX(ACC_HDMI_MODE41_LFE, L_R, 0 _LFE1, CSURR_0, CNTRL_CNTRR),	// Bug 4518
	    ...
	    DUPE(ACC_HDMI_MODE42, L_R, NOT_CONNECTED, LSUR_RSUR, CNTRL_CNTRR), XXX(ACC_MODE42_LFE, L_R, 0 _LFE1, LSUR_RSUR, CNTRL_CNTRR),	// Bug 4518
	    DUPE(ACC_HDMI_MODE42_LFE, L_R, 0 _LFE1, LSUR_RSUR, CNTRL_CNTRR), XXX(ACC_MODE52, L_R, CNTR_0, LSUR_RSUR, CNTRL_CNTRR),	// Bug 4518
	    DUPE(ACC_HDMI_MODE52, L_R, CNTR_0, LSUR_RSUR, CNTRL_CNTRR), XXX(ACC_MODE52_LFE, L_R, CNTR_LFE1, LSUR_RSUR, CNTRL_CNTRR),	// Bug 4518
	    ...
	    XXX(ACC_HDMI_MODE22_T200_LFE, L_R, 0 _LFE1, LSUR_RSUR, LHIGH_RHIGH),	// Bug 4518
	    XXX(ACC_HDMI_MODE42_WIDE, L_R, NOT_CONNECTED, LSUR_RSUR, LWIDE_RWIDE),	// Bug 4518
	    XXX(ACC_HDMI_MODE42_WIDE_LFE, L_R, 0 _LFE1, LSUR_RSUR, LWIDE_RWIDE),	// Bug 4518
	    ...
	    XXX(ACC_HDMI_MODE33_T100, L_R, CNTR_0, LSUR_RSUR, CSURR_CHIGH),	// Bug 4518
	    ...
	    XXX(ACC_MODE30_T200_LFE, L_R, CNTR_LFE1, NOT_CONNECTED,
		LHIGH_RHIGH), XXX(ACC_MODE22_T010, L_R, NOT_CONNECTED,
				  LSUR_RSUR, TOPSUR_0), XXX(ACC_MODE22_T010_LFE,
							    L_R, 0 _LFE1,
							    LSUR_RSUR,
							    TOPSUR_0),
	    XXX(ACC_MODE32_T020, L_R, CNTR_0, LSUR_RSUR, LHIGHSIDE_RHIGHSIDE),
	    XXX(ACC_MODE32_T020_LFE, L_R, CNTR_LFE1, LSUR_RSUR,
		LHIGHSIDE_RHIGHSIDE), XXX(ACC_MODE23_T100, L_R, NOT_CONNECTED,
					  LSUR_RSUR, CHIGH_0),
	    XXX(ACC_MODE23_T100_LFE, L_R, 0 _LFE1, LSUR_RSUR, CHIGH_0),
	    XXX(ACC_MODE23_T010, L_R, NOT_CONNECTED, LSUR_RSUR, TOPSUR_0),
	    XXX(ACC_MODE23_T010_LFE, L_R, 0 _LFE1, LSUR_RSUR, TOPSUR_0),
#endif
...
//XXX( ACC_MODE32_LFE,          L_R,            CNTR_LFE1,      LSUR_RSUR,      LSIDESURR_RSIDESURR ), No enum
	    // delimiter
	    E(ACC_MODE_ID, NOT_CONNECTED, NOT_CONNECTED, NOT_CONNECTED,
	      NOT_CONNECTED)
#undef E
#undef XXX
};
...
const char *LookupAudioMode(enum eAccAcMode DiscreteMode)
{
	...
	for (i = 0; ACC_MODE_ID != ChannelAssignmentLookupTable[i].AccAcMode;
	     i++)
...
///
enum eAccAcMode TranslateChannelAssignmentToAudioMode(struct
						      snd_pseudo_mixer_channel_assignment
						      ChannelAssignment)
{
	int pair, i;
	...
	for (pair = 3; pair >= 0; pair--) {
		for (i = 0;
		     ACC_MODE_ID != ChannelAssignmentLookupTable[i].AccAcMode;
		     i++)
			if (0 ==
			    memcmp(&ChannelAssignmentLookupTable
				   [i].ChannelAssignment, &ChannelAssignment,
				   sizeof(ChannelAssignment))
			    &&
			    ChannelAssignmentLookupTable
			    [i].SuitableForDirectOutput)
				return
				    ChannelAssignmentLookupTable[i].AccAcMode;
		...
		if (pair != 0) {
			...
			DVB_TRACE
			    ("Cannot find matching audio mode - disconnecting pair %d\n",
			     pair);
		}
	}

	DVB_TRACE
	    ("Cannot find matching audio mode - falling back to ACC_MODE20t (Lt/Rt stereo)\n");
	return ACC_MODE20t;
}
...
TranslateAudioModeToChannelAssignment(enum eAccAcMode AudioMode)
{
	const struct snd_pseudo_mixer_channel_assignment Malleable =
	    {.malleable = 1 };
	...
	for (i = 0; ACC_MODE_ID != ChannelAssignmentLookupTable[i].AccAcMode;
		...
		if (ChannelAssignmentLookupTable[i].AccAcMode == AudioMode) {
			return
			    ChannelAssignmentLookupTable[i].ChannelAssignment;
		}
	}
...
/// a seperate method.
///
static int FillOutDeviceDownmixParameters(avr_v4l2_audio_handle_t *
					  AudioContext,
					  MME_LLChainPcmProcessingGlobalParams_t
					  * PcmParams, int dev_num)
{
	...
	int struct_size = 0;
	const struct snd_pseudo_mixer_downmix_rom *DownmixFirmware =
	    AudioContext->SharedContext->downmix_firmware;
	enum eAccAcMode OutputMode =
	    (enum eAccAcMode)PcmParams->CMC.Config[CMC_OUTMODE_MAIN];
	enum eAccAcMode InputMode =
	    AudioContext->LLDecoderStatus.CurrentDecoderAudioMode;

	bool EnableDMix = (InputMode != OutputMode)
	    && (!ll_bitexactness_testing);
	...
	if (DownmixFirmware) {
		...
		if (ACC_MODE_1p1 == InputMode) {
			if (AudioContext->ChannelSelect ==
			    V4L2_AVR_AUDIO_CHANNEL_SELECT_STEREO)
		...
		TargetOutputId.ca =
		    TranslateAudioModeToChannelAssignment(OutputMode);
		TargetInputId.ca =
		    TranslateAudioModeToChannelAssignment(EffectiveInputMode);
		TargetSortValue =
		    ((uint64_t) TargetInputId.n << 32) + TargetOutputId.n;
		...
		while (low < high) {
			...
			CurrentSortValue =
			    ((uint64_t) InputId.n << 32) + OutputId.n;
		...
		if (TargetOutputId.ca.malleable && OutputId.ca.malleable) {
			OutputMode =
			    TranslateChannelAssignmentToAudioMode(OutputId.ca);
			DVB_TRACE
			    ("Downmix firmware requested %s as malleable output surface\n",
		...
		if (CurrentSortValue == TargetSortValue) {
			const struct snd_pseudo_mixer_downmix_index *index =
			    DownmixFirmware->index + low;
			const snd_pseudo_mixer_downmix_Q15 *data =
			    (snd_pseudo_mixer_downmix_Q15 *)
			    (DownmixFirmware->index +
			     DownmixFirmware->header.num_index_entries);
			const snd_pseudo_mixer_downmix_Q15 *table =
			    data + index->offset;
			int x, y, i;
			...
			for (x = 0; x < index->output_dimension; x++)
				for (y = 0; y < index->input_dimension; y++)
					DMix->MainMixTable[x][y] =
					    table[x * index->input_dimension +
						  y];
			...
			if (ACC_MODE_1p1 == InputMode &&
				...
				for (x = 0; x < index->output_dimension; x++) {
					...
					int active_ch =
					    (AudioContext->ChannelSelect ==
					     V4L2_AVR_AUDIO_CHANNEL_SELECT_MONO_LEFT
					     ? MAIN_LEFT : MAIN_RIGHT);

					DMix->MainMixTable[x][active_ch] =
					    DMix->MainMixTable[x][MAIN_CNTR];
					DMix->MainMixTable[x][MAIN_CNTR] = 0;
				}
			}

			DVB_DEBUG("Found custom downmix table for %s to %s\n",
				  LookupAudioMode(EffectiveInputMode),
				  LookupAudioMode(OutputMode));
			for (i = 0; i < DMIX_NB_IN_CHANNELS; i++)
				DVB_DEBUG
				    ("%04x %04x %04x %04x %04x %04x %04x %04x\n",
				     DMix->MainMixTable[i][0],
				     DMix->MainMixTable[i][1],
				     DMix->MainMixTable[i][2],
				     DMix->MainMixTable[i][3],
				     DMix->MainMixTable[i][4],
				     DMix->MainMixTable[i][5],
				     DMix->MainMixTable[i][6],
				     DMix->MainMixTable[i][7]);
		} else {
			// not an error but certainly worthy of note
			DVB_TRACE
			    ("Downmix firmware has no entry for %s to %s - using defaults\n",
			     LookupAudioMode(EffectiveInputMode),
			     LookupAudioMode(OutputMode));
		}
	}
					...
					OutputEncoding_t OutputEncoding)
{
	...
	if (OutputEncoding == OUTPUT_FATPIPE) {
		MME_FatpipeGlobalParams_t *FatPipe =
		    &PcmParams->FatPipeOrSpdifOut;
		...
		FatPipe->Config.FEC =
		    ((mixer_settings->fatpipe_metadata.md[0] >> 6) & 1);
		FatPipe->Config.Encryption =
		    ((mixer_settings->fatpipe_metadata.md[0] >> 5) & 1);
		FatPipe->Config.InvalidStream =
		    ((mixer_settings->fatpipe_metadata.md[0] >> 4) & 1);
		FatPipe->MetaData.LevelShiftValue =
		    mixer_settings->fatpipe_metadata.md[6] & 31;
		FatPipe->MetaData.Lfe_Gain =
		    mixer_settings->fatpipe_metadata.md[2] & 31;
#if PCMPROCESSINGS_API_VERSION >= 0x100415
		FatPipe->MetaData.Effects =
		    mixer_settings->fatpipe_metadata.md[14] & 0xffff;
#endif

		// the firmware will manage non-userspace metadata internally (i.e. get them from the decoder itself).

		memset(FatPipe->Spdifout.Validity, 0xff,
		       sizeof(FatPipe->Spdifout.Validity));
...
#define _B(x) (1 << ((x) % 32))
		...
		const unsigned int additional_format_information =
		    B(3) | B(4) | B(5);
		...
		const unsigned int sampling_frequency =
		    B(24) | B(25) | B(26) | B(27);
...
#undef _B
		...
		for (i = 0; i < STSZ; i++)
			ChannelStatusMask[i] =
			    mixer_settings->iec958_mask.status[i * 4 +
							       0] << 24 |
			    mixer_settings->iec958_mask.status[i * 4 +
							       1] << 16 |
			    mixer_settings->iec958_mask.status[i * 4 +
		...
		ChannelStatusMask[0] &= ~(use_of_channel_status_block | linear_pcm_identification | additional_format_information |	/* auto fill in for PCM, 000 for coded data */
					  mode | sampling_frequency);	/* auto fill in */
		...
		for (i = 0; i < STSZ; i++)
			SpdifOut->Spdifout.ChannelStatus[i] |=
			    ChannelStatusMask[i] &
			    (mixer_settings->iec958_metadata.status[i * 4 +
								    0] << 24 |
			     mixer_settings->iec958_metadata.status[i * 4 +
								    1] << 16 |
			     mixer_settings->iec958_metadata.status[i * 4 +
					...
					bool DeployEmergencyMute)
{
...

	{
		...
		if (mixer_settings->bass_mgt_bypass) {
			...
			BassMgt->Apply =
			    (ll_bitexactness_testing) ? ACC_MME_DISABLED :
			    ACC_MME_ENABLED;
			...
			memset(BassMgt->Volume, BASSMGT_ATTENUATION_MUTE,
			       sizeof(BassMgt->Volume));
			for (i = 0; i < SND_PSEUDO_MIXER_CHANNELS; i++)
				BassMgt->Volume[i] =
				    -mixer_settings->master_volume[i];
			...
			DVB_DEBUG("BassMgt[%d]:  %d  %d  %d  %d  %d  %d\n",
				  dev_num, BassMgt->Volume[0],
				  BassMgt->Volume[1], BassMgt->Volume[2],
				  BassMgt->Volume[3], BassMgt->Volume[4],
				  BassMgt->Volume[5]);
		}
	}

	{
		...
		DCRemove->Apply =
		    (mixer_settings->dc_remove_enable ? ACC_MME_ENABLED :
		     ACC_MME_DISABLED);
	}

	{
		MME_Resamplex2GlobalParams_t *Resamplex2 =
		    &PcmParams->Resamplex2;
		Resamplex2->Id =
		    PCMPROCESS_SET_ID(ACC_PCM_RESAMPLE_ID, dev_num);
		Resamplex2->StructSize = sizeof(*Resamplex2);
		struct_size += sizeof(*Resamplex2);

		Resamplex2->Apply =
		    (ll_force_downsample) ? ACC_MME_ENABLED : ACC_MME_AUTO;
		...
		if (*OutputEncoding == OUTPUT_FATPIPE) {
			...
			Resamplex2->Apply =
			    ...
			    Resamplex2->Apply;
			Resamplex2->Range =
			    TranslateIntegerSamplingFrequencyToRange
			    (mixer_settings->downstream_topology.
			     card[dev_num].max_freq);
		}
	}

	{
		...
		CMC->Config[CMC_DUAL_MODE] =
		    AudioContext->ChannelSelect ==
		    V4L2_AVR_AUDIO_CHANNEL_SELECT_MONO_LEFT ? ACC_DUAL_LEFT_MONO
		    : AudioContext->ChannelSelect ==
		    V4L2_AVR_AUDIO_CHANNEL_SELECT_MONO_RIGHT ?
		    ACC_DUAL_RIGHT_MONO : ACC_DUAL_LR;
	...
	struct_size +=
	    FillOutDeviceDownmixParameters(AudioContext, PcmParams, dev_num);

	//

	struct_size +=
	    FillOutDeviceSpdifParameters(AudioContext, PcmParams,
					 mixer_settings, dev_num,
					 *OutputEncoding);
...

	{
		...
		if ((*OutputEncoding == OUTPUT_FATPIPE)
		    || (mixer_settings->chain_limiter_enable[dev_num] == false))
		...
		if (!mixer_settings->chain_enable[dev_num] ||
		    (DeployEmergencyMute &&
		     ...
		     mixer_settings->emergency_mute)) {
			// We deploy the emergency mute only if the route is pre or post mix
			Limiter->SoftMute = 1;

#ifdef PCMPROCESSINGS_API_VERSION && (PCMPROCESSINGS_API_VERSION >= 0x090301)
			...
			Limiter->EmergencyMute.bits.AutoUnmute = SND_PSEUDO_MIXER_EMERGENCY_MUTE_NO_MUTE == mixer_settings->emergency_mute ? ACC_MME_TRUE : ACC_MME_FALSE;	// Let the FW unmute by itself if it triggers emergency mute
			Limiter->EmergencyMute.bits.MuteId = 0;
#endif
		}
...
#if DRV_MULTICOM_AUDIO_DECODER_VERSION >= 0x090128
		AudioContext->LLDecoderStatus.CurrentMuteStatus.State =
		    Limiter->EmergencyMute.bits.State;
#endif
		...
		Limiter->Delay = mixer_settings->chain_latency[dev_num] + AudioContext->CompensatoryLatency;	//in ms

		DVB_DEBUG("Limiter[%d]  Gain %d  Delay %d\n", dev_num,
			  Limiter->Gain, Limiter->Delay);
	}
	...
	if (AudioContext->PostMortemForce) {
		...
		PcmParams->CMC.Id =
		    PCMPROCESS_SET_ID(ACC_PCM_DEATH_ID, dev_num);
...
///
int ProcessSpecializedTransformStatus(MME_LowlatencySpecializedTransformStatus_t
				      * TransformStatus)
{
	...
	NewStatus.BytesUsed =
	    sizeof(NewStatus) - sizeof(NewStatus.DecoderFrameStatus);

	// Examine each item in turn and copy into the correct part of NewStatus
	while (BytesRemaining > 0 && Template->StructSize > 0) {
		DVB_DEBUG
		    ("Template %p  BytesRemaining %d  Id %08x  StructSize %d\n",
		     Template, BytesRemaining, Template->Id,
		     Template->StructSize);
		switch (Template->Id) {
...
#define PCMPROCESS_GET_ID(id) ((id >> 8) - ACC_PCMPROCESS_MAIN_MT)
		...
		case PCMPROCESS_SET_ID(ACC_PCM_CMC_ID, 3):
			if (sizeof(MME_PcmProcessingOutputChainStatus_t) ==
			    Template->StructSize) {
				DVB_DEBUG("Got CMC reply for chain %d\n",
					  PCMPROCESS_GET_ID(Template->Id));
				memcpy(&
				       (NewStatus.OutputChainStatus
					[PCMPROCESS_GET_ID(Template->Id)]),
				       Template,
				       sizeof
				...
				DVB_ERROR
				    ("Bad MME_PcmProcessingOutputChainStatus_t size "
		...
		case PCMPROCESS_SET_ID(ACC_PCM_LIMITER_ID, 3):
			if (sizeof(MME_LimiterStatus_t) == Template->StructSize) {
				DVB_DEBUG("Got limiter reply for chain %d\n",
					  PCMPROCESS_GET_ID(Template->Id));
				memcpy(&
				       (NewStatus.LimiterStatus
					[PCMPROCESS_GET_ID(Template->Id)]),
				       Template, sizeof(MME_LimiterStatus_t));
			} else {
				DVB_ERROR
				    ("Bad MME_LimiterStatus_t size (firmware version skew?)\n");
				Result = -1;
			}
			break;
		...
		default:
			DVB_ERROR
			    ("Unknown structure in reply - Id %08x, Size %d\n",
...
static void AvrAudioLLConditionalAbort(avr_v4l2_audio_handle_t * AudioContext,
				       MME_Command_t * Cmd)
{
	MME_CommandState_t State = Cmd->CmdStatus.State;
	MME_ERROR Err;
	...
	if (MME_COMMAND_COMPLETED != State && MME_COMMAND_FAILED != State) {
		...
		Err =
		    MME_AbortCommand(AudioContext->LLTransformerHandle,
				     Cmd->CmdStatus.CmdId);
		if (MME_SUCCESS != Err)
...
static int AvrAudioLLThreadCleanup(avr_v4l2_audio_handle_t * AudioContext)
{
	...
	if (!AudioContext->ThreadShouldStop) {
		...
		DVB_TRACE
		    ("Audio thread terminated early - waiting for shutdown request\n");
		wait_event(AudioContext->WaitQueue,
			   AudioContext->ThreadShouldStop);
	...
	if (AudioContext->IsLLTransformerInitialized) {
		do {
			...
			for (i = 0; i < AVR_LOW_LATENCY_MAX_TOTAL_SEND_BUFFERS;
			     i++)
				AvrAudioLLConditionalAbort(AudioContext,
							   &AudioContext->SendBufferCommands
							   [i]);
			...
			if (MMEStatus == MME_SUCCESS) {
				DVB_DEBUG
				    ("Successful terminated transformer %s\n",
				     LL_MT_NAME);
				break;
			}

			DVB_DEBUG
			    ("Waiting to terminate transformer %s (MME_TermTransformer returned %d)\n",
			     LL_MT_NAME, MMEStatus);
	...
	if (retries <= 0) {
		DVB_ERROR
		    ("Timed out after ~%dms whilst trying to terminate transformer %s\n",
...
/// for that frequency.
///
enum eFsRange TranslateIntegerSamplingFrequencyToRange(unsigned int
						       IntegerFrequency)
{
...
/// for that frequency.
///
enum eAccFsCode TranslateIntegerSamplingFrequencyToDiscrete(unsigned int
							    IntegerFrequency)
{
	int i;

	for (i = 0; IntegerFrequency < SamplingFrequencyLookupTable[i].Integer; i++) ;	// do nothing
								...
								DiscreteFrequency)
{
	...
	for (i = 0;
	     DiscreteFrequency != SamplingFrequencyLookupTable[i].Discrete; i++)
...
void FillOutLLSetGlobalCommand(avr_v4l2_audio_handle_t * AudioContext)
{
	...
	Command_p->CmdStatus.AdditionalInfoSize =
	    sizeof(MME_LowlatencySpecializedTransformStatus_t);
...
void FillOutLLTransformCommand(avr_v4l2_audio_handle_t * AudioContext)
{
	...
	MME_LowlatencyTransformParams_t *TransformParams_p =
	    &AudioContext->TransformParams;
...
int InitLLLogBuffers(avr_v4l2_audio_handle_t * AudioContext)
{
	...
	if (ll_input_log_enable) {
		...
		if (CreateBufferPool(&AudioContext->InputPool.Pool,
			...
			DVB_ERROR
			    ("Unable to allocate input buffers for logging\n");
			return 1;
		}

		for (i = 0; i < AVR_LOW_LATENCY_MAX_LOG_INPUT_SEND_BUFFERS; i++) {
			AudioContext->BufferEntries[i] =
			    &AudioContext->InputPool.Entries[i];
		}
	}
	...
	if (ll_output_log_enable) {
		if (CreateBufferPool(&AudioContext->OutputPool.Pool,
			...
			DVB_ERROR
			    ("Unable to allocate output buffers for logging\n");
			return 1;
		}

		for (i = 0; i < AVR_LOW_LATENCY_MAX_LOG_OUTPUT_SEND_BUFFERS;
		     i++) {
			AudioContext->BufferEntries[i +
						    AVR_LOW_LATENCY_MAX_LOG_INPUT_SEND_BUFFERS]
			    = &AudioContext->OutputPool.Entries[i];
		}
	}
	...
	if (ll_input_log_enable) {
		...
		for (i = 0; i < AVR_LOW_LATENCY_MAX_LOG_INPUT_SEND_BUFFERS; i++) {
			AudioContext->DataBufferList[i] =
			    &AudioContext->DataBuffers[i];
			...
			DataBufferPtr[0]->TotalSize =
			    AudioContext->BufferEntries[i]->Size;
			DataBufferPtr[0]->StartOffset = 0;

			ScatterPagePtr->Page_p =
			    AudioContext->BufferEntries[i]->Ptr;
			ScatterPagePtr->Size =
			    AudioContext->BufferEntries[i]->Size;
	...
	if (ll_output_log_enable) {
		// Pre-set the send buffer commands
		for (i = AVR_LOW_LATENCY_MAX_LOG_INPUT_SEND_BUFFERS;
		     i < AVR_LOW_LATENCY_MAX_LOG_OUTPUT_SEND_BUFFERS; i++) {
			AudioContext->DataBufferList[i] =
			    &AudioContext->DataBuffers[i];
			CmdPtr->StructSize = sizeof(MME_Command_t);
			CmdPtr->CmdCode = MME_SEND_BUFFERS;
...
int CreateBufferPool(BufferPool_t * PoolPtr, BufferEntry_t * EntryPtr,
		     unsigned int NbElt, unsigned int EltSize)
{
	unsigned int i;
	unsigned int size = NbElt * EltSize;

	PoolPtr->PhysPtr = OSDEV_MallocPartitioned("aud-avr-debuglog", size);
	...
	PoolPtr->CachedPtr =
	    ioremap_cache((unsigned int)PoolPtr->PhysPtr, size);
...
avr_v4l2_audio_handle_t *AvrAudioNew(avr_v4l2_shared_handle_t * SharedContext)
{
	...
	if (AudioContext) {
		...
		AudioContext->DeviceContext =
		    &SharedContext->avr_device_context;
		...
		AudioContext->DiscreteSampleRate =
		    AVR_AUDIO_DISCRETE_SAMPLE_RATE_48000;
		...
		AudioContext->SetGlobalCommand.CmdStatus.State =
		    MME_COMMAND_FAILED;
		for (i = 0; i < AVR_LOW_LATENCY_MAX_TOTAL_SEND_BUFFERS; i++)
			AudioContext->SendBufferCommands[i].CmdStatus.State =
			    MME_COMMAND_FAILED;
...
int AvrAudioDelete(avr_v4l2_audio_handle_t * AudioContext)
{
	...
	if (ll_audio_enable) {
		if (ll_input_log_enable && AudioContext->InputPool.Pool.PhysPtr) {
			...
			if (OSDEV_NoError !=
			    OSDEV_FreePartitioned("aud-avr-debuglog",
						  AudioContext->InputPool.
						  Pool.PhysPtr)) {
				DVB_ERROR
				    ("Error while trying to free the input pool buffer\n");
			}
		}

		if (ll_output_log_enable
		    && AudioContext->OutputPool.Pool.PhysPtr) {
			if (OSDEV_NoError !=
			    OSDEV_FreePartitioned("aud-avr-debuglog",
						  AudioContext->OutputPool.
						  Pool.PhysPtr)) {
				DVB_ERROR
				    ("Error while trying to free the output pool buffer\n");
			}
		}
	}
...
static void AvrAudioLLAggressiveApply(avr_v4l2_audio_handle_t * AudioContext)
{
	if (AudioContext->ThreadHandle) {
		...
		if (0 != res)
			DVB_ERROR
			    ("Internal error - cannot toggle the overlay\n");
	}
}
...
void AvrAudioSetAacDecodeEnable(avr_v4l2_audio_handle_t * AudioContext,
				bool Enabled)
{
...
void AvrAudioSetCompensatoryLatency(avr_v4l2_audio_handle_t * AudioContext,
				    long long MicroSeconds)
{
	if (AudioContext) {
		if (MicroSeconds < 0 || MicroSeconds > 100000) {
			DVB_ERROR
			    ("Ignoring invalid compensatory latency (%lld)\n",
			     MicroSeconds);
			AudioContext->CompensatoryLatency = 0;
		} else {
			// convert to milliseconds and rounded *up* (to prevent audio leading video)
			AudioContext->CompensatoryLatency =
			    (MicroSeconds + 999) / 1000;
		}
...
AvrAudioGetEmergencyMuteReason(avr_v4l2_audio_handle_t * AudioContext)
{
	if (!AudioContext) {
		return -EINVAL;
	}
...
int AvrAudioSetEmergencyMuteReason(avr_v4l2_audio_handle_t * AudioContext,
				   avr_audio_emergency_mute_reason_t reason)
{
	int ret = 0;
	if (!ll_audio_enable) {

		if (!AudioContext->DeviceContext->AudioStream)
			return -EINVAL;

		if (AudioContext->EmergencyMuteReason !=
		    EMERGENCY_MUTE_REASON_NONE) {
			ret =
			    DvbStreamEnable(AudioContext->
					    DeviceContext->AudioStream, reason);
			if (ret < 0) {
				DVB_ERROR("DvbStreamEnable failed\n");
				return -EINVAL;
			}
		}
		...
		sysfs_notify(&
			     ((*(AudioContext->EmergencyMuteClassDevice)).kobj),
			     NULL, "emergency_mute");
	} else {
...
void AvrAudioSetFormatRecogniserEnable(avr_v4l2_audio_handle_t * AudioContext,
				       bool Enabled)
{
...
void AvrAudioSetInput(avr_v4l2_audio_handle_t * AudioContext,
		      unsigned int InputId)
{
	...
	// so for now we are using the aggressively applying the change.
#if 0
	AvrAudioLLInstantApply(AudioContext);
#else
	AvrAudioLLAggressiveApply(AudioContext);
...
void AvrAudioSetHdmiAudioMode(avr_v4l2_audio_handle_t * AudioContext,
			      int audio_mode)
{
...
AvrAudioGetChannelSelect(avr_v4l2_audio_handle_t * AudioContext)
{
	return AudioContext->ChannelSelect;
}

void AvrAudioSetSilenceThreshold(avr_v4l2_audio_handle_t * AudioContext,
				 int Threshold)
{
...
void AvrAudioSetSilenceDuration(avr_v4l2_audio_handle_t * AudioContext,
				int Duration)
{
...
int AvrAudioIoctlOverlayStart(avr_v4l2_audio_handle_t * AudioContext)
{
	...
	AudioContext->ThreadHandle =
	    kthread_create(ThreadFn, AudioContext, ThreadName);
	...
	if (sched_setscheduler
	    (AudioContext->ThreadHandle, SCHED_RR, &Param_audio)) {
		DVB_ERROR
		    ("FAILED to set scheduling parameters to priority %d (SCHED_RR)\n",
...
int AvrAudioIoctlOverlayStop(avr_v4l2_audio_handle_t * AudioContext)
{
	...
	if (AudioContext->ThreadHandle) {
		...
		res =
		    wait_event_timeout(AudioContext->WaitQueue,
				       !AudioContext->ThreadShouldStop,
				       (HZ * ll_audioterm_timeout));

		elapsed = ktime_sub(ktime_get(), elapsed);

		if (AudioContext->ThreadShouldStop) {
			DVB_ERROR
			    ("%s after ~%dms waiting for audio thread to terminate\n",
			     0 == res ? "Timed out" : "Error whilst",
			     (int)ktime_to_almost_ms(elapsed));
			IssuePostMortemDiagnostics(AudioContext);
			return -ETIMEDOUT;
		}
		DVB_DEBUG
		    ("Stop completed OK after ~%dms: res %d  ThreadShouldstop %d\n",
		     (int)ktime_to_almost_ms(elapsed), res,
		     AudioContext->ThreadShouldStop);
	...
	if (AudioContext->IsSysFsStructureCreated) {
		...
		for (attr_idx = 0; attr_idx < g_attrCount; attr_idx++) {
			device_remove_file(&AudioContext->StreamClassDevice,
					   (struct device_attribute *)
					   attribute_array[attr_idx]);
		}
		...
		device_del(&AudioContext->StreamClassDevice);
...
void AvrAudioMixerSettingsChanged(avr_v4l2_audio_handle_t * AudioContext)
{
	if (AudioContext) {
		...
		if (AudioContext->MasterLatencyWhenInitialized !=
		    GetMasterLatencyOffset(AudioContext))
			AvrAudioLLAggressiveApply(AudioContext);
		else
			AvrAudioLLInstantApply(AudioContext);
	}
...
void DumpMMECommand(MME_Command_t * CmdInfo_p)
{
	       ...
	       (unsigned int)CmdInfo_p->DataBuffers_p);
	{
		...
		for (buf_idx = 0;
		     buf_idx <
		     (CmdInfo_p->NumberInputBuffers +
		      CmdInfo_p->NumberOutputBuffers); buf_idx++) {
			int NbScatterPages =
			    DataBuffer->NumberOfScatterPages, j;
			MME_ScatterPage_t *Scatter_p =
			    DataBuffer->ScatterPages_p;
