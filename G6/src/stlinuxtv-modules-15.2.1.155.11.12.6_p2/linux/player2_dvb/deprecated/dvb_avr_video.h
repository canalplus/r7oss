...
#define DVP_ANCILLARY_PARTITION			"vid-raw-input"
...
typedef struct {
	...
	uint32_t ActiveAreaXStart;
	uint32_t FullVBIHeight;
//  uint32_t           OutputStandards;
//  bool               SquarePixel;
//  uint32_t           HDMIVideoCodes[HDMI_CODE_COUNT];
...
//  uint32_t          LinesByFrame;
//  uint32_t          ulPixelClock;
...
typedef struct dvp_v4l2_video_handle_s {
	...
	struct device *DvpSysfsClassDevice;
	...
	unsigned long long
	 DvpElapsedCaptureTimeRecord[DVP_MAX_RECORDED_INTEGRATIONS];
