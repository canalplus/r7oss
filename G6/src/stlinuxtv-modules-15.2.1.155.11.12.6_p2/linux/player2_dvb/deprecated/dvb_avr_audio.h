...
typedef struct dvp_v4l2_audio_handle_s {
	...
	struct device *EmergencyMuteClassDevice;
	...
	struct device *PlaybackClassDevice;	// pointer to a structure created by the sysfs_player
	...
	MME_Command_t TransformCommand, SetGlobalCommand,
	    SendBufferCommands[AVR_LOW_LATENCY_MAX_TOTAL_SEND_BUFFERS];
	MME_LowlatencySpecializedGlobalParams_t SetGlobalParams;
	MME_LowlatencyTransformParams_t TransformParams;
	MME_LowlatencySpecializedTransformStatus_t TransformStatus,
	    SetGlobalStatus;

	MME_DataBuffer_t DataBuffers[AVR_LOW_LATENCY_MAX_TOTAL_SEND_BUFFERS];
	 MME_DataBuffer_t
	    * DataBufferList[AVR_LOW_LATENCY_MAX_TOTAL_SEND_BUFFERS];
	...
	struct {
		BufferPool_t Pool;
		 BufferEntry_t
		    Entries[AVR_LOW_LATENCY_MAX_LOG_INPUT_SEND_BUFFERS];
		//        bool          IsInitalized;
	} InputPool;
	...
	struct {
		BufferPool_t Pool;
		 BufferEntry_t
		    Entries[AVR_LOW_LATENCY_MAX_LOG_OUTPUT_SEND_BUFFERS];
		//        bool          IsInitalized;
	} OutputPool;
...
long long AvrAudioGetCompensatoryLatency(avr_v4l2_audio_handle_t *
					 AudioContext);
...
int AvrAudioSetEmergencyMuteReason(avr_v4l2_audio_handle_t * Context,
...
void AvrAudioSetSilenceDuration(avr_v4l2_audio_handle_t * AudioContext,
				int Duration);
unsigned long AvrAudioGetSilenceDuration(avr_v4l2_audio_handle_t *
					 AudioContext);
