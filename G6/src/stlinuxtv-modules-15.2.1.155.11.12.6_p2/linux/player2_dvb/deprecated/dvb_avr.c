...
extern struct class_device *player_sysfs_get_class_device(void *playback,
							  void *stream);
...
int avr_set_external_time_mapping(avr_v4l2_shared_handle_t * shared_context,
				  struct DvbStreamContext_s *stream,
				  unsigned long long nativetime,
				  unsigned long long systemtime)
{
	int result = 0;
	struct DvbPlaybackContext_s *playback =
	    shared_context->avr_device_context.Playback;
	...
	result =
	    DvbStreamSetOption(stream, DVB_OPTION_EXTERNAL_TIME_MAPPING, 1);
	if (result < 0) {
		DVB_ERROR("DVB_OPTION_EXTERNAL_TIME_MAPPING set failed\n");
		return -EINVAL;
	}
	//
	...
	result =
	    DvbPlaybackSetNativePlaybackTime(playback, nativetime, systemtime);
	if (result < 0) {
		DVB_ERROR("DvbPlaybackSetNativePlaybackTime failed with %d\n",
			  result);
		return -EINVAL;
	}
 ...
 */
void avr_invalidate_external_time_mapping(avr_v4l2_shared_handle_t *
					  shared_context)
{
 ...
 */
static void avr_update_mixer_settings(void *ctx, const struct snd_pseudo_mixer_settings
				      *mixer_settings)
{
	...
	if (memcmp
	    (&shared_context->mixer_settings, mixer_settings,
	     sizeof(shared_context->mixer_settings))) {
...
void avr_set_vsync_offset(avr_v4l2_shared_handle_t * shared_context,
			  long long vsync_offset)
{
	AvrAudioSetCompensatoryLatency(shared_context->audio_context,
				       vsync_offset);
}
				   ...
				   struct stm_v4l2_handles *handle)
{
	...
	if (shared_context->avr_device_context.Playback != NULL) {
		DVB_ERROR
		    ("Playback (and therefore the overlay) is already started.");
		return 0;
	}
	...
	if (shared_context->avr_device_context.Playback == NULL) {
		ret =
		    DvbPlaybackCreate(shared_context->
				      avr_device_context.SyncContext->Id,
				      &shared_context->
				      avr_device_context.Playback);
		if (ret < 0) {
			DVB_ERROR("DvbPlaybackCreate failed\n");
			return -EINVAL;
		}
	}
	...
	ret = DvbPlaybackSetOption(shared_context->avr_device_context.Playback,
				   ...
				   DVB_OPTION_VALUE_SYSTEM_CLOCK_MASTER);
	if (ret < 0) {
		DVB_ERROR
		    ("DVB_OPTION_VALUE_SYSTEM_CLOCK_MASTER coult not be set\n");
		return -EINVAL;
	}
	// Wait for the playback's creational event to be acted upon
	ret =
	    DvbPlaybackGetPlayerEnvironment(shared_context->
					    avr_device_context.Playback,
					    &playerplayback);
	if (ret < 0) {
	...
	while (NULL == playerclassdev) {
		DVB_TRACE
		    ("Spinning waiting for creational event to be delivered\n");
	...
	if (shared_context->avr_device_context.AudioCaptureStatus ==
	    DVP_CAPTURE_ON) {
		if (handle->v4l2type[STM_V4L2_AUDIO_INPUT].driver
		    && handle->v4l2type[STM_V4L2_AUDIO_INPUT].handle) {
			ret =
			    AvrAudioIoctlOverlayStart(handle->v4l2type
						      [STM_V4L2_AUDIO_INPUT].handle);
			if (0 != ret) {
	...
	if (shared_context->avr_device_context.VideoCaptureStatus ==
	    DVP_CAPTURE_ON) {
		if (handle->v4l2type[STM_V4L2_VIDEO_INPUT].driver
		    && handle->v4l2type[STM_V4L2_VIDEO_INPUT].handle) {
			ret =
			    DvpVideoIoctlOverlayStart(handle->v4l2type
						      [STM_V4L2_VIDEO_INPUT].handle);
			if (ret != 0) {
				  ...
				  struct stm_v4l2_handles *handle)
{
	...
	if (handle->v4l2type[STM_V4L2_AUDIO_INPUT].handle)
		AvrAudioIoctlOverlayStop(handle->
					 v4l2type[STM_V4L2_AUDIO_INPUT].handle);

	if (handle->v4l2type[STM_V4L2_VIDEO_INPUT].handle) {
		ret =
		    DvpVideoClose(handle->
				  v4l2type[STM_V4L2_VIDEO_INPUT].handle);
		if (ret < 0) {
	...
	if (ret < 0) {
		DVB_ERROR("DvbPlaybackDelete failed\n");
		return -EINVAL;
	}
...
static int avr_ioctl_video_set_input(avr_v4l2_shared_handle_t * shared_context,
				     struct stm_v4l2_handles *handle, int id)
{
	int ret = 0;
	...
	if (handle->v4l2type[STM_V4L2_VIDEO_INPUT].handle == NULL) {
		...
		video_context =
		    kzalloc(sizeof(dvp_v4l2_video_handle_t), GFP_KERNEL);
		...
		video_context->DeviceContext =
		    &shared_context->avr_device_context;
		video_context->DvpRegs = shared_context->mapped_dvp_registers;
		video_context->DvpIrq = shared_context->dvp_irq;
		video_context->DvpLatency =
		    (shared_context->target_latency * 1000);
		DvpVideoIoctlSetControl(video_context,
					V4L2_CID_STM_DVPIF_RESTORE_DEFAULT, 0);
	...
	if (shared_context->avr_device_context.Playback == NULL) {
		DVB_DEBUG
		    ("Overlay Interface has not started yet, nothing to do.\n");
		return 0;
	}
	// Video Start - if and only if a video input has been selected (handle is non-NULL)
				     ...
				     struct stm_v4l2_handles *handle, int id)
{
	...
	if (id > 0)
		AvrAudioSetInput(handle->v4l2type[STM_V4L2_AUDIO_INPUT].handle,
				 id - 1);
	...
	if (shared_context->avr_device_context.AudioCaptureStatus ==
	    DVP_CAPTURE_OFF) {
		if (handle->v4l2type[STM_V4L2_AUDIO_INPUT].handle)
			AvrAudioIoctlOverlayStop(handle->v4l2type
						 [STM_V4L2_AUDIO_INPUT].handle);
	}
...
static int avr_ioctl(struct stm_v4l2_handles *handle,
		     struct stm_v4l2_driver *driver, int device,
		     enum _stm_v4l2_driver_type type, struct file *file,
...
#define CASE(x) case x: DVB_DEBUG( #x "\n" );
	...
	/**********************************************************/
		...
		CASE(VIDIOC_S_FBUF) {
			...
			DvpVideoIoctlSetFramebuffer(handle->v4l2type
						    [STM_V4L2_VIDEO_INPUT].handle,
						    argp->fmt.width,
						    argp->fmt.height,
						    argp->fmt.bytesperline,
						    argp->fmt.priv);
			break;
		}
		...
		CASE(VIDIOC_S_STD) {
			...
			res =
			    DvpVideoIoctlSetStandard(handle->v4l2type
						     [STM_V4L2_VIDEO_INPUT].handle,
						     id);
			if (res != 0)
				return res;
		...
		CASE(VIDIOC_ENUMINPUT) {
			...
			strncpy(input->name, g_vinDevice[index].name, sizeof(input->name));
		...
		CASE(VIDIOC_S_INPUT) {
			int input_id =
			    (*(int *)arg) - driver->index_offset[device];
			...
			if (input_id == 0)
				shared_context->
				    avr_device_context.VideoCaptureStatus =
				    DVP_CAPTURE_OFF;
			else
				shared_context->
				    avr_device_context.VideoCaptureStatus =
			...
			res =
			    avr_ioctl_video_set_input(shared_context, handle,
						      input_id);
			if (res)
				return res;
		...
		CASE(VIDIOC_OVERLAY) {
			...
			{
				res =
				    avr_ioctl_overlay_start(shared_context,
							    handle);
				if (0 != res)
					return res;
			...
			{
				res =
				    avr_ioctl_overlay_stop(shared_context,
							   handle);
				if (0 != res)
					return res;
			} else {
		...
		CASE(VIDIOC_S_CROP) {
			...
			res =
			    DvpVideoIoctlCrop(handle->v4l2type
					      [STM_V4L2_VIDEO_INPUT].handle,
					      crop);
			if (res != 0)
				return res;
	...
	/**********************************************************/
		...
		CASE(VIDIOC_REQBUFS) {
			...
			dvp_v4l2_video_handle_t *VideoContext =
			    (dvp_v4l2_video_handle_t *)
			    handle->v4l2type[STM_V4L2_VIDEO_INPUT].handle;
			unsigned int i;

			AncillaryAssert((request->type ==
					 V4L2_BUF_TYPE_VBI_CAPTURE),
					"VIDIOC_REQBUFS: Type not V4L2_BUF_TYPE_VBI_CAPTURE");
			AncillaryAssert((request->memory == V4L2_MEMORY_MMAP),
					"VIDIOC_REQBUFS: Memory not V4L2_MEMORY_MMAP");

			res =
			    DvpVideoIoctlAncillaryRequestBuffers
			    (handle->v4l2type[STM_V4L2_VIDEO_INPUT].handle,
			     request->count, request->reserved[0],
			     &request->count, &request->reserved[0]);
			if (res != 0)
				return res;

			for (i = 0; i < request->count; i++)
				VideoContext->AncillaryBufferState[i].Mapped =
				    false;
		...
		CASE(VIDIOC_QUERYBUF) {
			...
			AncillaryAssert((buffer->type ==
					 V4L2_BUF_TYPE_VBI_CAPTURE),
					"VIDIOC_QUERYBUF: Type not V4L2_BUF_TYPE_VBI_CAPTURE");

			res =
			    DvpVideoIoctlAncillaryQueryBuffer(handle->v4l2type
							      [STM_V4L2_VIDEO_INPUT].handle,
							      buffer->index,
							      &Queued, &Done,
							      &PhysicalAddress,
			...
			buffer->flags = (Queued ? V4L2_BUF_FLAG_QUEUED : 0) |
			    ...
			    (VideoContext->
			     AncillaryBufferState[buffer->
						  index].Mapped ?
			     V4L2_BUF_FLAG_MAPPED : 0);
			buffer->field = V4L2_FIELD_NONE;
			buffer->timestamp.tv_sec =
			    (unsigned int)(CaptureTime / 1000000);
			...
			buffer->m.offset =
			    PhysicalAddress -
			    VideoContext->AncillaryBufferPhysicalAddress;
			buffer->length = Size;
		...
		CASE(VIDIOC_QBUF) {
			...
			AncillaryAssert((buffer->type ==
					 V4L2_BUF_TYPE_VBI_CAPTURE),
					"VIDIOC_QBUF: Type not V4L2_BUF_TYPE_VBI_CAPTURE");
			AncillaryAssert((buffer->memory == V4L2_MEMORY_MMAP),
					"VIDIOC_QBUF: Memory not V4L2_MEMORY_MMAP");

			res =
			    DvpVideoIoctlAncillaryQueueBuffer(handle->v4l2type
							      [STM_V4L2_VIDEO_INPUT].handle,
							      buffer->index);
			if (res != 0)
				return res;
		...
		CASE(VIDIOC_DQBUF) {
			...
			AncillaryAssert((buffer->type ==
					 V4L2_BUF_TYPE_VBI_CAPTURE),
					"VIDIOC_DQBUF: Type not V4L2_BUF_TYPE_VBI_CAPTURE");
			AncillaryAssert((buffer->memory == V4L2_MEMORY_MMAP),
					"VIDIOC_DQBUF: Memory not V4L2_MEMORY_MMAP");

			Blocking = (file->f_flags & O_NONBLOCK) == 0;

			res =
			    DvpVideoIoctlAncillaryDeQueueBuffer(handle->v4l2type
								[STM_V4L2_VIDEO_INPUT].handle,
								&buffer->index,
								Blocking);
			if (res != 0)
		...
		CASE(VIDIOC_STREAMON) {
			...
			AncillaryAssert(((*type) == V4L2_BUF_TYPE_VBI_CAPTURE),
					"VIDIOC_STREAMON: Type not V4L2_BUF_TYPE_VBI_CAPTURE");

			res =
			    DvpVideoIoctlAncillaryStreamOn(handle->v4l2type
							   [STM_V4L2_VIDEO_INPUT].handle);
			if (res != 0)
				return res;
		...
		CASE(VIDIOC_STREAMOFF) {
			...
			AncillaryAssert(((*type) == V4L2_BUF_TYPE_VBI_CAPTURE),
					"VIDIOC_STREAMOFF: Type not V4L2_BUF_TYPE_VBI_CAPTURE");

			res =
			    DvpVideoIoctlAncillaryStreamOff(handle->v4l2type
							    [STM_V4L2_VIDEO_INPUT].handle);
			if (res != 0)
				return res;
	...
	/**********************************************************/
		...
		CASE(VIDIOC_ENUMAUDIO) {
			...
			strncpy(audio->name, g_ainDevice[index].name, sizeof(audio->name));
		...
		CASE(VIDIOC_S_AUDIO) {
			...
			if (index == 0)
				shared_context->
				    avr_device_context.AudioCaptureStatus =
				    DVP_CAPTURE_OFF;
			else
				shared_context->
				    avr_device_context.AudioCaptureStatus =
			...
			res =
			    avr_ioctl_audio_set_input(shared_context, handle,
						      index);
			if (res)
				return res;
	...
	/**********************************************************/
		...
		CASE(VIDIOC_S_CTRL) {
			...
			case V4L2_CID_STM_AVR_AUDIO_EMPHASIS:
				FAIL_IF_NULL(handle->v4l2type
					     [STM_V4L2_AUDIO_INPUT].handle);
				if ((ctrlvalue != 0) || (ctrlvalue != 1))
					return -ERANGE;

				AvrAudioSetEmphasis(handle->v4l2type
						    [STM_V4L2_AUDIO_INPUT].handle,
						    ctrlvalue);
				break;
			...
			case V4L2_CID_STM_AVR_AUDIO_HDMI_LAYOUT:
				FAIL_IF_NULL(handle->v4l2type
					     [STM_V4L2_AUDIO_INPUT].handle);
				if ((ctrlvalue < V4L2_HDMI_AUDIO_LAYOUT0)
				    || (ctrlvalue > V4L2_HDMI_AUDIO_HBR))
					return -ERANGE;

				AvrAudioSetHdmiLayout(handle->v4l2type
						      [STM_V4L2_AUDIO_INPUT].handle,
						      ctrlvalue);
				break;
			...
			case V4L2_CID_STM_AVR_AUDIO_HDMI_AUDIOINFOFRAME_DB4:
				...
				AvrAudioSetHdmiAudioMode(handle->v4l2type
							 [STM_V4L2_AUDIO_INPUT].handle,
							 ctrlvalue);
				break;
			...
			case V4L2_CID_STM_AUDIO_FORMAT_RECOGNIZER:
				...
				if ((ctrlvalue != 0) && (ctrlvalue != 1))
					return -ERANGE;
				AvrAudioSetFormatRecogniserEnable
				    (handle->v4l2type
				     [STM_V4L2_AUDIO_INPUT].handle, ctrlvalue);
				break;
			...
			case V4L2_CID_STM_AUDIO_AAC_DECODE:
				...
				if ((ctrlvalue != 0) && (ctrlvalue != 1))
					return -ERANGE;
				AvrAudioSetAacDecodeEnable(handle->v4l2type
							   [STM_V4L2_AUDIO_INPUT].handle,
							   ctrlvalue);
				break;
			...
			case V4L2_CID_STM_AVR_AUDIO_CHANNEL_SELECT:
				...
				if ((ctrlvalue <
					...
					return -ERANGE;
				AvrAudioSetChannelSelect(handle->v4l2type
							 [STM_V4L2_AUDIO_INPUT].handle,
							 ctrlvalue);
				break;
			...
			case V4L2_CID_STM_AVR_AUDIO_SILENCE_THRESHOLD:
				...
				if ((ctrlsigned < -96) || (ctrlsigned > 0))
					return -ERANGE;
				AvrAudioSetSilenceThreshold(handle->v4l2type
							    [STM_V4L2_AUDIO_INPUT].handle,
							    ctrlsigned);
				break;
			...
			case V4L2_CID_STM_AVR_AUDIO_SILENCE_DURATION:
				FAIL_IF_NULL(handle->v4l2type
					     [STM_V4L2_AUDIO_INPUT].handle);
				AvrAudioSetSilenceDuration(handle->v4l2type
							   [STM_V4L2_AUDIO_INPUT].handle,
							   ctrlvalue);
				break;
		...
		CASE(VIDIOC_G_CTRL) {
			...
			case V4L2_CID_STM_AUDIO_FORMAT_RECOGNIZER:
				FAIL_IF_NULL(handle->v4l2type
					     [STM_V4L2_AUDIO_INPUT].handle);
				pctrl->value =
				    AvrAudioGetFormatRecogniserEnable
				    (handle->v4l2type
				     [STM_V4L2_AUDIO_INPUT].handle);
				break;
			...
			case V4L2_CID_STM_AVR_AUDIO_HDMI_LAYOUT:
				FAIL_IF_NULL(handle->v4l2type
					     [STM_V4L2_AUDIO_INPUT].handle);
				pctrl->value =
				    AvrAudioGetHdmiLayout(handle->v4l2type
							  [STM_V4L2_AUDIO_INPUT].handle);
				break;
			case V4L2_CID_STM_AVR_AUDIO_HDMI_AUDIOINFOFRAME_DB4:
				FAIL_IF_NULL(handle->v4l2type
					     [STM_V4L2_AUDIO_INPUT].handle);
				pctrl->value =
				    AvrAudioGetHdmiAudioMode(handle->v4l2type
							     [STM_V4L2_AUDIO_INPUT].handle);
				break;
			case V4L2_CID_STM_AVR_AUDIO_EMPHASIS:
				FAIL_IF_NULL(handle->v4l2type
					     [STM_V4L2_AUDIO_INPUT].handle);
				pctrl->value =
				    AvrAudioGetEmphasis(handle->v4l2type
							[STM_V4L2_AUDIO_INPUT].handle);
				break;
			...
			case V4L2_CID_STM_AUDIO_AAC_DECODE:
				FAIL_IF_NULL(handle->v4l2type
					     [STM_V4L2_AUDIO_INPUT].handle);
				pctrl->value =
				    AvrAudioGetAacDecodeEnable(handle->v4l2type
							       [STM_V4L2_AUDIO_INPUT].handle);
				break;
			...
			case V4L2_CID_STM_AUDIO_MUTE:
				FAIL_IF_NULL(handle->v4l2type
					     [STM_V4L2_AUDIO_INPUT].handle);
				pctrl->value =
				    AvrAudioGetEmergencyMuteReason
				    (handle->v4l2type
				     [STM_V4L2_AUDIO_INPUT].handle);
				break;
			...
			case V4L2_CID_STM_AVR_AUDIO_CHANNEL_SELECT:
				FAIL_IF_NULL(handle->v4l2type
					     [STM_V4L2_AUDIO_INPUT].handle);
				pctrl->value =
				    AvrAudioGetChannelSelect(handle->v4l2type
							     [STM_V4L2_AUDIO_INPUT].handle);
				break;
			...
			case V4L2_CID_STM_AVR_AUDIO_SILENCE_THRESHOLD:
				FAIL_IF_NULL(handle->v4l2type
					     [STM_V4L2_AUDIO_INPUT].handle);
				pctrl->value =
				    AvrAudioGetSilenceThreshold(handle->v4l2type
								[STM_V4L2_AUDIO_INPUT].handle);
				break;
			...
			case V4L2_CID_STM_AVR_AUDIO_SILENCE_DURATION:
				FAIL_IF_NULL(handle->v4l2type
					     [STM_V4L2_AUDIO_INPUT].handle);
				pctrl->value =
				    AvrAudioGetSilenceDuration(handle->v4l2type
							       [STM_V4L2_AUDIO_INPUT].handle);
				break;

			default:
				ret =
				    DvpVideoIoctlGetControl(handle->v4l2type
							    [STM_V4L2_VIDEO_INPUT].handle,
							    pctrl->id,
							    &pctrl->value);
				if (ret < 0) {
...
static int avr_close(struct stm_v4l2_handles *handle,
		     enum _stm_v4l2_driver_type type, struct file *file)
{
	int ret = 0;
	...
	if (!handle->v4l2type[STM_V4L2_VIDEO_INPUT].driver) {
		if (!handle->v4l2type[STM_V4L2_AUDIO_INPUT].driver) {
			DVB_DEBUG
			    ("Video and Audio handles NULL. Nothing to do.\n");
			return -EINVAL;
		}
		shared_context =
		    handle->v4l2type[STM_V4L2_AUDIO_INPUT].driver->priv;
	} else {
		shared_context =
		    handle->v4l2type[STM_V4L2_VIDEO_INPUT].driver->priv;
	}
...
//    in one lump.
//
#warning "TODO2.4 check this conversion of linuxdvb_vm_nopage to new vm api"
static int avr_vm_fault(struct vm_area_struct *vma, struct vm_fault *vmf)
{
	 ...
	 */
	page_addr =
	    (void *)(((unsigned long)vmf->virtual_address - vma->vm_start) +
		     (vma->vm_pgoff << PAGE_SHIFT));
#warning "TODO2.4 How do we get at the buffer address?"
...
static int avr_mmap(struct stm_v4l2_handles *handle,
		    enum _stm_v4l2_driver_type type, struct file *file,
		    struct vm_area_struct *vma)
{
	...
	dvp_v4l2_video_handle_t *VideoContext =
	    (dvp_v4l2_video_handle_t *) handle->
	    v4l2type[STM_V4L2_VIDEO_INPUT].handle;
...
//
	...
	ValidMapSize =
	    (VideoContext->AncillaryBufferCount *
	     VideoContext->AncillaryBufferSize) + (1 << PAGE_SHIFT) - 1;
	ValidMapSize = ValidMapSize & (~((1 << PAGE_SHIFT) - 1));

	if ((vma->vm_pgoff != 0)
	    || ((vma->vm_end - vma->vm_start) != ValidMapSize)) {
		printk
		    ("avr_mmap - Only whole buffer memory can be mapped - Invalid address range should be Offset = 0, length = Count * Size(%d*%d)[%08lx %04lx]\n",
		     VideoContext->AncillaryBufferCount,
		     VideoContext->AncillaryBufferSize, vma->vm_pgoff,
...
static struct stm_v4l2_driver avr_video_overlay = {
	...
	.control_range = {
			  {V4L2_CID_STM_AVR_VIDEO_FIRST,
			   V4L2_CID_STM_AVR_VIDEO_LAST},
			  {V4L2_CID_STM_AVR_VIDEO_FIRST,
			   V4L2_CID_STM_AVR_VIDEO_LAST},
			  {V4L2_CID_STM_AVR_VIDEO_FIRST,
			   V4L2_CID_STM_AVR_VIDEO_LAST}},
	.n_indexes = {ARRAY_SIZE(g_vinDevice), ARRAY_SIZE(g_vinDevice),
		      ARRAY_SIZE(g_vinDevice)},
	.ioctl = avr_ioctl,
	.close = avr_close,
	.poll = NULL,
	.mmap = avr_mmap,
...
static struct stm_v4l2_driver avr_audio_overlay = {
	...
	.control_range = {
			  {V4L2_CID_STM_AVR_AUDIO_I_FIRST,
			   V4L2_CID_STM_AVR_AUDIO_I_LAST},
			  {V4L2_CID_STM_AVR_AUDIO_I_FIRST,
			   V4L2_CID_STM_AVR_AUDIO_I_LAST},
			  {V4L2_CID_STM_AVR_AUDIO_I_FIRST,
			   V4L2_CID_STM_AVR_AUDIO_I_LAST}},
	.n_indexes = {ARRAY_SIZE(g_ainDevice), ARRAY_SIZE(g_ainDevice),
		      ARRAY_SIZE(g_ainDevice)},
	.ioctl = avr_ioctl,
	.close = avr_close,
	.poll = NULL,
	.mmap = NULL,
...
{
	avr_v4l2_shared_handle_t *ctx =
	    kzalloc(sizeof(avr_v4l2_shared_handle_t), GFP_KERNEL);
	...
	ctx->avr_device_context.VideoId = INVALID_MEDIA_ID;	/* CodeToInteger('v','i','d','s'); */
	...
	ctx->avr_device_context.AudioId = INVALID_MEDIA_ID;	/* CodeToInteger('a','u','d','s'); */
...
static int avr_probe(struct device *dev)
{
	...
	res =
	    snd_pseudo_register_mixer_observer(0, avr_update_mixer_settings,
					       shared_context);
	if (res < 0)
		DVB_ERROR
		    ("Cannot register mixer observer (%d), using default latency value\n",
		     res);
	// no further recovery possible

	// get persistent access to the downmix firmware (if present)
...
static struct platform_driver avr_driver = {
	.driver = {
		   .name = "dvr",
		   .owner = THIS_MODULE,
		   },
...
{
	return platform_driver_register(&avr_driver);
}

static void avr_cleanup(void)
{
#if 0
	platform_driver_unregister(&avr_driver);
#endif
}
