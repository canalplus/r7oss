...
Source file name :diseqc_stv0910.c
Author :          Rahul
...
static int diseqc_stv0910_init(struct stm_fe_diseqc_s *priv);
static int diseqc_stv0910_transfer(struct stm_fe_diseqc_s *priv,
	stm_fe_diseqc_mode_t mode, stm_fe_diseqc_msg_t *msg_d, uint32_t num);
static int diseqc_stv0910_term(struct stm_fe_diseqc_s *priv);

/*
 * Name: stm_fe_diseqc_stv0910_attach()
 *
 * Description: Installed the diseqc_stv0910 wrapper function pointers for LLA
 *
 */
int stm_fe_diseqc_stv0910_attach(struct stm_fe_diseqc_s *priv)
{
	if (!priv->ops)
		return -EFAULT;

	priv->ops->init = diseqc_stv0910_init;
	priv->ops->transfer = diseqc_stv0910_transfer;
	priv->ops->term = diseqc_stv0910_term;
	priv->ops->detach = stm_fe_diseqc_stv0910_detach;

	return 0;
}
EXPORT_SYMBOL(stm_fe_diseqc_stv0910_attach);

/*
 * Name: stm_fe_diseqc_stv0910_detach()
 *
 * Description: uninstall the diseqc_stv0910 wrapper function
 *
 */
int stm_fe_diseqc_stv0910_detach(struct stm_fe_diseqc_s *priv)
{
	if (!priv->ops)
		return -EFAULT;
...
EXPORT_SYMBOL(stm_fe_diseqc_stv0910_detach);

static int stv0910_diseqc_get_path(struct diseqc_config_s *config)
{
	if (IS_NAME_MATCH(config->diseqc_name, "DISEQC_STV0910P1"))
		return FE_SAT_DEMOD_1;
	else
		return FE_SAT_DEMOD_2;
}
#define THIS_PATH(p) (stv0910_diseqc_get_path(p->config))


/*
 * Name: diseqc_stv0910_init()
 *
...
static int diseqc_stv0910_init(struct stm_fe_diseqc_s *priv)
{
	...
	FE_STV0910_InternalParams_t *params = NULL;

	demod_priv = (struct stm_fe_demod_s *)priv->demod_h;
	params = (FE_STV0910_InternalParams_t *)demod_priv->demod_params;
...
/*
 * Name: diseqc_stv0910_get_mode()
 *
 * Description: retreive the diseqc mode
 *
 */
static FE_Sat_DiseqC_TxMode diseqc_stv0910_get_mode(FE_STV0910_Handle_t hdl,
					 ...
					 uint8_t *tx_data, uint32_t *nbdata)
{
	...
	params = (FE_STV0910_InternalParams_t *) hdl;
			...
			uint32_t num_msg)
{
	...
	if (mode & STM_FE_DISEQC_COMMAND)
		mode_d = diseqc_stv0910_get_mode(hdl, mode, msg_d[i].msg,
				&msg_d[i].msg_len);
	else
		mode_d = diseqc_stv0910_get_mode(hdl, mode, tx_data, &nbdata);

	err = FE_STV0910_DiseqcInit(hdl, mode_d, THIS_PATH(priv));
	...
	if (!(mode & STM_FE_DISEQC_COMMAND)) {
		err = FE_STV0910_DiseqcSend(hdl, tx_data, nbdata,
						THIS_PATH(priv));
		if (err != FE_LLA_NO_ERROR) {
	...
	while (i < num_msg) {
		if (msg_d[i].op == STM_FE_DISEQC_TRANSFER_SEND) {
			...
			ret = lnb_priv->ops->setconfig(lnb_priv, &lnbconfig);
			err = FE_STV0910_DiseqcSend(hdl, msg_d[i].msg,
			...
			err = FE_STV0910_DiseqcReceive(hdl, msg_d[i].msg,
					&msg_d[i].msg_len, THIS_PATH(priv));
...
static int32_t __init stm_fe_diseqc_stv0910_init(void)
{
	pr_info("Loading diseqc_stv0910 driver module...\n");

	return 0;
}

module_init(stm_fe_diseqc_stv0910_init);

static void __exit stm_fe_diseqc_stv0910_term(void)
{
	pr_info("Removing diseqc_stv0910 driver module ...\n");
}

module_exit(stm_fe_diseqc_stv0910_term);

MODULE_DESCRIPTION("Low level driver for STV0910 diseqc controller");
