...
Source file name :diseqc_stv090x.c
Author :          Shobhit
...
static int diseqc_stv090x_init(struct stm_fe_diseqc_s *priv);
static int diseqc_stv090x_transfer(struct stm_fe_diseqc_s *priv,
	stm_fe_diseqc_mode_t mode, stm_fe_diseqc_msg_t *msg_d, uint32_t num);
static int diseqc_stv090x_term(struct stm_fe_diseqc_s *priv);

/*
 * Name: stm_fe_diseqc_stv090x_attach()
 *
 * Description: Installed the diseqc_stv090x wrapper function pointers for LLA
 *
 */
int stm_fe_diseqc_stv090x_attach(struct stm_fe_diseqc_s *priv)
{
	...
	priv->ops->init = diseqc_stv090x_init;
	priv->ops->transfer = diseqc_stv090x_transfer;
	priv->ops->term = diseqc_stv090x_term;
	priv->ops->detach = stm_fe_diseqc_stv090x_detach;

	return 0;
}
EXPORT_SYMBOL(stm_fe_diseqc_stv090x_attach);

/*
 * Name: stm_fe_diseqc_stv090x_detach()
 *
 * Description: uninstall the diseqc_stv090x wrapper function
 *
 */
...
EXPORT_SYMBOL(stm_fe_diseqc_stv090x_detach);

/*
 * Name: diseqc_stv090x_init()
 *
...
static int diseqc_stv090x_init(struct stm_fe_diseqc_s *priv)
{
	...
	FE_STV0903_InternalParams_t *params = NULL;
	demod_priv = (struct stm_fe_demod_s *)priv->demod_h;

	params = (FE_STV0903_InternalParams_t *)demod_priv->demod_params;
...
/*
 * Name: diseqc_stv090x_get_mode()
 *
 * Description: retreive the diseqc mode
 *
 */
static FE_Sat_DiseqC_TxMode diseqc_stv090x_get_mode(FE_STV0903_Handle_t hdl,
					 ...
					 uint8_t *tx_data, uint32_t *nbdata)
{
	...
	params = (FE_STV0903_InternalParams_t *)hdl;
	...
	stm_fe_diseqc_mode_t mode, stm_fe_diseqc_msg_t *msg_d, uint32_t num)
{
	...
	if (mode & STM_FE_DISEQC_COMMAND)
		mode_d = diseqc_stv090x_get_mode(hdl, mode, msg_d[i].msg,
				&msg_d[i].msg_len);
	else
		mode_d = diseqc_stv090x_get_mode(hdl, mode, tx_data, &nbdata);

	err = FE_STV0903_DiseqcInit(hdl, mode_d);
	...
	if (!(mode & STM_FE_DISEQC_COMMAND)) {
		err = FE_STV0903_DiseqcSend(hdl, tx_data, nbdata);
	...
	while (i < num) {
		if (msg_d[i].op == STM_FE_DISEQC_TRANSFER_SEND) {
			err = FE_STV0903_DiseqcSend(hdl, msg_d[i].msg,
					msg_d[i].msg_len);
			...
			err = FE_STV0903_DiseqcReceive(hdl, msg_d[i].msg,
					&msg_d[i].msg_len);
...
static int32_t __init stm_fe_diseqc_stv090x_init(void)
{
	pr_info("Loading diseqc_stv090x driver module...\n");

	return 0;
}

module_init(stm_fe_diseqc_stv090x_init);

static void __exit stm_fe_diseqc_stv090x_term(void)
{
	pr_info("Removing diseqc_stv090x driver module ...\n");
}

module_exit(stm_fe_diseqc_stv090x_term);

MODULE_DESCRIPTION("Low level driver for STV0903 diseqc controller");
