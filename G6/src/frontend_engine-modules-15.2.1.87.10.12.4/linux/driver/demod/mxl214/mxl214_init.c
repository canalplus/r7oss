...
STCHIP_Register_t DefMXL214_HRCLSVal[MXL214_HRCLS_NBREGS];

STCHIP_Error_t MXL214_Init(Demod_InitParams_t *InitParams,
				STCHIP_Handle_t *hChipHandle)
{
	...
	InitParams->Chip->NbRegs   = MXL214_HRCLS_NBREGS;
	InitParams->Chip->NbFields = MXL214_HRCLS_NBFIELDS;
	...
	if (hChip != NULL) {
	    ...
	    **     ----------------------
	    ********************************/
		ChipUpdateDefaultValues(hChip, DefMXL214_HRCLSVal);
