...
#define MXL_HRCLS_PARALLEL_PAD_DRV_COUNT 12
  ...
  MXLENTERAPISTR(devId);
  MXLENTERAPI(MxL_HRCLS_DEBUG("\n %s %d outputId=%d, enable=%d, lsbOrMsbFirst=%d, mpegSyncPulseWidth=%d, mpegValidPol=%d, mpegSyncPol=%d, mpegClkPol=%d, clkFreq=%d, SYN pad drv=%d, VAL pad drv=%d, DAT pad drv=%d\n",
							   __func__,__LINE__,
							   outputId,
