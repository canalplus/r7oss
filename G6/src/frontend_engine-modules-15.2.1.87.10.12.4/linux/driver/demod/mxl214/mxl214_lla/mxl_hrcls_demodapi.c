...
//    resampleRateRatio = ((UINT64)(devContextPtr->adcSampRateInHz)*(UINT64)(1<<24)*3)/(256ULL*4*(UINT64)symbolRate[i]);
    resampleRateRatio = MXL_DIV_ROUND64((UINT64) ((UINT64) devContextPtr->adcSampRateInHz * (UINT64) (1<<14) * (UINT64) 3), symbolRate[i]);
...
//          symbolRateInHz = ((UINT64)(devContextPtr->adcSampRateInHz)*(UINT64)(1<<14)*(byPass?1:3))/((UINT64) resampleRateRatio);
          symbolRateInHz = MXL_DIV_ROUND64((UINT64)(devContextPtr->adcSampRateInHz)*(UINT64)(1<<14)*(byPass?1:3), resampleRateRatio);
          if (FIRMWARE_STATUS_LOADED == devContextPtr->firmwareStatus)
          {
    ...
    if (devContextPtr->driverInitialized)
    {
      ...
      if (status == MXL_SUCCESS)
      {
        ...
        if (resampleRate)
        {
          ...
          temp *= 2000000;
          temp = MXL_DIV_ROUND64_S(temp, tmpRate2);
          temp = MXL_DIV_ROUND64_S(temp, (SINT64) 4);
    ...
    )
{
  ...
  MXLENTERAPISTR(devId);
  MXLENTERAPI(MxL_HRCLS_DEBUG("\n%s %d demodId=%d\n",__func__,__LINE__,demodId););
  if ((devContextPtr) && (demodId < devContextPtr->demodsCnt) &&
 ...
 *  @param[out] frequencyInHzPtr pointer to returned frequency in Hz
 *
 *  @apibrief   Used to readout frequency of DFE channel in Hz
 *
 *  @usage      After channel is tuned, this API can be called at any time to get
 *              frequency of a DFE channel
 *
 *  @equ261     None
 *
 *  @return     MXL_SUCCESS or MXL_FAILURE
...
MXL_STATUS_E MxLWare_HRCLS_API_ReqChanFrequency(
    UINT8     devId,
    MXL_HRCLS_CHAN_ID_E  chanId,
    UINT32*   frequencyInHzPtr
    )
{
  MXL_STATUS_E  status = MXL_SUCCESS;
  UINT16 dfeChanXbarSel = 0, fcwLo = 0, fcwHi = 0;
  SINT32 fcw;
  SINT32 ddfsTuningFreq;
  UINT32 fadcDivBy32, fadcDivBy64;

  MXL_HRCLS_DEV_CONTEXT_T * devContextPtr = MxL_HRCLS_Ctrl_GetDeviceContext(devId);

  MXLENTERAPISTR(devId);
  MXLENTERAPI(MxL_HRCLS_DEBUG("chanId=%d\n", chanId););
  if (chanId == MXL_HRCLS_CHAN_NB)
  {
	  MXLENTERAPI(MxL_HRCLS_DEBUG("Readout frequency for NB tuner not support\n"););
  }

  if ((devContextPtr) && ((chanId < devContextPtr->dfeChanCount) ))
  {
    if (devContextPtr->driverInitialized)
    {
      MXL_HRCLS_CHAN_ID_E phyChanId = MxL_HRCLS_Ctrl_ConvertLogical2PhysicalChanId(devContextPtr, chanId);
      {
        status = MxL_HRCLS_Ctrl_ReadRegisterField(devId,
												  EXTRACT_ADDR(DFE_CHN0_XBAR_SEL)+ (phyChanId*0x200),
												  EXTRACT_LSBLOC(DFE_CHN0_XBAR_SEL),
												  EXTRACT_NBITS(DFE_CHN0_XBAR_SEL),
												  &dfeChanXbarSel);
        status = MxL_HRCLS_Ctrl_ReadRegisterField(devId,
												  EXTRACT_ADDR(DFE_CHN0_DMIX_FCW_LO)+ (phyChanId*0x200),
												  EXTRACT_LSBLOC(DFE_CHN0_DMIX_FCW_LO),
												  EXTRACT_NBITS(DFE_CHN0_DMIX_FCW_LO),
												  &fcwLo);
        status = MxL_HRCLS_Ctrl_ReadRegisterField(devId,
												  EXTRACT_ADDR(DFE_CHN0_DMIX_FCW_HI)+ (phyChanId*0x200),
												  EXTRACT_LSBLOC(DFE_CHN0_DMIX_FCW_HI),
												  EXTRACT_NBITS(DFE_CHN0_DMIX_FCW_HI),
												  &fcwHi);

        if( fcwHi & 0x1 )
        {
          fcw = (SINT32)((SINT64)fcwLo -(SINT64)65536);
        }
        else{
          fcw = (SINT32)fcwLo;
        }

        fadcDivBy32  = devContextPtr->adcSampRateInHz/32;//2^5
        fadcDivBy64  = fadcDivBy32 >> 1; //2^6

        ddfsTuningFreq = (SINT32)((SINT64)fcw*((SINT64)devContextPtr->adcSampRateInHz)/(SINT64)(1<<21));
        *frequencyInHzPtr = (UINT32)(ddfsTuningFreq + (SINT32)(fadcDivBy32*(dfeChanXbarSel - (dfeChanXbarSel & 0x1))));
      }
    } else status = MXL_NOT_INITIALIZED;
  } else status = MXL_INVALID_PARAMETER;
  MXLEXITAPISTR(devId, status);
 ...
 *  @param[in]  devId MxL device id
 *  @param[in]  demodId Demodulator ID number
 *  @param[out] symbolRateInHzPtr symbol rate
 *
 *  @apibrief   This API retrieves the symbol rate.
 *
 *  @usage      After channel tuning, it can be called at any time.
 *
 *  @return     MXL_SUCCESS or MXL_FAILURE
...
MXL_STATUS_E MxLWare_HRCLS_API_ReqDemodsymbolRate(
    UINT8     devId,
    MXL_HRCLS_DMD_ID_E demodId,
    UINT32*   symbolRateInHzPtr
    )
{
  UINT8 status = MXL_SUCCESS;
  UINT8 byPass;
  /*UINT16 regData = 0;*/  /*unused */
  UINT32 symbolRateInHz = 0;
  UINT32 resampleRateRatio = 0;
  ...
  MXLENTERAPISTR(devId);
  MXLENTERAPI(MxL_HRCLS_DEBUG("demodId=%d\n", demodId););
  if ((devContextPtr) && (symbolRateInHzPtr) && (demodId < devContextPtr->demodsCnt))
  {
    if (devContextPtr->driverInitialized)
    {
      demodId = MxL_HRCLS_Ctrl_ConvertLogical2PhysicalDemodId(devContextPtr, demodId);
      if (status == MXL_SUCCESS)
      {
        ...
        if ((status == MXL_SUCCESS) && (resampleRateRatio))
        {
          ...
          MXLDBG2(MxL_HRCLS_DEBUG("symbolRate=%dSps\n", symbolRateInHz););
        }
        else
        {
          MXLERR(MxL_HRCLS_ERROR("%s: RATE_RESAMP_RATE has not been programmed\n", __FUNCTION__););
  ...
  *symbolRateInHzPtr = symbolRateInHz;
  return (MXL_STATUS_E) status;
}
