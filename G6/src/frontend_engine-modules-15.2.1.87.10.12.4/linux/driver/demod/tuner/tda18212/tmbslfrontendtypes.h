...
typedef struct _tmbslFrontEndIoFunc_t {
	...
	tmErrorCode_t(*Read)(tmUnitSelect_t tUnit, UInt32 AddrSize,
	      unsigned char *pAddr, UInt32 ReadLen, unsigned char *pData);
		/** Write hardware register, 8bit aligned function */
	tmErrorCode_t(*Write)(tmUnitSelect_t tUnit, UInt32 AddrSize,
	       unsigned char *pAddr, UInt32 WriteLen, unsigned char *pData);
		/** SPI Read hardware function */
	tmErrorCode_t(*SpiRead)(tmUnitSelect_t tUnit, unsigned char CS,
				 UInt32 ReadBitsLen, unsigned char *pData);
		/** SPI Write hardware register */
	tmErrorCode_t(*SpiWrite)(tmUnitSelect_t tUnit, unsigned char CS,
				  UInt32 WriteBitsLen, unsigned char *pData);
		/** SPI Write followed by Read hardware register */
	tmErrorCode_t(*SpiWriteRead)(tmUnitSelect_t tUnit, unsigned char CS,
			      UInt32 WriteBitsLen, unsigned char *pDataWrite,
			      UInt32 ReadBitsLen, unsigned char *pDataRead);
...
typedef struct _tmbslFrontEndMutexFunc_t {
	...
	tmErrorCode_t(*Acquire)(ptmbslFrontEndMutexHandle pMutexHandle,
				 UInt32 timeOut);
...
#define P_FUNC_SMUTEX_ACQUIRE_VALID(_PFUNC) (P_FUNC_SMUTEX_ACQUIRE(_PFUNC) \
									!= Null)
#define FUNC_SMUTEX_RELEASE_VALID(_FUNC) (FUNC_SMUTEX_RELEASE(_FUNC) != Null)
#define P_FUNC_SMUTEX_RELEASE_VALID(_PFUNC) (P_FUNC_SMUTEX_RELEASE(_PFUNC) \
									!= Null)
...
#define tmDBGPRINTEx(_level, _format, tunit)
