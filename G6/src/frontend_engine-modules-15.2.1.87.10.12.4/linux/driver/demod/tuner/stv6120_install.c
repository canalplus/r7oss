...
STV6120 installation functions
...
int stm_fe_stv6120_attach(struct stm_fe_demod_s *priv)
{
	...
	size = STV6120_NBREGS * sizeof(STCHIP_Register_t);
	...
	tuner_ops->init = STV6120_TunerInit;
	tuner_ops->set_referenceFreq = STV6120_TunerSetReferenceFreq;
	tuner_ops->set_gain = STV6120_TunerSetGain;
	tuner_ops->get_gain = STV6120_TunerGetGain;
	tuner_ops->set_frequency = STV6120_TunerSetFrequency;
	tuner_ops->switch_input = STV6120_TunerSwitchInput;
	tuner_ops->set_bandWidth = STV6120_TunerSetBandwidth;
	tuner_ops->set_standby = STV6120_TunerSetStandby;
	tuner_ops->get_status = STV6120_TunerGetStatus;
	tuner_ops->get_frequency = STV6120_TunerGetFrequency;
	tuner_ops->set_hmr_filter = STV6120_TunerSetHMR_Filter;
	tuner_ops->set_lnainput = STV6120_TunerSetLNAInput;
	tuner_ops->term = STV6120_TunerTerm;
	tuner_ops->detach = stm_fe_stv6120_detach;

	return 0;

}
EXPORT_SYMBOL(stm_fe_stv6120_attach);

/*
 * Name: stm_fe_stv6120_detach()
 *
...
EXPORT_SYMBOL(stm_fe_stv6120_detach);

static int32_t __init stv6120_lla_init(void)
{
	pr_info("Loading stv6120 tuner module ...\n");

	return 0;
}

module_init(stv6120_lla_init);

static void __exit stv6120_lla_term(void)
{
	pr_info("Removing stv6120 tuner module ...\n");

}

module_exit(stv6120_lla_term);

MODULE_DESCRIPTION("Low level driver for STV6120 satellite tuner");
