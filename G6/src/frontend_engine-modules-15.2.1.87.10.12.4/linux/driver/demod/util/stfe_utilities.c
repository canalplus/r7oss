...
Source file name : stfe_utilities.c
Author :           Shobhit
...
#define MULT32X32(a, b) (((((U32)((a) >> 16)) * ((U32)((b) >> 16))) << 1) \
		+ ((((U32)((a) >> 16)) * ((U32)(b & 0x0000ffff))) >> 15) \
		+ ((((U32)((b) >> 16)) * ((U32)((a) & 0x0000ffff))) >> 15))

/*****************************************************
**FUNCTION  ::  PowOf4 >>A #define is used that is in stfe_utilities.h
...
/*****************************************************
**FUNCTION	::	PowOf2 >>A #define is used that is in stfe_utilities.h
...
U32 GetPowOf2(S32 number)
{
	...
	while (PowOf2(i) < number)
		i++;

	return i;
}
EXPORT_SYMBOL(GetPowOf2);
...
S32 Log10Int(S32 x)
{
	...
	if (x < 0xffff) {
		x = (x << 16) & 0xffff0000;
		/* x written in fixed point mode with frac
		*and integer parts on 16 bits each */

		y = 0x483f4;	/* log10(2^15) */
		if (x < 0x00008000) {
			x <<= 16;
			y -= 0x4d104;	/* log10(2^16) */
		}
		if (x < 0x00800000) {
			x <<= 8;
			y -= 0x26882;	/* log10(2^8) */
...
unsigned int intlog2(u32 value)
{
  ...
  */
	...
	log2 = ((msb << 24) + (logtable[logentry] << 8) + interpolation);
	return log2;
}
...
EXPORT_SYMBOL(Log10Int);
/* End of stfe_utilities.c */
