...
Source file name : d0900.c
Author :           Ankur

Wrapper for STV0900 LLA
...
#define STV0900_CURRENT_LLA_REVISION "STV0900-LLA_REL_4.8_DECEMBER_2010"

#define THIS_PATH(p) (stv0900_get_path(p->config))
...
MODULE_PARM_DESC(lla_wrapper_debug,
		"Turn on/off stv0900 lla wrapper debugging (default:off).");
#define dpr_info(x...) do { if (lla_wrapper_debug) pr_info(x); } while (0)
#define dpr_err(x...) do { if (lla_wrapper_debug) pr_err(x); } while (0)

static int stv0900_init(struct stm_fe_demod_s *priv);
static int stv0900_tune(struct stm_fe_demod_s *priv, bool *lock);
static int stv0900_scan(struct stm_fe_demod_s *priv, bool *lock);
static int stv0900_status(struct stm_fe_demod_s *priv, bool *locked);
static int stv0900_tracking(struct stm_fe_demod_s *priv);
static int stv0900_unlock(struct stm_fe_demod_s *priv);
static int stv0900_term(struct stm_fe_demod_s *priv);
static int stv0900_abort(struct stm_fe_demod_s *priv, bool abort);
static int stv0900_standby(struct stm_fe_demod_s *priv, bool standby);
static int stv0900_restore(struct stm_fe_demod_s *priv);

/*
 * Name: stm_fe_stv0900_attach()
 *
 * Description: Installed the wrapper function pointers for LLA
 *
 */
int stm_fe_stv0900_attach(struct stm_fe_demod_s *priv)
{
	if (!priv->ops)
		return -EFAULT;
	priv->ops->init = stv0900_init;
	priv->ops->tune = stv0900_tune;
	priv->ops->scan = stv0900_scan;
	priv->ops->tracking = stv0900_tracking;
	priv->ops->status = stv0900_status;
	priv->ops->unlock = stv0900_unlock;
	priv->ops->term = stv0900_term;
	priv->ops->abort = stv0900_abort;
	priv->ops->standby = stv0900_standby;
	priv->ops->detach = stm_fe_stv0900_detach;
	priv->ops->restore = stv0900_restore;
...
EXPORT_SYMBOL(stm_fe_stv0900_attach);

/*
 * Name: stm_fe_stv0900_detach()
 *
 * Description: Uninstalled the wrapper function pointers for LLA
 *
 */
int stm_fe_stv0900_detach(struct stm_fe_demod_s *priv)
{
	if (!priv->ops)
		return -EFAULT;
...
EXPORT_SYMBOL(stm_fe_stv0900_detach);


static int stv0900_get_path(struct demod_config_s *config)
{
	if (IS_NAME_MATCH(config->demod_name, "STV0900P1")) {
		dpr_info("%s: path selected is STV0900P1", __func__);
		return FE_SAT_DEMOD_1;
	} else {
		dpr_info("%s: path selected is STV0900P2", __func__);
		return FE_SAT_DEMOD_2;
	}
}

static struct stm_fe_demod_s *get_demod_from_name(struct stm_fe_demod_s *priv,
								char *name)
{
...
static void get_tsoutput(U8 ChipID, FE_STV0900_TS_Output *Path1TSoutpt,
		 FE_STV0900_TS_Output *Path2TSoutpt,
		 FE_TS_OutputMode_t P1TSMode, FE_TS_OutputMode_t P2TSMode)
{
	if (ChipID >= 0x30) {
		/*Cut 3.0 */
		*Path1TSoutpt = FE_STV0900_TS3;
		*Path2TSoutpt = FE_STV0900_TS2;
	}
	if (ChipID >= 0x10) {
		/*Cut 2.0/1.0 */
		switch (P1TSMode) {
		...
		case FE_TS_DVBCI_CLOCK:
			...
			case FE_TS_PARALLEL_PUNCT_CLOCK:
			case FE_TS_DVBCI_CLOCK:
			/* Mode Mux Stream, parallel interface is multiplexed
			 * between path 1 and path2.
			 * Path1 || on TS3, Path2 || on TS3.
			 */
				*Path1TSoutpt = FE_STV0900_MUXTS;
				*Path2TSoutpt = FE_STV0900_MUXTS;
				break;

			case FE_TS_SERIAL_PUNCT_CLOCK:
			case FE_TS_SERIAL_CONT_CLOCK:
			default:
			/* Path1 || on TS3, Path2 serial on TS2. */
				*Path1TSoutpt = FE_STV0900_TS3;
				*Path2TSoutpt = FE_STV0900_TS2;
				break;
			}
		...
		default:
			...
			case FE_TS_PARALLEL_PUNCT_CLOCK:
			case FE_TS_DVBCI_CLOCK:
			/*Path1 serial on TS2, Path2 || on TS3*/
				*Path1TSoutpt = FE_STV0900_TS2;
				*Path2TSoutpt = FE_STV0900_TS3;
				break;

			case FE_TS_SERIAL_PUNCT_CLOCK:
			case FE_TS_SERIAL_CONT_CLOCK:
			default:
			/* Path1 serial on TS3, Path2 serial on TS2 */
				*Path1TSoutpt = FE_STV0900_TS3;
				*Path2TSoutpt = FE_STV0900_TS2;
				break;
			}
...
static FE_STV0900_Error_t set_tsoutput(struct stm_fe_demod_s *assoc_demod,
			       FE_STV0900_InternalParams_t *params,
			       FE_STV0900_TSConfig_t *p1_ts_conf,
			       FE_STV0900_TSConfig_t *p2_ts_conf, int path)
{
	FE_STV0900_Error_t err = FE_LLA_NO_ERROR;

	/* For dual path */
	if (assoc_demod) {
		get_tsoutput(params->hDemod->ChipID,
				&params->Path1TSoutpt, &params->Path2TSoutpt,
				p1_ts_conf->TSMode, p2_ts_conf->TSMode);
		err |= FE_STV0900_SetTSOutput(params, params->Path1TSoutpt,
				p1_ts_conf->TSMode, params->Path2TSoutpt,
				p2_ts_conf->TSMode);
		return err;
	}
	/* For single path */
	if (path == FE_SAT_DEMOD_1)
		/* TS3 is default for path 1 for single instance.  P2 will also
		 * have default values. */
		err = FE_STV0900_SetTSOutput(params, FE_STV0900_TS3,
				p1_ts_conf->TSMode, FE_STV0900_TS2,
				FE_TS_SERIAL_PUNCT_CLOCK);
	else if (path == FE_SAT_DEMOD_2)
		/* TS3 is default for path 2 also for single instance.  Path 1
		 * will also have default values. */
		err = FE_STV0900_SetTSOutput(params, FE_STV0900_TS2,
				FE_TS_SERIAL_PUNCT_CLOCK, FE_STV0900_TS3,
				p2_ts_conf->TSMode);
...
static int stv0900_init(struct stm_fe_demod_s *priv)
{
	FE_STV0900_InternalParams_t *p = NULL;
	FE_Sat_InitParams_t ip;
	struct demod_config_s *conf = priv->config;
	FE_STV0900_TSConfig_t p1_ts_conf, p2_ts_conf;
	uint32_t p1_ts_speed, p2_ts_speed;
	int ret = 0, path = stv0900_get_path(conf);
	...
	strncpy(ip.DemodName, "STV0900", sizeof(ip.DemodName));
	...
	if (path == FE_SAT_DEMOD_1) {
		...
		dpr_info("%s: tuner params for demod 0:\n"
				  ...
				  "TunerInputSelect = %d\n", __func__,
				   ...
				   ip.TunerInputSelect);
		assoc_priv = get_demod_from_name(priv, "STV0900P2");
	} else {
		...
		assoc_priv = get_demod_from_name(priv, "STV0900P1");
	}
	...
	p = stm_fe_malloc(sizeof(FE_STV0900_InternalParams_t));
	if (!p) {
		pr_err("%s: mem alloc failed stv0900 params\n", __func__);
		return -ENOMEM;
	}
	...
	if (!assoc_priv->demod_params) {
		...
		if (path == FE_SAT_DEMOD_1) {
			...
			stmfe_set_ts_config(assoc_priv->config->ts_out,
				conf->ts_clock, FE_SAT_DEMOD_2, &p2_ts_conf);
		} else {
			stmfe_set_ts_config(assoc_priv->config->ts_out,
	...
	size = STV0900_NBREGS * sizeof(STCHIP_Register_t);
	...
	err = FE_STV0900_Init(&ip, &priv->demod_params);
	if (err != FE_LLA_NO_ERROR) {
		pr_err("%s: FE_STV0900_Init failed: %d\n", __func__, err);
		ret = -EINVAL;
		goto err_ret;
	}

	err = set_tsoutput(assoc_priv->demod_params, p, &p1_ts_conf,
			&p2_ts_conf, path);
	if (err != FE_LLA_NO_ERROR) {
		pr_err("%s: set_tsoutput failed %d\n", __func__, err);
		ret = -EINVAL;
		goto err_ret;
	}

	err = FE_STV0900_SetTSConfig(p, &p1_ts_conf, &p2_ts_conf, &p1_ts_speed,
								&p2_ts_speed);
	if (err != FE_LLA_NO_ERROR) {
		pr_err("%s: STV0900_SetTSConfig failed: %d\n", __func__, err);
		ret = -EINVAL;
		goto err_ret;
	}
 ...
 * Name: stv0900_term()
 *
 * Description: terminating the stv0900 LLA
 *
 */
static int stv0900_term(struct stm_fe_demod_s *priv)
{
...
{
	FE_STV0900_Error_t err;
	FE_STV0900_SearchParams_t sp;
	FE_STV0900_SearchResult_t rp;
	...
	memset(&rp, '\0', sizeof(FE_STV0900_SearchResult_t));
	memset(&sp, '\0', sizeof(FE_STV0900_SearchParams_t));
	...
	err = FE_STV0900_Search((FE_STV0900_Handle_t) hdl, &sp, &rp);
	if (!err && rp.Locked) {
		*lock = rp.Locked;
...
static int stv0900_scan(struct stm_fe_demod_s *priv, bool *lock)
{
	...
	dpr_info("%s: Freq = %d KHz\nSearchRange = %d Hz\nPath = %d\n"
			  "SymbolRate = %d bds\n", __func__,
			   sp.Frequency, sp.SearchRange, sp.Path,
			   sp.SymbolRate);
	err = FE_STV0900_Search((FE_STV0900_Handle_t) hdl, &sp, &rp);
	if (!err && rp.Locked) {
		*lock = rp.Locked;
...
static int stv0900_tracking(struct stm_fe_demod_s *priv)
{
	...
	err = FE_STV0900_GetSignalInfo(hdl, THIS_PATH(priv), &si);
	if (!err) {
		if (si.Standard == FE_SAT_DVBS1_STANDARD) {
			...
			FE_STV900_GetPacketErrorRate(hdl, THIS_PATH(priv),
		...
		if (info->std & STM_FE_DEMOD_TX_STD_S1_S2) {
			...
			isat->fec = stmfe_get_sat_fec_rate(si.Standard,
					si.PunctureRate, si.ModCode);
			isat->inv = si.Spectrum ?  FE_SAT_IQ_NORMAL :
							FE_SAT_IQ_SWAPPED;
			isat->mod = stmfe_get_sat_modulation(info->std,
					si.ModCode);
...
static int stv0900_status(struct stm_fe_demod_s *priv, bool *locked)
{
	...
	*locked = FE_STV0900_Status(hdl, THIS_PATH(priv));
...
static int stv0900_unlock(struct stm_fe_demod_s *priv)
{
	...
	err = FE_STV0900_Unlock(hdl, THIS_PATH(priv));
...
static int stv0900_abort(struct stm_fe_demod_s *priv, bool abort)
{
	...
	err = FE_STV0900_SetAbortFlag(hdl, abort, THIS_PATH(priv));
	if (err != FE_LLA_NO_ERROR) {
		pr_err("%s: failed\n", __func__);
		ret = -EINVAL;
	} else {
...
static int stv0900_standby(struct stm_fe_demod_s *priv, bool standby)
{
	...
	hdl = priv->demod_params;
	err = FE_STV0900_SetStandby(hdl, standby, THIS_PATH(priv));
	if (err != FE_LLA_NO_ERROR) {
		pr_err("%s: failed\n", __func__);
		ret = -EINVAL;
	} else {
...
static int stv0900_restore(struct stm_fe_demod_s *priv)
{
	...
	err = set_tsoutput(assoc_priv->demod_params,
				     priv->demod_params, &p1_ts_conf,
				     &p2_ts_conf, path);
	if (err != FE_LLA_NO_ERROR) {
		pr_err("%s: set_tsoutput failed %d\n", __func__, err);
		ret = -EINVAL;
	...
	err = FE_STV0900_SetTSConfig(priv->demod_params, &p1_ts_conf,
...
static int32_t __init stv0900_lla_init(void)
{
	pr_info("Loading stv0900 driver module...\n");

	return 0;
}

module_init(stv0900_lla_init);

static void __exit stv0900_lla_term(void)
{
	pr_info("Removing stv0900 driver module...\n");

}

module_exit(stv0900_lla_term);

MODULE_DESCRIPTION("Low level driver for STV0900 satellite demodulator");
MODULE_AUTHOR("STMicroelectronics Limited");
MODULE_VERSION(STV0900_CURRENT_LLA_REVISION);
MODULE_LICENSE("GPL");
