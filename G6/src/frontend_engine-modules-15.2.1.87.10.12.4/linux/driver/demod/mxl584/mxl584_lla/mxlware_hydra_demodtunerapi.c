...
static  SINT16 spectrumData[1024];
  ...
  if (mxlStatus == MXL_SUCCESS)
  {
    ...
    switch(devHandlePtr->deviceType)
    {
      ...
      case MXL_HYDRA_DEVICE_TEST:
        if (tunerId > MXL_HYDRA_TUNER_ID_3)
          mxlStatus = MXL_INVALID_PARAMETER;
        break;
...
MXL_STATUS_E MxLWare_HYDRA_API_CfgDemodChanTune(UINT8 devId, MXL_HYDRA_TUNER_ID_E tunerId, MXL_HYDRA_DEMOD_ID_E demodId, MXL_HYDRA_TUNE_PARAMS_T * chanTuneParamsPtr)
{
  ...
  MXLENTERAPISTR(devId);
  MXLENTERAPI(MXL_HYDRA_PRINT(" %s  demodId=%d\n", __func__,demodId););
 ...
 * @param[in]   spectrumStepSize
 * @param[in]   spectrumResolution
...
//  SINT16 spectrumData[1024];
  ...
  MXLENTERAPISTR(devId);
  MXLENTERAPI(MXL_HYDRA_PRINT("tunerId=%d, freqStartInHz=%d, freqStepInHz=%d, numOfFreqSteps=%d\n", tunerId, freqStartInKHz, spectrumStepSize, numOfFreqSteps););
  ...
  if (status == MXL_SUCCESS)
  {
    ...
    if (powerBufPtr)
    {
      ...
      if (spectrumStepSize > MXL_HYDRA_STEP_SIZE_24_XTAL_408_20KHZ)
      {
        MXL_HYDRA_PRINT(" Spectrum Step Size not supported : %d", spectrumStepSize);
      ...
      minAgc.stepSizeInKHz = spectrumStepSize;
      minAgc.totalSteps = numOfFreqSteps;

      // build command
      BUILD_HYDRA_CMD(MXL_HYDRA_SPECTRUM_MIN_GAIN_CMD, MXL_CMD_WRITE, cmdSize, &minAgc, cmdBuff);
      // send command to device
      ...
      *spectrumReadbackStatusPtr = (MXL_HYDRA_SPECTRUM_ERROR_CODE_E)(regData & 0xFFFF);
      ...
      if ((status == MXL_SUCCESS) && ((regData & 0xFFFF0000) == 0x10000))
      {
        ...
        endFreq = (freqStartInKHz * 1000) + (freqStep[spectrumStepSize] * numOfFreqSteps);
        tmpNumOfSteps = numOfFreqSteps;

        // loop entire spectrum
        for (startFreq = (freqStartInKHz * 1000);  startFreq < endFreq; startFreq += (freqStep[spectrumStepSize] * maxNumOfSteps[spectrumStepSize]))
        {
          tmpNumOfSteps -= totalSize;
          index += totalSize;

          // if num of steps is not less than max allowed then step size should be valid max num of steps
          if (tmpNumOfSteps < maxNumOfSteps[spectrumStepSize]) totalSize = tmpNumOfSteps;
          else totalSize = maxNumOfSteps[spectrumStepSize];
          ...
          if (status == MXL_SUCCESS)
          {
            ...
            pwrSpectrum.spectrumDivision = spectrumResolutionIndB;
            pwrSpectrum.stepSizeInKHz = spectrumStepSize;
            pwrSpectrum.totalSteps = totalSize;
            pwrSpectrum.startingFreqInkHz = (UINT32)(startFreq/1000);

            MXL_HYDRA_PRINT("tunerId = %d, demodId = %d freqStartInKHz = %d, freqStepInKHz = %d, numOfFreqSteps = %d resolution = %d\n",
                                          tunerId, demodId,
                                          pwrSpectrum.startingFreqInkHz, spectrumStepSize,
                                          pwrSpectrum.totalSteps, spectrumResolutionIndB);

            // build command
            BUILD_HYDRA_CMD(MXL_HYDRA_SPECTRUM_REQ_CMD, MXL_CMD_WRITE, cmdSize, &pwrSpectrum, cmdBuff);
              ...
              *spectrumReadbackStatusPtr = (MXL_HYDRA_SPECTRUM_ERROR_CODE_E)(regData & 0xFFFF);
              ...
              if ((status == MXL_SUCCESS) && ((regData & 0xFFFF0000) == 0x10000) && (spectrumReadyFlag == MXL_TRUE))
              {
                ...
                if (spectrumDataAddr[1])
                {
                  ...
                  // size to MAX
                  if (spectrumDataAddr[0] > maxNumOfSteps[spectrumStepSize])
                    spectrumDataAddr[0] = maxNumOfSteps[spectrumStepSize];
...
MXL_STATUS_E MxLWare_HYDRA_API_ReqDemodConstellationData(UINT8 devId, MXL_HYDRA_DEMOD_ID_E demodId, SINT16 *iSamplePtr, SINT16 *qSamplePtr)
{
  ...
  if ((mxlStatus == MXL_SUCCESS) &&((iSamplePtr) && (qSamplePtr)))
  {
    ...
    // Lock demod status
    HYDRA_DEMOD_STATUS_LOCK(devId,demodId);
    ...
    *qSamplePtr = (SINT16)(regData & 0xFFFF);
    ...
    // Unlock demod status
    HYDRA_DEMOD_STATUS_UNLOCK(devId,demodId);
...
MXL_STATUS_E MxLWare_HYDRA_API_ReqDemodChanParams(UINT8 devId, MXL_HYDRA_DEMOD_ID_E demodId, MXL_HYDRA_TUNE_PARAMS_T *demodTuneParamsPtr)
{
  ...
  if (mxlStatus == MXL_SUCCESS)
  {
	...
	/*MXL_HYDRA_PRINT(" demodIndex : %d\n", demodId);
	MXL_HYDRA_PRINT(" spectrumInfo : %d\n", demodTuneParamsPtr->spectrumInfo);
	MXL_HYDRA_PRINT(" standardMask : %d\n", demodTuneParamsPtr->standardMask);
	MXL_HYDRA_PRINT(" fec : %d\n", demodTuneParamsPtr->params.paramsS.fec);
	MXL_HYDRA_PRINT(" modulation : %d\n", demodTuneParamsPtr->params.paramsS.modulation);
	MXL_HYDRA_PRINT(" rollOff : %d\n", demodTuneParamsPtr->params.paramsS.rollOff);
	*/
