...
UINT32 MxL_Ctrl_GetTsID(MXL_HYDRA_CONTEXT_T *devHandlePtr, MXL_HYDRA_DEMOD_ID_E logicalDemodId)
{
  ...
  return demodMap[devHandlePtr->deviceType][logicalDemodId].physicalTsId;
}
...
void MxL_UpdateDataForEndianness(UINT32 size, UINT8* dataPtr)
{
  UINT32 i;
...
static MXL_STATUS_E MxL_Ctrl_MessageRead(UINT8 devId, UINT16 size, UINT8* buffPtr)
{
  ...
  buffPtr[0] = 0xFD;                                  // Copy PLID
  buffPtr[1] = 0x00;                                  // Hardware Length

  MxL_CovertDataForEndianness(MXL_ENABLE_BIG_ENDIAN, 2, (UINT8 *)buffPtr);
  status = MxLWare_HYDRA_OEM_I2cWrite(devId, 2, buffPtr);  // write 2 bytes to I2C device
...
MXL_STATUS_E MxLWare_HYDRA_CheckForCmdResponse(UINT8 devId, MXL_HYDRA_CONTEXT_T *ctxPtr, UINT8 cmdId)
{
  ...
  if (ctxPtr)
  {
    ...
    while (timeOutCount < 100)
    {
      ...
      MXL_HYDRA_PRINT("[%s] length = %d CMDID = %d", __FUNCTION__, dataLen, respCmd);
      ...
      else
      {
        ...
        for (i = 0; i < (UINT8)ctxPtr->cmdData.dataSize; (i=i+4))
        {
          MXL_HYDRA_PRINT("%02X %02X %02X %02X", ctxPtr->cmdData.data[i],
                                                 ctxPtr->cmdData.data[i+1],
                                                 ctxPtr->cmdData.data[i+2],
                                                 ctxPtr->cmdData.data[i+3]);
        }
...
char * MxLWare_HYDRA_DBG_GetDeviceName(MXL_HYDRA_DEVICE_E deviceType)
{
  switch (deviceType)
  {
    case MXL_HYDRA_DEVICE_581: return "MxL581"; break;
    case MXL_HYDRA_DEVICE_584: return "MxL584"; break;
    case MXL_HYDRA_DEVICE_544: return "MxL544"; break;
    case MXL_HYDRA_DEVICE_585: return "MxL585"; break;
    case MXL_HYDRA_DEVICE_561: return "MxL561"; break;
