                                           ...
                                           MXL_BOOL_E invertOutputPolarity)
{
  ...
  if (mxlStatus == MXL_SUCCESS)
  {
    ...
    mxlStatus |= SET_REG_FIELD_DATA(XPT_BERT_LOCK_THRESHOLD, 0x05);

    // XPT_BERT_LOCK_WINDOW = 0xC8
    mxlStatus |= SET_REG_FIELD_DATA(XPT_BERT_LOCK_WINDOW, 0xC8);
                                            ...
                                            MXL_BOOL_E *bitCountSaturationFlagPtr)
{
  ...
  if (mxlStatus == MXL_SUCCESS)
  {
    if ((totalbitCountPtr) && (totalbitErrorPtr) && (bitCountSaturationFlagPtr))
    {
      ...
      *bitCountSaturationFlagPtr = (MXL_BOOL_E) (regData)?MXL_TRUE:MXL_FALSE;
      ...
      MxLWare_HYDRA_ReadRegisterBlock(devId, xpt_reg_address, 2 * 4, (UINT8 *) buffer);
