...
MODULE_PARM_DESC(lla_wrapper_debug,
		 "Turn on/off mxl582 lla wrapper debugging (default:off).");
...
int stm_fe_mxl582_attach(struct stm_fe_demod_s *priv)
{

	if (!priv->ops)
		return -EFAULT;

	priv->ops->init = mxl582_init;
	priv->ops->tune = mxl582_tune;
	priv->ops->scan = mxl582_scan;
	priv->ops->tracking = mxl582_tracking;
	priv->ops->status = mxl582_status;
	priv->ops->unlock = mxl582_unlock;
	priv->ops->term = mxl582_term;
	priv->ops->abort = mxl582_abort;
	priv->ops->standby = mxl582_standby;
	priv->ops->detach = stm_fe_mxl582_detach;
	priv->ops->restore = mxl582_restore;
	priv->ops->tsclient_init = NULL;
	priv->ops->tsclient_term = NULL;
	priv->ops->tsclient_pid_configure = mxl582_ts_pid_start;
	priv->ops->tsclient_pid_deconfigure = mxl582_ts_pid_stop;

	dpr_info("%s: demod attached\n", __func__);

	return 0;
}
EXPORT_SYMBOL(stm_fe_mxl582_attach);

int mxl582_ts_pid_start(stm_object_h demod_object, stm_object_h
				demux_object, uint32_t pid)
{
	FE_MXL582_Error_t err;
	...
	err = FE_MXL582_cfg_pid_fltr(handle, Demod,
				pid, enable);

	if (err != FE_LLA_NO_ERROR) {
		pr_err("%s: mxl582_ts_pid_start fail\n", __func__);
		ret = -EINVAL;
	}
	return ret;
}

int mxl582_ts_pid_stop(stm_object_h demod_object, stm_object_h
			demux_object, uint32_t pid)
{
	FE_MXL582_Error_t err;
	FE_MXL582_Handle_t handle;
	...
	err = FE_MXL582_cfg_pid_fltr(handle, Demod, pid, disable);
	if (err != FE_LLA_NO_ERROR) {
		pr_err("%s: mxl582_ts_pid_stop fail\n", __func__);
		ret = -EINVAL;
	}
	return ret;
}
...
/*
 * Name: stm_fe_mxl582_detach()
 *
 * Description: Uninstalled the wrapper function pointers for driver
 *
 */

int stm_fe_mxl582_detach(struct stm_fe_demod_s *priv)
{
	if (!priv->ops)
		return -EFAULT;
...
EXPORT_SYMBOL(stm_fe_mxl582_detach);
 ...
 *
 */
int mxl582_init(struct stm_fe_demod_s *priv)
{
	...
	strncpy(ip.DemodName, "MXL582", sizeof(ip.DemodName));
	...
	p = stm_fe_malloc(sizeof(FE_MXL582_InternalParams_t));
	if (!p) {
		pr_err("%s: mem alloc failed for FE_MXL582_InternalParams_t\n",
			__func__);
		return -ENOMEM;
	}
	...
	size = MXL582_HYDRA_NBREGS * sizeof(STCHIP_Register_t);
	...
	/*No tuner with mxl582*/
	err = FE_MXL582_Init(&ip, &priv->demod_params);
	if (err != FE_LLA_NO_ERROR) {
		pr_err("%s: FE_MXL582_Init failed: %d\n", __func__, err);
		ret = -EINVAL;
		goto err_ret;
	}

	err = FE_MXL582_Taging_Muxing(priv);
	if (err != FE_LLA_NO_ERROR) {
		pr_err("%s: FE_MXL582_Taging Muxing failed: %d\n",
					__func__, err);
		ret = -EINVAL;
		goto err_ret;
	}
 ...
 * Name: mxl582_term()
 *
 * Description: terminating the 582 LLA
 *
 */
int mxl582_term(struct stm_fe_demod_s *priv)
{
	int ret = -1;
	...
	/*free up the mutex*/
	 FE_MXL582_Term(priv->demod_params);
 ...
 *
 */
int mxl582_tune(struct stm_fe_demod_s *priv, bool *lock)
{
	FE_MXL582_Error_t err;
	...
	memset(&rp, '\0', sizeof(FE_MXL582_SearchResult_t));
	memset(&sp, '\0', sizeof(FE_MXL582_SearchParams_t));
	...
	err = FE_MXL582_Search((FE_MXL582_Handle_t) hdl,
					priv->demod_id, &sp, &rp);
	if (!err && rp.Locked) {
		*lock = rp.Locked;
 ...
 *
 */
int mxl582_scan(struct stm_fe_demod_s *priv, bool *lock)
{
	FE_MXL582_Error_t err;
	FE_MXL582_SearchParams_t sp;
	FE_MXL582_SearchResult_t rp;
	...
	err = FE_MXL582_Search((FE_MXL582_Handle_t) hdl, priv->demod_id, &sp,
									&rp);
	if (!err && rp.Locked) {
		*lock = rp.Locked;
...
/*
 * Name: mxl582_tracking()
 *
 * Description: Copying the lock parameters of demodulator
 *
 */
int mxl582_tracking(struct stm_fe_demod_s *priv)
{
	FE_MXL582_Error_t err;
	FE_MXL582_Handle_t hdl;
	FE_MXL582_SignalInfo_t sinfo;
	uint32_t packetcount_err = 0/*, packetscount_total*/;
	stm_fe_demod_channel_info_t *info =
		&priv->c_tinfo.u_info.tune.demod_info;
	stm_fe_demod_dvbs2_channel_info_t *isat = &info->u_channel.dvbs2;

	FE_MXL582_DEMOD_t Demod = priv->demod_id;
	hdl = priv->demod_params;
	err = FE_LLA_NO_ERROR;

	memset(&sinfo, '\0', sizeof(FE_MXL582_SignalInfo_t));

	err |= FE_MXL582_GetSignalInfo(hdl, Demod, &sinfo);
	if (!(err)) {
...
int mxl582_status(struct stm_fe_demod_s *priv, bool *locked)
{
	...
	*locked = FE_MXL582_Status(hdl, Demod);
 ...
 *
 */
int mxl582_unlock(struct stm_fe_demod_s *priv)
{
	FE_MXL582_Error_t err;
	FE_MXL582_Handle_t hdl;
	int ret = 0;
	...
	err = FE_MXL582_Unlock(hdl, Demod);
 ...
 */
int mxl582_abort(struct stm_fe_demod_s *priv, bool abort)
{
	FE_MXL582_Error_t err;
	FE_MXL582_Handle_t hdl;
	int ret = 0;
	FE_MXL582_DEMOD_t Demod = priv->demod_id;
	hdl = priv->demod_params;
	err = FE_MXL582_SetAbortFlag(hdl, abort, Demod);
	if (err != FE_LLA_NO_ERROR) {
		pr_err("%s: failed\n", __func__);
		ret = -EINVAL;
	} else {
 ...
 *
 */
int mxl582_standby(struct stm_fe_demod_s *priv, bool standby)
{
	FE_MXL582_Error_t err;
	FE_MXL582_Handle_t hdl;
	int ret = 0;
	FE_MXL582_DEMOD_t Demod = priv->demod_id;
	hdl = priv->demod_params;
	err = FE_MXL582_SetStandby(hdl, standby, Demod);
	if (err != FE_LLA_NO_ERROR) {
		pr_err("%s: failed\n", __func__);
		ret = -EINVAL;
	} else {
...
int mxl582_restore(struct stm_fe_demod_s *priv)
{
	return 0;
}
static int32_t __init mxl582_lla_init(void)
{
	pr_info("Loading mxl582 driver module...\n");

	return 0;
}
module_init(mxl582_lla_init);

static void __exit mxl582_lla_term(void)
{
	pr_info("Removing mxl582 driver module ...\n");

}
module_exit(mxl582_lla_term);


MODULE_DESCRIPTION("Low level driver for MXL582 satellite demodulator");
MODULE_VERSION("2.1.1.7RC100");
MODULE_LICENSE("GPL");
